function allinput = wrapper_level1_conte_loi2_lss(covidx, varargin)
...
for s = 1:length(subdir)
    ...
    for r = 1:length(rundir)
        ...
        for c = 1:length(b.condlabels)
            ...
            runs(r).conditions(1).durations = b.blockwise(c, 4);
            runs(r).conditions(2).name      = 'OTHER';
            runs(r).conditions(2).onsets    = other(:, 3);
    ...
    % w2      = (ismember(cond, [1 3]) - ismember(cond, [2 4]))/2;
    % weights = [w1; w2];
    % ncon    = size(weights,1);
    % conname = {'Why_-_How' 'Face_-_Hand'};
    % for c = 1:ncon
    %     contrasts(c).type       = 'T';
