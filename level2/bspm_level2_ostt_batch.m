function varargout = bspm_level2_ostt_batch(condirpat, varargin)
...
if ~iscell(omitpat), omitpat = cellstr(omitpat); end
rmidx = cellismember(cpath, omitpat); 
cpath(rmidx) = []; 
conlist(rmidx) = []; 
...
for c = 1:length(conname)
    ...
    if runit
        ...
        varargout{c} = bspm_level2_ostt(cons, 'outdir', outdir, 'tag', tag, 'implicit', implicit, 'mask', mask, 'pctgroup', pctgroup, 'negativecon', negativecon, 'nan2zero', nan2zero); 
    end
end
fprintf('\n - DONE - \n\n'); 
