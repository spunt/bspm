function matlabbatch = bsnpm_level2_tstt(cons1, cons2, varargin)
% BSNPM_LEVEL2_OSTT
%
%   ARGUMENTS:
...
if nan2zero, bspm_batch_imcalc([cons1; cons2], '', 'nan2zero'); end
...
if isempty(outdir)
    ...
    if tag
        gasubdir    = fullfile(gadir, sprintf('SnPM_%s_TSTT_%s_%dPERM_vFWHM%dx%dx%d_%s_N%d_vs_%s_N%d_%s_%s', upper(level2correct), tag, nPerm, varianceSmoothing, grouplabels{1}, length(cons1), grouplabels{2}, length(cons2), mtag, bspm_timestamp(1)));
    else
        gasubdir    = fullfile(gadir, sprintf('SnPM_TSTT_%s_%dPERM_vFWHM%dx%dx%d_%s_N%d_vs_%s_N%d_%s_%s', upper(level2correct), nPerm, varianceSmoothing, grouplabels{1}, length(cons1), grouplabels{2}, length(cons2), mtag, bspm_timestamp(1)));
    end
...
matlabbatch{1}.spm.tools.snpm.des.TwoSampT.dir{1}       = outdir; 
matlabbatch{1}.spm.tools.snpm.des.TwoSampT.scans1       = cellstr(cons1);
matlabbatch{1}.spm.tools.snpm.des.TwoSampT.scans2       = cellstr(cons2);
matlabbatch{1}.spm.tools.snpm.des.TwoSampT.nPerm        = nPerm;
matlabbatch{1}.spm.tools.snpm.des.TwoSampT.vFWHM        = varianceSmoothing;
matlabbatch{1}.spm.tools.snpm.des.TwoSampT.bVolm        = 1;
matlabbatch{1}.spm.tools.snpm.des.TwoSampT.ST.ST_U      = clusterFormingThresh;

% | MASKING & GLOBAL CALCULATIONS
matlabbatch{1}.spm.tools.snpm.des.TwoSampT.masking.tm.tm_none            = 1;
matlabbatch{1}.spm.tools.snpm.des.TwoSampT.masking.im                    = implicitmask;
matlabbatch{1}.spm.tools.snpm.des.TwoSampT.masking.em{1}                 = mask;
