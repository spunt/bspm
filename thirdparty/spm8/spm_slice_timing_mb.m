function spm_slice_timing_mb(P, sliceorder, refslice, timing, prefix)
...
%
% Modified by M Erb, at U. Tuebingen, 2013, for multiband
% slice timing
...
% multi-band (to do: richtiger factor oben)
MB=1;
if min(size(sliceorder)) > 1, MB=size(sliceorder,2); end
factor=factor*MB;
...
for subj = 1:nsubjects
    ...
    if nslices_t ~= nslices,
		...
		[ry, rx] = ind2sub(size(sliceorder),rslice); % ry = time point, rx = index of parallel slice
        for k = 1:nslices,

            % Set up time acquired within slice order
            %shiftamount  = (find(sliceorder==k) - rslice) * factor;
		    [sy, sx] = ind2sub(size(sliceorder),find(sliceorder==k)); % sy = time point, sx = index of parallel slice
            %shiftamount  = (sy - rslice) * factor;
            shiftamount  = (sy - ry) * factor;
