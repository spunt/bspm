...
so.help    = {
              ...
              ''
              'for multi-band it must be a T x S matrix. (T=time points, S=parallel slices (MB)'
              'example: [[ 1 4]; [ 3 6]; [ 2 5] ]'
              'you can use: me_get_mb_time'
}';
so.strtype = 'e';
%so.num     = [1 Inf];
so.num     = [Inf Inf];
...
%generic2.name    = 'Data';
generic2.name    = 'Slice order';
generic2.help    = {'Slice order. Array for multi-band.'};
generic2.values  = {so};
generic2.num     = [1 Inf];
% ---------------------------------------------------------------------
...
stmb         = cfg_exbranch;
stmb.tag     = 'stmb';
stmb.name    = 'Slice Timing MB';
stmb.val     = {generic nslices tr ta so refslice prefix };
stmb.help    = {
