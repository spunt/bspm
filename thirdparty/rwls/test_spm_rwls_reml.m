...
% check defaults
%--------------------------------------------------------------------------
try, A; catch, A  = 0;  end         % assume proportional hyperpriors not specified
try, N; catch, N  = 1;  end         % assume a single sample if not specified
try, K; catch, K  = 512; end        % default number of iterations
...
if nargout > 3
    ...
    %----------------------------------------------------------------------
    Ft = spm_trace(hP,inv(Ph)) - length(Ph) - length(Cq);
