...
if nargin < 3
    iSeed = uint16(fGetSeed(dImg, round(size(img,3)/2)));
    if isempty(iSeed), return; end
elseif length(iSeed)==1
    iSeed = uint16(fGetSeed(dImg, iSeed));
    if isempty(iSeed), return; end
else
...
% ***
% =========================================================================
function iSeed = fGetSeed(dImg, iSlice)
dImg = dImg./max(dImg(:));
dImg = dImg - min(dImg(:));
iImg = uint8(dImg.*255);
try
    hF = figure(...
        'Position'             , [0 0 size(dImg, 2)*5 size(dImg, 1)*5], ...
        'Units'                , 'pixels', ...
        'Visible'              , 'off', ...
        'Color'                , 'k', ...
        'DockControls'         , 'off', ...
        'MenuBar'              , 'none', ...
        'Name'                 , 'Click Seed (up/down to change slice)', ...
        'NumberTitle'          , 'off', ...
        'BusyAction'           , 'cancel', ...
        'Pointer'              , 'crosshair', ...
