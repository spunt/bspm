function outm = surfaceFlattening( inm_,varargin )
...
%     C(indices_of_borders(idx),:)=inm.points(indices_of_borders(idx),[1 2]);
    ...
    MA = sparse(rows,cols,[valsMA*lambda(1) zeros(1,numel(indices_of_borders))],inm.npoints,inm.npoints);
    MX = sparse(rows,cols,[valsMX*lambda(2) ones(1,numel(indices_of_borders))],inm.npoints,inm.npoints);
