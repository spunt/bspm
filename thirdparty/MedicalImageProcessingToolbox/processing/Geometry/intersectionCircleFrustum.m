function [theta, point, whichface, thetafrustum] = intersectionCircleFrustum(circle,frustum)
...
for face=1:4
    ...
    if 0
        ...
        hold on; plotCircleArch(circle); hold off;
        axis on;
        grid on;
        xlabel('x')
        ylabel('y')
...
Mfrus = [frustum.directions frustum.centre(:) ; 0 0 0 1];
if ~numel(point)
    point = NaN(3,2);
    theta=[NaN NaN];
    
     % no intersection could be dut to the fact that it is totally
    % contained, in which case the angle to be returned would be [-180 180]
    % we take one random point of the circle and check if it is inside the
    % frustum
    point_of_the_circle_2D = [circle.radius 0 0 1]';
    point_of_the_circle_3D = M*point_of_the_circle_2D ;
    point_of_the_circle_frustum =  Mfrus\point_of_the_circle_3D;
    distance = norm(point_of_the_circle_frustum (1:3));
    % check radiuses
    if distance<frustum.radiuses(1) || distance>frustum.radiuses(2)
        return;
    end
    %check angles
     point_of_the_circle_angles= [atan2(point_of_the_circle_frustum(1,:),point_of_the_circle_frustum(3,:))
                           atan2(point_of_the_circle_frustum(2,:),point_of_the_circle_frustum(3,:))]*180/pi;
    a1 = angleInRange(point_of_the_circle_angles(1,:),frustum.angles(1:2),1E-03);
    a2 = angleInRange(point_of_the_circle_angles(2,:),frustum.angles(3:4),1E-03);
...
mid_point_in_frustum_coordinates = Mfrus\mid_point;
