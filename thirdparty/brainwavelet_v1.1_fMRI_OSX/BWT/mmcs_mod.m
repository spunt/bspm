function Nbmat = mmcs_mod(M,padM,wlh,whh,bin)
%
% FUNCTION:     mmcs_mod -- Finds chain coefficients (moderate harshness).
%                            
...
%                           maxima and minima.
%               padM     -- Padded matrix to accommodate neighbour
...
%% check nscale compatibility

if Nscales<=2
    cprintf('err','\nCannot use this algorithm with 2 scales or less\n');
    Nbmat=[];
    return
end
...
tmpmat=padM;
tmpmat(:,(2+whh:Nscalesx),:)=0;
    
Nbmat(:,1,:)=nghbrcount(padM(:,(1:1+whh+whh),:),wlh,whh,NX,1,Nts,bin);
Nbmat(:,2,:)=nghbrcount(tmpmat(:,(2:2+whh+whh),:),wlh,whh,NX,1,Nts,bin);

%% find scale 1 and 2 chain coefficients

binNbmat=Nbmat(:,(1:2),:);
binNbmat(binNbmat>0)=1;

mmcs_coef=zeros(NX,2,Nts);
mmcs_coef((binNbmat+M(:,(1:2),:))==2)=1;

%% neighbourhood search scales 3(+)

tmpmat=padM;
tmpmat(:,(1+whh:2+whh),:)=circularize(mmcs_coef,'y',wlh);
    
Nbmat(:,(3:Nscales),:)=nghbrcount(tmpmat(:,(3:Nscalesx),:)...
        ,wlh,whh,NX,Nscales-2,Nts,bin);
