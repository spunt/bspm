...
try
    % remove any tiny WM clusters
    white_mask = bramila_clusterextend(white_mask,10);
catch err
   warning('Failed to run bramila_clusterextend (is bramila path in your search folder?)\n (%s)',err.message)
end
...
try
   csf_mask = bramila_clusterextend(csf_mask,5);
catch err
   % no reason to repeat the message
   %warning('Failed to run bramila_clusterextend (%s)',err.message)
end
