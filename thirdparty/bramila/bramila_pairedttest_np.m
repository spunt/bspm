function stats = bramila_pairedttest_np(data,design,niter)
% BRAMILA_PAIREDTTEST_NP - A "non parametric" paired T-test that, instead of
% relying on the t-distribtion, uses permutations of matched subjects (paired ttest) to
...
%
%   - Usage:
%   stats = bramila_pairedttest_np(data,design,niter)
%   - Input:
...
if(length(g1) ~= length(g2))
	error('This is a paired ttest, design variable has to contain the same amount of 1 and 2');
end
% a check on niter
if(niter<0)
...
function pval = tt_np_pval(data,g1,g2,niter,tval)
	...
	dataB=data(g2);
    for iter=1:niter
		% get a random vector of zeros and ones as long as half of the cols in data
		% the 'ones' will be the columns to be swapped
		toswap=sign(randn(1,ND/2));
		ids=find(toswap>0);
		tempA=dataA;
		tempA(ids)=dataB(ids);
		tempB=dataB;
