...
outiter=zeros(niter,2);
parfor iter=1:niter % we use max statistics
    outiter(iter,:) = tt_np_maxstats(data,g1,g2,iter);
end


stats.maxstat(1)=prctile(outiter(:,1),95);
stats.maxstat(2)=prctile(outiter(:,2),5);
end

function outiter = tt_np_maxstats(data,g1,g2,iter)
    disp(num2str(iter))
    % shuffle columns
    ND=size(data,2);
    perm=randperm(ND);
    temp=data(:,perm);
    temptvals=tt_np(temp,g1,g2);
    outiter(1,1)=max(temptvals);
    outiter(1,2)=min(temptvals);
end
