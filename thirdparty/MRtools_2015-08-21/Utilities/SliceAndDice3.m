function [N newTarg] = SliceAndDice3(inputVol,Target,VoxDim,BB,interpOrd,prefix)
...
% if ~isempty(contains('aschultz',{UserTime})); disp('Yo');
%     keyboard;
% end
% try
origin = [];
for ii = 1:3;
    %[r c] = find(tb==tar(ii));
    [r c] = find(abs(tb-tar(ii))==min(min(abs(tb-tar(ii)))));
    if abs(abs(tb(r,c)-tar(ii))) > 1.e-4
        error('Can''t find the origin:  Line 156');
    end
