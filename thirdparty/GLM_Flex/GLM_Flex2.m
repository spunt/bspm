...
if isfield(I,'Transform')
    ...
    if isfield(I.Transform,'crtlFor')
        I.writeFin = 1;
        DD = crtlFor(DD,[ones(size(I.Transform.crtlFor,1),1) I.Transform.crtlFor]);
...
for ll = size(X,1):-1:I.minN
    ...
    for ii = 1:size(ch,1)
        ...
        if I.RemoveOutliers==1 || numel(I.F.Vi)>1;
            %%% Run a basic GLM
            pv = pinv(xx);
        ...
        if I.RemoveOutliers == 1;
            ...
            if isfield(I,'Thresh') && ~isempty(I.Thresh)
                thresh = spm_invFcdf(I.Thresh,[df1 size(xx,1)-df1-1]);
            else
                thresh = spm_invFcdf(.5,[df1 size(xx,1)-df1-1]);
            end
            ...
            if ~isempty(a)
                disp(' ');
                disp(['Found ' num2str(numel(a)) ' Outliers']);
                disp(' ');
                scans = Xind(ith(a));
                cols = vec(a);
...
if isfield(I,'Cons')  
   [I oo] = GLM_Flex_Contrasts2(I,1:length(I.Cons),oo); 
end
