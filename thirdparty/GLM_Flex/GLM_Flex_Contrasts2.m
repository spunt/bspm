function [I oo] = GLM_Flex_Contrasts2(I,these,oo)
...
for ii = these;
    ...
    if ~isfield(Cons(ii),'c') || isempty(Cons(ii).c)
        [trash c] = MakeContrastMatrix2(I.F,I.Cons(ii).Groups,I.Cons(ii).Levs);
    else
        c = Cons(ii).c;
    end
    ...
    for jj = 1:length(oo.Xind)
        ...
        %keyboard;
        if ~isfield(Cons(ii),'c') || isempty(Cons(ii).c)
            [trash c] = MakeContrastMatrix2(I.F,I.Cons(ii).Groups,I.Cons(ii).Levs,Xind);
        ...
        %[trash a] = UserTime; if strcmpi(a,'aschultz'); keyboard; end;
...
function [c cc] = getC(Con,x,CovarCols)
    if iscell(Con.Groups)
        ...
        if any(CovarCols)
            ...
            [r,c,cc] = MakeContrastMatrix('b',Con.Groups,Con.Levs,x,[],CovarCols);
        end
    end
end
