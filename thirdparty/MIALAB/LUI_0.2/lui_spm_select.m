function [t,sts] = lui_spm_select(varargin)
% File selector
% FORMAT [t,sts] = lui_spm_select(n,typ,mesg,sel,wd,filt,frames)
%     n    - Number of files
...
%           'batch' - lui_spm batch files (.mat and XML)
...
%
% FORMAT cpath = lui_spm_select('CPath',path,cwd)
...
%
% FORMAT [files,dirs]=lui_spm_select('List',direc,filt)
...
% FORMAT [files,dirs]=lui_spm_select('ExtList',direc,filt,frames)
% As above, but for selecting frames of 4D NIfTI files
% frames   - vector of frames to select (defaults to 1, if not specified)
% FORMAT [files,dirs]=lui_spm_select('FPList',direc,filt)
% FORMAT [files,dirs]=lui_spm_select('ExtFPList',direc,filt,frames)
% As above, but returns files with full paths (i.e. prefixes direc to each)
%____________________________________________________________________________
% Copyright (C) 2005 Wellcome Department of Imaging Neuroscience

% John Ashburner
% $Id: lui_spm_select.m 941 2007-10-14 17:01:18Z volkmar $
    ...
    switch lower(varargin{1})
        ...
        case {'list', 'fplist', 'extlist', 'extfplist'}
            ...
            if regexpi(varargin{1}, 'fplist') % return full pathnames
                direc = lui_spm_select('cpath', varargin{2});
...
lui_spmdir = fileparts(which(mfilename));
if ( lui_spm_matlab_version_chk('7') >= 0 ) && isdeployed,
    ind = findstr(lui_spmdir,'_mcr')-1;
    [lui_spmdir,junk] = fileparts(lui_spmdir(1:ind(1)));
end;
prevdirs([lui_spmdir filesep]);
    ...
    for j=1:numel(varargin),
        ...
        if ischar(varargin{j}),
            for i=1:size(varargin{j},1),
                [pth{i} n e v] = lui_spm_fileparts(deblank(varargin{j}(i,:)));
                fle{i} = [n e v];
            end;
...
if isempty(pth),
    for k = 1:numel(fle)
    [unused,nam,ext,num] = lui_spm_fileparts(fle{k});
    if ~isempty(num),
        ind = [str2num(num) 1 1];
        ind = ind(1);
    else
...
'lui_spm.  The following is a list of symbols with special meaning for filtering the filenames:'],...
'    ^     start of string',...
'    $     end of string',...
'    .     any character',...
...
switch lui_spm_platform('filesys'),
case 'unx',
    mch = '^/';
    fs  = '/';
    fs1 = '/';
