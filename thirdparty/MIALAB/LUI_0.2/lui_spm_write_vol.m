function V = lui_spm_write_vol(V,Y)
% Write an image volume to disk, setting scales and offsets as appropriate
% FORMAT V = lui_spm_write_vol(V,Y)
% V (input)  - a structure containing image volume information (see lui_spm_vol)
...
%
% Note that if there is no 'pinfo' field, then lui_spm will figure out the
...
% John Ashburner
% $Id: lui_spm_write_vol.m 1020 2007-12-06 20:20:31Z john $
...
%-----------------------------------------------------------------------
V = lui_spm_create_vol(V);
V = lui_spm_write_plane(V,Y,':');
