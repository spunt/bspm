function [strout,R2] = lui_spm_str_manip(strin,options)
% miscellaneous string manipulation options
% FORMAT string_out = lui_spm_str_manip(string_in,options)
...
%                          are replaced by './'.
%                          eg. lui_spm_str_manip('/dir1/dir2/file.img','a16') would
%                          produce '../dir2/file.img'.
%       'v'              - delete non valid filename characters
%                          Valid are '.a..zA..Z01..9_-: ' & filesep
%       'p'              - canonicalise pathname (see lui_spm_select('CPath',strin))
...
% John Ashburner
% $Id: lui_spm_str_manip.m 765 2007-03-15 13:57:33Z volkmar $
...
while (~isempty(options))
	...
	for i=1:prod(size(strout))
		...
		case 'p'
			str = strvcat(lui_spm_select('CPath',cellstr(str)));
