function V = lui_spm_vol(P)
% Get header information etc for images.
% FORMAT V = lui_spm_vol(P)
...
%       V.dt    - A 1x2 array.  First element is datatype (see lui_spm_type).
...
%
% Note that lui_spm_vol can also be applied to the filename(s) of 4-dim
% volumes. In that case, the elements of V will point to a series of 3-dim
% images.
%
% This is a replacement for the lui_spm_map_vol and lui_spm_unmap_vol stuff of
% MatLab4 lui_spms (lui_spm94-97), which is now obsolete.
%_______________________________________________________________________
% Copyright (C) 2005 Wellcome Department of Imaging Neuroscience

% John Ashburner
% $Id: lui_spm_vol.m 982 2007-10-26 14:01:54Z john $
...
[pth,nam,ext,n1] = lui_spm_fileparts(deblank(p));
...
if isempty(n),
    V = lui_spm_vol_nifti(p);
else
    V = lui_spm_vol_nifti(p,n);
end;
    
if isempty(n) && length(V.private.dat.dim) > 3
    V0(1) = V;
    for i = 2:V.private.dat.dim(4)
        V0(i) = lui_spm_vol_nifti(p, i);
    end
    V = V0;
end
...
str = {...
	...
	['        ',lui_spm_str_manip(q,'k40d')],...
	' ',...
	'Please check that it is in the correct format.'};
lui_spm('alert*',str,mfilename,sqrt(-1));
return;
%_______________________________________________________________________
