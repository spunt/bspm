function [status, fieldsUsed] = lui_spm_matlab_version_chk(chk,tbx)
% lui_spm_MATLAB_VERSION_CHK Check a version number against a Toolbox version.
%    FORMAT [STATUS FIELDSUSED] = lui_spm_MATLAB_VERSION(CHK,TBX)
%
...
%       status = lui_spm_matlab_version_chk('7');
...
%       status = lui_spm_matlab_version_chk('7.2');
...
%       status = lui_spm_matlab_version_chk('6.5.0');
%       returns status == 1     : major + minor + release revision is later
%                                 for matlab
%       The statement ( lui_spm_matlab_version_chk('6.5.0') > 0 ) is true for
...
%
% Darren Gitelman
% $Id: lui_spm_matlab_version_chk.m 539 2006-05-19 17:59:30Z Darren $
