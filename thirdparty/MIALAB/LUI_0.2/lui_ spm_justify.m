function out = lui_spm_justify(varargin)
% lui_spm_JUSTIFY Justifies a text string
%    OUT = lui_spm_JUSTIFY(N,TXT) justifies text string TXT to
%    the length specified by N.
%
%    OUT = lui_spm_JUSTIFY(OBJ,TXT) justifies text string to
...
%
%    Example:
%    out = lui_spm_justify(40,{['Statistical Parametric ',...
...
%    'in software that is called lui_spm']});
%    strvcat(out{:})
%
%------------------------------------------------------------------------
...
% John Ashburner
% $Id: lui_spm_justify.m 491 2006-03-31 17:36:45Z john $
...
if nargin < 2
    error('Incorrect usage of lui_spm_justify.')
end
n = varargin{1};
if ishandle(n)
