...
function [ IDX, C, sumd ] = ipctb_pir_kmeans2( X, k, prm )
...
prm = ipctb_pir_getPrmDflt( prm, dfs );
...
for i=1:nTrial
  ...
  if( display); disp(msg); end
  [IDX,C,sumd,nIter]=ipctb_pir_kmeans2main(X,k,nOutl,minCl,maxIter,display,metric);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [ IDX, C, sumd, nIter ] = ipctb_pir_kmeans2main( X, k, nOutl, ...
...
% ce032908 substitute of: index = stat\randsample(N,k)
index = zeros(k,1);
for i=1:k
    bRedo = true;
    while bRedo
        nTry = round(.5+rand*(N-.000000001));
        bRedo = false;
        for i2 = 1:(i-1)
            if nTry == index(i2)
                bRedo = true;
...
while( sum(abs(oldIDX - IDX)) ~= 0 && nIter < maxIter)
  ...
  oldIDX = IDX;  D = ipctb_pir_pdist2( X, C, metric ); [mind IDX] = min(D,[],2);
  ...
  if( k==0 ); IDX( ipctb_pir_randint2( 1,1, [1,N] ) ) = 1; k=1; end;
  for i=1:k;
  ...
  nIter = nIter+1;
  if( display )
    fprintf( [repmat('\b',[1 ndisdigits]) ipctb_pir_int2str2(nIter,ndisdigits)] );
  end;
end
