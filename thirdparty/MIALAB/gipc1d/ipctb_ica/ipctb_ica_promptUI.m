function [answer, pos] = ipctb_ica_promptUI(uiType,promptString,choiceString,returnType,H)
...
if whichVersion > 13
    if ~isempty(existingUIPos)
        if ipctb_ica_findstr(uiType, 'popup')
...
if(strcmp(uiType,'popup'))
    ...
    pos = [.05 y0 .9 .05];
    [popupHandle]=ipctb_ica_getUIPopUp(H,string,pos,'','on','prompt');
...
if(strcmp(uiType,'edit'))
    ...
    [labelHandle]=ipctb_ica_getUILabel(H,promptString,pos1,'on','prompt');
    [editHandle]=ipctb_ica_getUIEdit(H,choiceString,pos2, keypressCallback, 'on','prompt');
...
if(strcmp(uiType,'listbox'))
    string = str2mat(promptString,choiceString);
    pos = [.05 y0 .9 .05];
    [listHandle]=ipctb_ica_getUIlistbox(H,string,pos,'','on','prompt');
