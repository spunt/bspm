function [data_sets, numOfSub, numOfSess, selected_data_sets, data_folders] = ipctb_ica_get_sub_data(sourceDir, filePattern, ...
    flagFolder, verbose)
...
% [data_sets, numOfSub, numOfSess] = ipctb_ica_get_sub_data(sourceDir,
...
% get subject folders
subFolders = ipctb_ica_listSubFolders(sourceDir);
...
if ~isempty(subFolders)
    ...
    for ii = 1:size(subFolders, 1)
        if strcmpi(flagFolder, 'data_in_subject_folder')
            ...
            try
                tempSub = ipctb_ica_listSubFolders(deblank(subFolders(ii, :)));
            catch
                tempSub = [];
            end
        end
        ...
        if ~isempty(tempSub)
            for jj = 1:size(tempSub, 1)
                try
                    tempF = ipctb_ica_listFiles_inDir(deblank(tempSub(jj, :)), filePattern);
                catch
                    tempF = [];
                end
                if ~isempty(tempF)
                    ...
                    data_sets(data_count).name = ipctb_ica_fullFile('files', tempF, 'directory', deblank(tempSub(jj, :)));
    ...
    currentFiles = ipctb_ica_listFiles_inDir(subFolder, filePattern);
    if ~isempty(currentFiles)
        % full file names
        data_sets(1).name = ipctb_ica_fullFile('files', currentFiles, 'directory', sourceDir);
        sessionVec = 1;
