function [varargout] = ipctb_ica_select_data(varargin)
...
[InputHandle] = ipctb_ica_getGraphics(titleFig, 'normal', titleFig);
...
if ~isempty(figure_menu)
    ...
    if strcmp(lower(figure_menu), 'spm')
        menu2H = uimenu('parent', menu1H, 'label', 'SPM File Window', 'callback', 'ipctb_ica_openHTMLHelpFile(''SPMFileSelection.htm'');');
    else
        menu2H = uimenu('parent', menu1H, 'label', [modalityType, ' Data Window'], 'callback', 'ipctb_ica_openHTMLHelpFile(''fMRIDataSelectionWindow.htm'');');
    end
end
...
% plot frame
frameH = ipctb_ica_uicontrol('parent', InputHandle, 'units', 'normalized', 'style', 'frame', 'position', framePos);
...
selectedTextH = ipctb_ica_uicontrol('parent', InputHandle, 'units', 'normalized', 'style', 'text', 'position', ...
...
listH = ipctb_ica_uicontrol('parent', InputHandle, 'units', 'normalized', 'style', 'listbox', 'position', listPos, ...
...
viewH = ipctb_ica_uicontrol('parent', InputHandle, 'units', 'normalized', 'position', pushPos1, 'style', 'pushbutton', ...
    'string', 'View', 'tag', 'view', 'callback', {@viewFilesCallback, listH});

changeH = ipctb_ica_uicontrol('parent', InputHandle, 'units', 'normalized', 'position', pushPos2, 'style', 'pushbutton', ...
...
helpH = ipctb_ica_uicontrol('parent', InputHandle, 'units', 'normalized', 'position', pushPos3, 'style', 'pushbutton', ...
    'BackgroundColor', BUTTON_COLOR, 'ForegroundColor', [1 0 0], 'fontunits', UI_FONTUNITS, 'fontname', ...
    UI_FONTNAME, 'FontSize', UI_FS, 'string', '?', 'tag', 'help', 'fontweight', 'bold', 'userdata', helpData, ...
    'callback', {@helpCallback, InputHandle});

okH = ipctb_ica_uicontrol('parent', InputHandle, 'units', 'normalized', 'position', pushPos4, 'style', 'pushbutton', ...
...
try
    ...
    if isfield(figureData, 'extns')
        % get the extensions and file name
        extns = figureData.extns;
    else
        extns.data.file.ext = [];
        extns.data.file.file_name = [];
    end
    ...
    if isempty(dataFiles(getValue).name) | ~strcmp(lower(figureData.changeFiles), 'no')
        ...
        [dataFiles(getValue).name, filter_answer] = ipctb_ica_selectEntry('typeEntity', 'file', 'typeselection', type_file_selection, ...
            'num_datasets', 2, 'count_datasets', 1, 'filter', filter_answer, 'title', promptString, 'startPath', ...
            currentDir);
    else
        %         msgString = ['Do you want to change the previously selected files?'];
        %         [answerQuestion] = questionDialog('title', 'Selecting Files?', 'textbody', msgString);
        %         if answerQuestion == 1
        %             [dataFiles(getValue).name] = ipctb_ica_selectEntry('typeEntity', 'file', 'typeselection', type_file_selection, ...
        %                 'num_datasets', 2, 'count_datasets', 1, 'filter', filter_string, 'title', promptString, 'startPath', ...
        %                 currentDir);
        %         end
    end
    ...
    %numberFiles = size(dataFiles(getValue).name, 1); % number of files

    % use the new function to get the time points information (handles nii
    % format also)
    [numberFiles, extensions, dims] = ipctb_ica_get_countTimePoints(dataFiles(getValue).name);
    ...
    % loop over files for current extension
    for nExts = 1:length(extensions.data(1).file)
        extns.data(getValue).file(nExts).ext =  extensions.data(1).file(nExts).ext;
        extns.data(getValue).file(nExts).file_name =  extensions.data(1).file(nExts).file_name;
    end
    ...
    if ~strcmp(spm_check, 'no')
        try
            ...
            for ii = 1:numOfSess
                % design information
                [spmData] = ipctb_ica_loadSPM_new('spmName', dataFiles(getValue).name, 'countTimePoints', ...
            ...
            %[SPM] = ipctb_ica_checkSPM(pwd, dataFiles(getValue).name, diffTimePoints(getValue));
        catch
    ...
    set(handles, 'pointer', 'arrow');
    ipctb_ica_errorDialog(lasterr, 'File selection error');
    disp(lasterr);
end
...
try
    ...
    [pathstr2, name2, extn2] = fileparts(deblank(filter_string));
    % get the extns variable
    if isfield(figureData, 'extns')
        % get the extns data
        extns = figureData.extns;
    end
    ...
    for ii = 1:length(dataFiles)
        ...
        %[numFiles(ii), extns] = ipctb_ica_get_countTimePoints(P); %size(P, 1);
        numFiles(ii) = figureData.dataCount(ii);

        % loop over the size of files
        for jj = 1:size(P, 1)
            % get the current file extension
            ext = extns.data(ii).file(jj).ext;

            [ext] = ipctb_ica_parseExtn(ext);

            % check the file naming
            if strcmpi(extn2, '.nii') | strcmpi(extn2, '.img')
                if ~strcmpi(ext, '.nii') & ~strcmpi(ext, '.img')
                    error([' Data format for ', listString(ii, :), ' should be .nii or .img']);
                end

            else

                % check the .MAT extension or other
                if ~strcmpi(ext, extn2)
                    error([' Data format for ', listString(ii, :), ' should be ', extn2]);
                end
            ...
            name = extns.data(ii).file(jj).file_name;

            % check the imaginary file naming
            if strcmp(dataType, 'complex')
                underScorePositions = ipctb_ica_findstr(name, '_');
                if ~isempty(underScorePositions)
                    % check the under score positions for the beginning
                    % letters
                    status1 = strcmp(name(1:underScorePositions(1)), complex_file_naming{2});
                    % check the under score positions for the ending
                    % letters
                    status2 = strcmp(name(underScorePositions(end):end), complex_file_naming{2});
                    if status1 == 0 & status2 == 0
                        % check file naming
                        if strcmp(complex_file_naming{2}(1), '_')
                            error([' files naming should end with ', complex_file_naming{2}]);
                        elseif strcmp(complex_file_naming{2}(end), '_')
                            error([' files naming should start with ', complex_file_naming{2}]);
                        else
                            error('Please check the READ_NAMING_COMPLEX_IMAGES variable in ipctb_ica_defaults.m. It should start or end with _');
                        end
                        % end for checking
                    end
                else
                    error('File names should contain prefixes or suffixes to distinguish complex images');
                end
            end
        end
        % end for loop over the files in the current data
...
% open dialog box
figHandle = ipctb_ica_dialogBox('title', titleFig, 'textBody', textBody, 'textType', 'large');
waitfor(figHandle);
...
% open dialog box
figHandle = ipctb_ica_dialogBox('title', getUserdata.title, 'textBody', getUserdata.string, 'textType', 'large');
...
if isfield(userData, 'fileName')
    ...
    fileName = ipctb_ica_inputdlg2(prompt, dlg_title, num_lines, def);
    % make a full file
...
if strcmp(lower(fileFormat), 'append')
    ...
    ipctb_ica_save(fileName, 'SPMFiles', '-append');
else
    files = get(handles, 'userdata');
    SPMFiles.name = [];
    ipctb_ica_save(fileName, 'files', 'SPMFiles', 'numOfSub', 'numOfSess');
end
...
disp(dispString);
% open dialog box
figHandle = ipctb_ica_dialogBox('title', 'File Saved', 'textBody', dispString, 'textType', 'large');
waitfor(figHandle);
...
dialogHandle = ipctb_ica_dialogBox('title', 'Information on number of images for each dataset', 'textBody', ...
