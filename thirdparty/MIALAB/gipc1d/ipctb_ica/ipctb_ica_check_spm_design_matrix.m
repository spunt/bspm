function ipctb_ica_check_spm_design_matrix(designMatrix, numOfSub, numOfSess, diffTimePoints, spmMatFlag, inputFile)
...
if ~isempty(designMatrix(1).name)
    ...
    if strcmpi(spmMatFlag, 'same_sub_same_sess')
        ...
        % do spm checking
        [spmData] = ipctb_ica_loadSPM_new('spmName', deblank(designMatrix(1).name), 'countTimePoints', ...
    ...
    elseif strcmpi(spmMatFlag, 'same_sub_diff_sess')
        ...
        for nSub = 1:numOfSub
            ...
            checkTp = find(checkTimePoints == 0);
            if ~isempty(checkTp)
                error(['The number of images cannot be different over subjects. Use '
                    ' diff_sub_diff_sess value for keyword_designMatrix variable in file: ', ...
                    inputFile, ' if you want to run the analysis.']);
            end
        ...
        for ii = 1:numOfSub
            ...
            % loop over sessions
            for jj = 1:numOfSess
                [spmData] = ipctb_ica_loadSPM_new('spmName', deblank(designMatrix(ii).name), ...
