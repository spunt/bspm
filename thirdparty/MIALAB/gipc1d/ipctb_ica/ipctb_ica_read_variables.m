function [inputData, status, message] = ipctb_ica_read_variables(file_name, keywd, varFlag, varCriteria, dispError)
...
if ~strcmpi(extns, '.m')
    ...
    try
        ...
        if strcmpi(varFlag, 'scalar')
            currentVar = ipctb_ica_read_scalar_file(fid, keywd, 'string');
            % get the status and message
            [status, message] = ipctb_ica_errorCheck(currentVar, varCriteria, keywd);
            if status == 0
                error(message);
            end
        else
            currentVar = ipctb_ica_read_vector_file(fid, keywd, 'string');
        end
    ...
    % get the status and message
    [status, message] = ipctb_ica_errorCheck(checkVar, varCriteria, keywd);
