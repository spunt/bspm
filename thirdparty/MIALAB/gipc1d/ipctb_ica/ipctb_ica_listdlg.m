function [selection, value] = ipctb_ica_listdlg(varargin)
...
[normalPos] = ipctb_ica_getfigPosition('normal');
...
if mod(length(varargin), 2) ~= 0
    ...
    error('Arguments to ipctb_ica_listdlg must in pairs.')
end
...
for ii = 1:2:length(varargin)
    switch lower(varargin{ii})
        ...
        otherwise
            error(['Unknown parameter name passed to ipctb_ica_listdlg.  Name was ' varargin{ii}])
    end
end

% Setup figure for GUI
figPos = ipctb_ica_getfigPosition('normal');
...
ipctb_ica_movegui(InputHandle, moveguiOption);
...
% prompt string text
promptTextH = ipctb_ica_uicontrol('parent', InputHandle, 'units', 'normalized', 'style', 'text', 'position', textPos, ...
...
okH = ipctb_ica_uicontrol('parent', InputHandle, 'units', 'normalized', 'style', 'pushbutton', 'position', okPos, 'string', okString, ...  
    'tooltipstring', 'Select Entries...', 'tag', 'ok', 'userdata', okData);

cancelH = ipctb_ica_uicontrol('parent', InputHandle, 'units', 'normalized', 'style', 'pushbutton', 'position', cancelPos, 'string', cancelString, ...
   'tooltipstring', 'Cancel Entries...', 'tag', 'cancel');

% plot help button if necessary
if exist('helpDetails', 'var')
    % Help push button
    helpH = ipctb_ica_uicontrol('parent', InputHandle, 'units', 'normalized', 'style', 'pushbutton', 'position', helpPos, 'string', '?', 'BackgroundColor', BUTTON_COLOR,...
...
if smode == 2
    % Help push button
    selectAllH = ipctb_ica_uicontrol('parent', InputHandle, 'units', 'normalized', 'style', 'pushbutton', 'position', selectAllPos, 'string', 'Select-All', ...
...
figHandle = ipctb_ica_dialogBox('title', titleString, 'textBody', str, 'textType', 'large');
...
try
    ...
    if ~isempty(maxSelection)        
        ...
        if length(ad.selection) > maxSelection
            ...
            ipctb_ica_errorDialog(errorMsg);        
        else            
            ad.selection = get(listbox, 'value');
