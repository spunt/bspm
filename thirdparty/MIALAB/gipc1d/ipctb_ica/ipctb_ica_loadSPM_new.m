function [spmData] = ipctb_ica_loadSPM_new(varargin)
...
try
    ...
    if strcmp(check_spm_design_matrix, 'yes')
        ...
        spmMat = ipctb_ica_includeFields_struct(spmMat, fieldsReq1);
        clear fieldsReq1;
        ...
        if isfield(Sess_new, 'U')
            ...
            if ~isempty(Sess_new(1).U)
                Sess_new = ipctb_ica_includeFields_struct(Sess_new, {'U'});
                fieldsReq1 = {'name', 'ons'};
                % include only the required fields
                for ii = 1:length(Sess_new)
                    Sess_new(ii).U =  ipctb_ica_includeFields_struct(Sess_new(ii).U, fieldsReq1);
                end
                clear fieldsReq1;
            end
    ...
    if strcmp(lower(flag_selecting_regressors), 'yes')
        ...
        if strcmp(lower(analType), 'batch')
            ...
            if strcmp(typeStr, 'single')
                [getIndex, name_button] = ipctb_ica_listdlg('PromptString', prompt, 'SelectionMode','single',  'ListString', sel_refnames, ...
                    'movegui', 'east', 'windowStyle', 'modal', 'title_fig', title_fig);
            else
                if isempty(maxSelection)
                    [getIndex, name_button] = ipctb_ica_listdlg('PromptString', prompt, 'SelectionMode','multiple',...
                        'ListString', sel_refnames, 'movegui', 'east', 'windowStyle', 'modal', ...
                        'title_fig', title_fig);
                else
                    [getIndex, name_button] = ipctb_ica_listdlg('PromptString', prompt, 'SelectionMode','multiple',...
                        'ListString', sel_refnames, 'movegui', 'east', 'windowStyle', 'modal', ...
    ...
    if strcmp(lower(get_timecourses), 'yes')
        ...
        if strcmp(lower(flag_refFunc), 'session_specific') & ~isempty(data_sessionNumber)
            ...
            for ii = 1:length(getIndex)
                ...
                [sesNum] = ipctb_ica_get_onsets(deblank(selectedRegressors(ii, :)), spmMat);
                if sesNum == 1
                    startTp = 1;
                    endTp = nscan(1);
                else
