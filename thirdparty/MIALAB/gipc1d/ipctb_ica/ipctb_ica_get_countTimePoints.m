function [diffTimePoints, extns, dims] = ipctb_ica_get_countTimePoints(files)
...
for ii = 1:length(files)
    ...
    for jj = 1:size(currentFiles, 1)
        ...
        if strcmpi(extns.data(ii).file(jj).ext, '.nii')
            ...
            [hdr] = ipctb_ica_read_hdr(deblank(currentFiles(jj, :)));            
        ...
        elseif strcmpi(extns.data(ii).file(jj).ext, '.img')
            ...
            [hdr, otherendian] = ipctb_ica_read_hdr(hdrFile);            
