function varargout = ipctb_spm_SpUtil(varargin)
...
if ~ipctb_spm_sp('isspc',varargin{2})
	sX = ipctb_spm_sp('Set',varargin{2});
else,   sX = varargin{2}; end
if nargin==2, c=eye(ipctb_spm_sp('size',sX,2)); else, c=varargin{3}; end;
if isempty(c), varargout={[]}; return, end

switch lower(action)
	case 'iscon'
		varargout = { ipctb_spm_sp('eachinspp',sX,c) };
	case 'allcon'
		varargout = {ipctb_spm_sp('isinspp',sX,c)};
	case 'conr'
		if size(c,1) ~= ipctb_spm_sp('size',sX,2) 
			error('Contrast not of the right size'), end
		%-Compute inner products of data weight vectors
		% (c'b = c'*pinv(X)*Y = w'*Y
		% (=> w*w' = c'*pinv(X)*pinv(X)'*c == c'*pinv(X'*X)*c
		r   = c'*ipctb_spm_sp('XpX-',sX)*c;
	...
	case 'cono'
		...
		varargout = { abs(c'* ipctb_spm_sp('XpX',sX) *c) < sX.tol};
end
...
if ~ipctb_spm_sp('isspc',sX),  sX = ipctb_spm_sp('set',varargin{2}); end;
if sX.rk == 0, error('c->Tsp null rank sX == 0'); end;
if ~isempty(c) & ipctb_spm_sp('size',sX,2) ~= size(c,1), 
...
%-------------------------------------------
if ~isempty(c) & ~ipctb_spm_sp('isinspp',sX,c),
   warning([sprintf('\n') 'c is not a proper contrast in ' action ...
            ' in ' mfilename sprintf('\n') '!!! projecting...' ]);
   disp('from'), c, disp('to'), c = ipctb_spm_sp('oPp:',sX,c)
end
...
case {0,1}
   if ~isempty(c) & any(any(c))                 %- c not empty & not null
      if cukFlag, varargout = { ipctb_spm_sp('cukxp-:',sX,c) };
      else        varargout = { ipctb_spm_sp('xp-:',sX,c) };
      end
   else if isempty(c), varargout = { [] };      %- c empty
        else,                                   %- c null
           if cukFlag, varargout = { ipctb_spm_sp('cukx',sX,c) }; 
           else,       varargout = { ipctb_spm_sp('x',sX)*c };
           end
        end;
   end
...
case 2
   if ~isempty(c) & any(any(c))         %- not empty and not null
      if cukFlag,
         varargout = {	
	    ipctb_spm_sp('cukxp-:',sX,c), ...				%- X1o
	    ipctb_spm_sp('cukx',sX,ipctb_spm_sp('r',ipctb_spm_sp('set',c))) };    %- X0
      else
         varargout = {	
	    ipctb_spm_sp(':',sX, ipctb_spm_sp('xp-:',sX,c)), ...            %- X1o
	    ipctb_spm_sp(':',sX, ...
               ipctb_spm_sp('x',sX)*ipctb_spm_sp('r',ipctb_spm_sp('set',c))) };   %- X0
      end
   else            
      if isempty(c),                    %- empty
         if cukFlag, varargout = { [], ipctb_spm_sp('cukx',sX) }; 
         else,       varargout = { [], ipctb_spm_sp('x',sX) };
         end
      else,                             %- null
         if cukFlag, 
            varargout = { ipctb_spm_sp(':',sX,ipctb_spm_sp('cukx',sX,c)), ...
                          ipctb_spm_sp(':',sX,ipctb_spm_sp('cukx',sX)) }; 
         else,
            varargout = { ipctb_spm_sp('x',sX)*c, ipctb_spm_sp('x',sX)};
         end
      end;
   end
...
%=======================================================================
% X1o = ipctb_spm_SpUtil('i0->X1o',sX,i0)

% arguments are checked in calls to ipctb_spm_Util
%--------------------------------------------
...
c  = ipctb_spm_SpUtil('i0->c',sX,i0);

if cukFlag, 
   varargout = { ipctb_spm_SpUtil('+c->TSp',sX,c) };
else 
   varargout = { ipctb_spm_SpUtil('c->TSp',sX,c) };
end
...
if ~ipctb_spm_sp('isspc',sX),  sX = ipctb_spm_sp('set',varargin{2}); end;
if ipctb_spm_sp('rk',sX) == 0, error('i0->c null rank sX == 0'); end;
sL	= ipctb_spm_sp('size',sX,2);
...
if ~ ipctb_spm_sp('isinspp',sX,c0), c0 = ipctb_spm_sp('oPp:',sX,c0); end;
if ~ ipctb_spm_sp('isinspp',sX,c1), c1 = ipctb_spm_sp('oPp:',sX,c1); end;

if ~isempty(c1)
   if ~isempty(c0) 
      %- varargout = { ipctb_spm_sp('res',ipctb_spm_sp('set',opp*c0),opp*c1) };
      %- varargout = { c1 - c0*pinv(c0)*c1 }; NB: matlab pinv uses
      %- svd: will fail if ipctb_spm_sp('set')  fails. 

      varargout = { ipctb_spm_sp('r:',ipctb_spm_sp('set',c0),c1) };

   else varargout = { ipctb_spm_sp('xpx',sX) }; end;
else
...
case {'+x0->c','x0->c'}				%- 
%=======================================================================
% c = ipctb_spm_SpUtil('X0->c',sX,X0)
% c = ipctb_spm_SpUtil('+X0->c',sX,cukX0)
% + version of 'x0->c'. 
% The + version returns the same in the base u(:,1:r).
...
if ~ipctb_spm_sp('isspc',sX),  sX = ipctb_spm_sp('set',varargin{2}); end;
if ipctb_spm_sp('rk',sX) == 0, error(['null rank sX == 0 in ' action]); end;
if cukFlag
   if ~isempty(cukX0) & ipctb_spm_sp('rk',sX) ~= size(cukX0,1),
      cukX0, ipctb_spm_sp('rk',sX),
      error(['cukX0 of wrong size ' mfilename ' ' action]), end;
else
   if ~isempty(X0) & ipctb_spm_sp('size',sX,1) ~= size(X0,1),
      X0, ipctb_spm_sp('size',sX,1),
      error(['X0 of wrong size ' mfilename ' ' action]),X0, end;
end
%--------- end argument check --------- 

if cukFlag
   if isempty(cukX0), X0 = []; else, X0 = ipctb_spm_sp('ox',sX)*cukX0; end
end
...
%=======================================================================
% [trRV,trRVRV]= ipctb_spm_SpUtil('trRV',x[,V])

if nargin == 1, error('insufficient arguments');
else sX = varargin{2};  end;

if ~ipctb_spm_sp('isspc',sX), sX = ipctb_spm_sp('Set',sX); end;

rk = ipctb_spm_sp('rk',sX);
sL = ipctb_spm_sp('size',sX,1);
...
%=======================================================================
% [trMV, trMVMV]] = ipctb_spm_SpUtil('trMV',sX [,V])
%
% NB : When V is given empty, the routine asssumes it's identity
% This is used in ipctb_spm_FcUtil.
...
if isempty(rk)
	...
	return; 
elseif  rk==0, warning('Rank is null in ipctb_spm_SpUtil trMV ');
...
%=======================================================================
% [df1,df2] = ipctb_spm_SpUtil('i0->edf',sX,i0,V)
%-----------------------------------------------------------------------
%--------- begin argument check ---------------------------------------- 
if nargin<3, error('insufficient arguments'),
else, i0 = varargin{3}; sX = varargin{2}; end
if ~ipctb_spm_sp('isspc',sX), sX = ipctb_spm_sp('Set',sX); end;
i0 	= sf_check_i0(i0,ipctb_spm_sp('size',sX,2));
if nargin == 4, V=varargin{4}; else, V = eye(ipctb_spm_sp('size',sX,1)); end;
if nargin>4, error('Too many input arguments'), end;
%--------- end argument check ------------------------------------------ 

warning(' Use F-contrast utilities if possible ... ');

[trRV,trRVRV]    = ipctb_spm_SpUtil('trRV', sX, V);
[trMpV,trMpVMpV] = ipctb_spm_SpUtil('trMV',ipctb_spm_SpUtil('i0->x1o',sX, i0),V);
...
%=======================================================================
% sz = ipctb_spm_SpUtil('size',x,dim)
...
case 'ix0check'                                      %-
%=======================================================================
% i0c = ipctb_spm_SpUtil('iX0check',i0,sL)
...
otherwise
%=======================================================================
error('Unknown action string in ipctb_spm_SpUtil')
...
if ~isempty(X0)
   ...
   sc0 = ipctb_spm_sp('set',ipctb_spm_sp('x-',sX,X0));
   if  sc0.rk
      c = ipctb_spm_sp('oPp:',sX,ipctb_spm_sp('r',sc0));
   else 
      c = ipctb_spm_sp('oPp',sX);
   end;
   c  = c(:,any(c));
   sL = ipctb_spm_sp('size',sX,2);
   ...
   c = ipctb_spm_sp('xpx',sX);
end

%
%- c = ipctb_spm_sp('r',sc0,ipctb_spm_sp('oxp',sX)); would also works.
