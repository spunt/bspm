function varargout=ipctb_spm_conman(varargin)
...
if (nargin==0) | ~ischar(varargin{1})
    ...
    [F,cF] = ipctb_spm_conman('Initialise','on',SPM,STATmode,n,Prompt,Mcstr,OK2chg);
    ...
    ipctb_spm_conman('Initialise','off');
...
switch lower(varargin{1}), case 'initialise'
    %=======================================================================
    % [F,cF] = ipctb_spm_conman('Initialise',Vis,xX,xCon,STATmode,n,Prompt,Mcstr,OK2chg)
    ...
    switch lower(Vis), case 'close'
        ...
        case 'on'
            ...
            if isempty(F)					%-Create ConMan win
                [F,H] = ipctb_spm_conman('CreateFig');
            else						%-Get handles
            ...
            STAT = ipctb_spm_conman('TFA',F,'',STATmode);		%-Init. TFA buttons
            set(H.hPrompt,'String',Prompt,'UserData',n)	%-Set prompt
            ipctb_spm_conman('ImDesMtx',xX,H.hDesMtxAx)		%-Depict DesMtx
            ipctb_spm_conman('ImParEst',xX,H.hParEstAx)		%-Parameter estimability
            ipctb_spm_conman('ListCon',H.hConList,xCon,STAT,[])	%-List contrasts
            if OK2chg, tmp='on'; else, tmp='off'; end	%-OK to change xCon?
            set(H.hNew,'Enable',tmp)			%-En/dis-able change UI
            %-****	if isempty(xCon), ipctb_spm_conman(); end		%-Go straight to DNewUI
    ...
    case 'imdesmtx'
        %=======================================================================
        % ipctb_spm_conman('ImDesMtx',xX,h)
        ...
        if isfield(varargin{2},'nKX') & ~isempty(varargin{2}.nKX)
            ...
            hDesMtxIm = image(...
                (ipctb_spm_DesMtx('sca',varargin{2}.xKXs.X,varargin{2}.name)+1)*32);
        end
        ...
        set(hDesMtxIm,'ButtonDownFcn','ipctb_spm_DesRep(''SurfDesMtx_CB'')')


        %=======================================================================
    case 'imparest'
        %=======================================================================
        % ipctb_spm_conman('ImParEst',xX,h)
        ...
        %-----------------------------------------------------------------------
        axes(h)
        est  = ipctb_spm_SpUtil('IsCon',xX.xKXs);
        nPar = length(est);
        ...
        set(hParEstIm,'ButtonDownFcn','ipctb_spm_DesRep(''SurfEstIm_CB'')')


        %=======================================================================
    case 'listcon'
        %=======================================================================
        % ipctb_spm_conman('ListCon',hConList,xCon,STAT,I)
        ...
        ipctb_spm_conman('GraphCons',xCon,Q(q),get(hConList,'Parent'))
        ipctb_spm_conman('StatusLine',get(hConList,'Parent'))


        %=======================================================================
    case 'graphcons'
        %=======================================================================
        % ipctb_spm_conman('GraphCons',xCon,I,F)

        if nargin<2, error('insufficient arguments'), end
        xCon = varargin{2};
        if nargin<3, I=[1:length(xCon)]; else, I=varargin{3}; end
        if nargin<4, F=[]; else, F=varargin{4}; end
        if isempty(F), F=ipctb_spm_figure('FindWin','ConMan'); end
        ...
        if isempty(I)
            ...
            for ii = nCon:-1:1
                ...
                set(h,'ButtonDownFcn','ipctb_spm_DesRep(''SurfCon_CB'')',...
    ...
    case 'statusline'
        %=======================================================================
        % ipctb_spm_conman('StatusLine',F,str,col)
    ...
    case 'done_cb'
        ...
        if ~isempty(str)             %-error: display error dialog box
            ipctb_spm('Beep')
            msgbox(str,sprintf('%s%s: %s...',ipctb_spm('ver'),...
                ipctb_spm('GetUser',' (%s)'),mfilename),'error','modal')
    ...
    case 'conlist_cb'
        ...
        if strcmp(get(F,'SelectionType'),'open'), ipctb_spm_conman('Done_CB'), end


        %=======================================================================
    case {'tfa','d_tf'}
        %=======================================================================
        % STAT = ipctb_spm_conman('TFA')
        % STAT = ipctb_spm_conman('TFA',F,STAT)
        % STAT = ipctb_spm_conman('TFA',F,STAT,STATmode)
        ...
        if ~D                    %-Additional UI setup for main contrast manager
            ...
            if nargin<3
                SPM      = get(F,'UserData');
                xCon     = SPM.xCon;
                ipctb_spm_conman('ListCon',findobj(F,'Tag','ConList'),xCon,STAT)
            end
            ...
            ipctb_spm_conman('GraphCons',[],[],F)                     %-Clear contrast plot
            ipctb_spm_conman('D_Status',F)                            %-Set StatusLine
    ...
    case 'parsecon'                       %-Parse (cell)string into contrast
        %=======================================================================
        % [c,I,emsg,imsg,msg] = ipctb_spm_conman('ParseCon',cstr,X,STAT)
        ...
        for i=1:size(c,1)
            ...
            if ischar(c{i})
                ...
                elseif size(c{i},2)>p
                    ...
                    if ~ipctb_spm_SpUtil('allCon',X,c{i}')
    ...
    case 'parseistr'                     %-Parse index string
        %=======================================================================
        % [iX0,I,emsg,imsg] = ipctb_spm_conman('ParseIStr',str,max)
    ...
    case 'reset_cb'
        ...
        ipctb_spm_conman('ListCon',hConList,xCon,STAT,[])


        %=======================================================================
    case 'd_setup_cb'
        %=======================================================================
        % ipctb_spm_conman('D_Setup_CB')
        ...
        ipctb_spm_conman('D_TF',F,STAT,STATmode);			%-Setup rest of define UI


        %=======================================================================
    case {'d_conmtx_cb','d_x1cols_cb'}
        %=======================================================================
        % ipctb_spm_conman('D_ConMtx_CB')
        % ipctb_spm_conman('D_X1cols_CB')
        ...
        if fcn==1                              %-Parse string from ConMtx widget
            ...
            [c,I,emsg,imsg] = ipctb_spm_conman('ParseCon',str,xX.xKXs,STAT);
            if all(I)
                if isfield(xX, 'fullrank')
                    ...
                    sXc = ipctb_spm_sp('set', c); sXc.u = sparse(sXc.u);
                    ...
                    if strcmp(STAT, 'T')
                        DxCon.name = '';
                        DxCon.STAT = STAT;
                        DxCon.c = ipctb_spm_sp(':', xX.xKXs, c);
                        DxCon.X0.ukX0 = sparse(diag(xX.xKXs.ds(1:rk))*xX.xKXs.v(:,1:rk)'*r);
                        DxCon.iX0 = 'c';
                        DxCon.X1o.ukX1o = ipctb_spm_sp('cukxp-:',xX.xKXs,c);
                        DxCon.eidf = [];
                        DxCon.Vcon = [];
                        DxCon.Vspm = [];
                    else
                    ...
                    DxCon = ipctb_spm_FcUtil('Set','',STAT,'c',c,xX.xKXs);
                end
            else
                DxCon = [];
            end
        ...
        elseif fcn==2               %-Process column indicies from X1cols widget
            ...
            nPar              = ipctb_spm_SpUtil('size',xX.xKXs,2);
            [iX0,I,emsg,imsg] = ipctb_spm_conman('ParseIStr',str,nPar);

            if I
                try	%-try-catch block for any errors in ipctb_spm_FcUtil!
                    DxCon = ipctb_spm_FcUtil('Set','',STAT,'iX0',iX0,xX.xKXs);
        ...
        if all(I)
            ...
            ipctb_spm_conman('GraphCons',DxCon,-1,F)		%-Depict contrast
        else
    ...
    case 'd_reset_cb'
        %=======================================================================
        % ipctb_spm_conman('D_Reset_CB')

        STAT = get(findobj(gcbf,'Tag','TFA','Value',1),'UserData');
        set(findobj(gcbf,'Tag','D_name'),'String','')		%-Clear name
        set(findobj(gcbf,'Tag','D_ConMtx'),'UserData',[])	%-Contrast definition
        ipctb_spm_conman('D_TF',gcbf,STAT);				%-Setup rest of define UI


        %=======================================================================
    case 'd_cancel_cb'
        %=======================================================================
        % ipctb_spm_conman('D_Cancel_CB')
        ...
        ipctb_spm_conman('StatusLine')


        %=======================================================================
    case 'd_ok_cb'
        %=======================================================================
        % ipctb_spm_conman('D_OK_CB')
        ...
        ipctb_spm_conman('TFA',F,xCon(end).STAT);			%-Set STAT
        ipctb_spm_conman('ListCon',hConList,xCon,xCon(end).STAT,I)	%-ListCon
    ...
    case 'createfig'
        ...
        %-----------------------------------------------------------------------
        WS = ipctb_spm('WinScale');				%-Window scaling factors
        FS = ipctb_spm('FontSizes');				%-Scaled font sizes
        PF = ipctb_spm_platform('fonts');			%-Font names (for this platform)
        if ipctb_spm_matlab_version_chk('7') >= 0, 	%-Screen size
        ...
        set([hT,hF,hA],'CallBack','ipctb_spm_conman(''TFA'');',...
        ...
        hConList = uicontrol(F,'Style','ListBox','Tag','ConList',...
            ...
            'Max',2,...
            'CallBack','ipctb_spm_conman(''ConList_CB'')',...
            'Interruptible','off','BusyAction','Queue',...
            'BackgroundColor','w',...
        ...
        hNew = uicontrol(F,'Style','Pushbutton','String','Define new contrast...',...
            ...
            'ForegroundColor','b',...
            'Callback','ipctb_spm_conman(''D_Setup_CB'')',...
            'Enable','on',...
        ...
        uicontrol(F,'Style','Pushbutton','String','Reset',...
            ...
            'ForegroundColor','r',...
            'Callback','ipctb_spm_conman(''Reset_CB'')',...
        ...
        uicontrol(F,'Style','Pushbutton','String','Done',...
            ...
            'ForegroundColor','m',...
            'Tag','Done','UserData',1,...
            'Callback','ipctb_spm_conman(''Done_CB'')',...
        ...
        uimenu(h,'Label','Rename selected contrast...',...
            'Tag','CM_Ren',...
            'CallBack','ipctb_spm_conman(''Rename_CB'')',...
            'Interruptible','off','BusyAction','Cancel');
        ...
        uimenu(h,'Label','help','Separator','on',...
            'CallBack','ipctb_spm_help(''ipctb_spm_conman'')',...
            'Interruptible','off','BusyAction','Cancel');
        uimenu(h,'Label','crash out','Separator','on',...
            'CallBack','ipctb_spm_conman(''Initialise'',''reset'');',...
            'Interruptible','off','BusyAction','Cancel');
        set(h,'CallBack','ipctb_spm_conman(''FConMenu_CB'')',...
            'Interruptible','off','BusyAction','Cancel');
        ...
        h = uicontrol(F,'Style','Edit','Tag','D_name',...
            ...
            'Callback','ipctb_spm_conman(''D_Status'')','Interruptible','off',...
        ...
        set([hDT,hDF],'CallBack','ipctb_spm_conman(''D_TF'');',...
            'Interruptible','off','BusyAction','Queue')
        H = [H,hDT,hDF];
        ...
        h = uicontrol(F,'Style','Edit','Tag','D_ConMtx',...
            ...
            'BackgroundColor',COLOUR,...
            'Max',2,...
            'CallBack','ipctb_spm_conman(''D_ConMtx_CB'')',...
            'Interruptible','off','BusyAction','Queue',...
            'UserData',[],...
        ...
        h = uicontrol(F,'Style','Edit','Tag','D_X1cols',...
            ...
            'HorizontalAlignment','Left',...
            'BackgroundColor',COLOUR,...
            'CallBack','ipctb_spm_conman(''D_X1cols_CB'')',...
            'Interruptible','off','BusyAction','Queue',...
        ...
        h = uicontrol(F,'Style','Pushbutton','String','Reset',...
            ...
            'ForegroundColor','b',...
            'Callback','ipctb_spm_conman(''D_Reset_CB'')',...
            'Interruptible','off','BusyAction','Cancel',...
        ...
        h = uicontrol(F,'Style','Pushbutton','String','Cancel',...
            ...
            'ForegroundColor','r',...
            'Callback','ipctb_spm_conman(''D_Cancel_CB'')',...
            'Interruptible','off','BusyAction','Cancel',...
        ...
        h = uicontrol(F,'Style','Pushbutton','String','OK',...
            ...
            'ForegroundColor','m',...
            'Callback','ipctb_spm_conman(''D_OK_CB'')',...
            'Interruptible','off','BusyAction','Cancel',...
        ...
        %-------------------------------------------------------------
        if strcmpi('EEG', ipctb_spm('checkmodality'))
