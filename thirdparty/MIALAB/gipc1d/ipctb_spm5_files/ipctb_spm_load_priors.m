function b0 = ipctb_spm_load_priors(B)
...
% deg = 3;
lm  = 0;
if ~isstruct(B), B  = ipctb_spm_vol(B); end;
Kb = length(B);
b0 = cell(Kb,1);
for k1=1:(Kb),
...
ipctb_spm_progress_bar('Init',B(1).dim(3),'Loading priors','Planes loaded');
for i=1:B(1).dim(3)
    M         = ipctb_spm_matrix([0 0 i]);
    s         = zeros(B(1).dim(1:2));
    for k1=1:Kb
        tmp           = ipctb_spm_slice_vol(B(k1),M,B(1).dim(1:2),0)*(1-lm*2)+lm;
...
%for k1=1:Kb+1,
%    b0{k1} = ipctb_spm_bsplinc(log(b0{k1}),[deg deg deg  0 0 0]);
%end;
