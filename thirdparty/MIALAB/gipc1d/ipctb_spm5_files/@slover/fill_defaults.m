...
if ~isempty(obj.figure)
  ...
  obj.figure = ipctb_spm_figure('FindWin', 'Graphics'); 
  if isempty(obj.figure)
    obj.figure = gcf;
  end
end
...
if ~dead_f
  if strcmp(get(obj.figure, 'Tag'),'Graphics')
    ...
    obj.area = ipctb_spm_mars_struct('fillafromb', obj.area, def_fig);
  end
end
...
if ischar(obj.transform)
  ...
  obj.transform = ipctb_spm_matrix(ts(orientn,:));
end
...
if ischar(obj.labels)
  ...
  obj.labels = ipctb_spm_mars_struct('fillafromb', obj.labels, def_labs);
end

% figure area stuff
obj.area = ipctb_spm_mars_struct('fillafromb', obj.area, def_area);
...
for i = 1:length(obj.img)
  img = obj.img(i);
  if ~ipctb_spm_mars_struct('isthere', img, 'type')
    % default is true colour, unless prop is Inf
    img.type = 'truecolour';
    if ipctb_spm_mars_struct('isthere', img, 'prop')
      if img.prop == Inf
	img.type = 'split';
	img.prop = 1;
      end
  ...
  if ~ipctb_spm_mars_struct('isthere', img, 'hold')
    if ~ipctb_spm_mars_struct('isthere', img.vol, 'imgdata')
  ...
  if ~ipctb_spm_mars_struct('isthere', img, 'background')
    img.background = NaN;
  end
  if ~ipctb_spm_mars_struct('isthere', img, 'prop')
  ...
  if ~ipctb_spm_mars_struct('isthere', img, 'range')
    [mx mn] = pr_volmaxmin(img.vol);
    img.range = [mn mx];
  end
  if ~ipctb_spm_mars_struct('isthere', img, 'cmap')
  ...
  if ~ipctb_spm_mars_struct('isthere', img, 'outofrange')
  ...
  if ~ipctb_spm_mars_struct('isthere', img, 'nancol')
    img.nancol = 0;
  end
  imgs(i) = img;
end
