...
params = ipctb_spm_mars_struct('fillafromb', params, ...
...
for i = 1:nslices
  ...
  for j=1:length(contimgs)
    ...
    if any(any(isfinite(i1)))
      ...
      if ipctb_spm_mars_struct('isthere', thisimg, 'linespec')
	...
	linespec = 'w-';
      ...
      if ipctb_spm_mars_struct('isthere', thisimg, 'contours')
	...
	if ipctb_spm_mars_struct('isthere', thisimg, 'linewidth')
...
obj.figure_struct = ipctb_spm_mars_struct('split', get(figno), fig_struct_fields);
...
i1 = ipctb_spm_sample_vol(V,vixyz(1,:),vixyz(2,:),vixyz(3,:), ...
		    [img.hold img.background]);
if ipctb_spm_mars_struct('isthere', img, 'func')
  eval(img.func);
end
