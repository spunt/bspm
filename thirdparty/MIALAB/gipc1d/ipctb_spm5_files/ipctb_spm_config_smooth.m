function opts = ipctb_spm_config_smooth
% Configuration file for smoothing jobs
%_______________________________________________________________________
% Copyright (C) 2005 Wellcome Department of Imaging Neuroscience

% John Ashburner
% $Id: ipctb_spm_config_smooth.m 471 2006-03-08 17:46:45Z john $
...
dtype.values = {0,ipctb_spm_type('uint8'),ipctb_spm_type('int16'),ipctb_spm_type('int32'),...
                ipctb_spm_type('float32'),ipctb_spm_type('float64')};
dtype.val = {0};
...
ipctb_spm_progress_bar('Init',n,'Smoothing','Volumes Complete');
for i = 1:n
        Q = deblank(P(i,:));
        [pth,nam,xt,nm] = ipctb_spm_fileparts(deblank(Q));
        U = fullfile(pth,['s' nam xt nm]);
        ipctb_spm_smooth(Q,U,s,dtype);
