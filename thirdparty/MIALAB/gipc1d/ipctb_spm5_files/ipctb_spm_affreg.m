function [M,scal] = ipctb_spm_affreg(VG,VF,flags,M,scal)
...
if flags.globnorm,
	...
	% ---------------------------------------------------------------
	for i=1:length(VG),
		VG(i).pinfo(1:2,:) = VG(i).pinfo(1:2,:)/ipctb_spm_global(VG(i));
	end;
	VF(1).pinfo(1:2,:) = VF(1).pinfo(1:2,:)/ipctb_spm_global(VF(1));
end;
% ---------------------------------------------------------------

if length(VG)==1,
	[G,dG1,dG2,dG3]  = ipctb_spm_sample_vol(VG(1),x1,x2,x3,1);
	if ~isempty(flags.WG),
		WG = abs(ipctb_spm_sample_vol(flags.WG,x1,x2,x3,1))+eps;
		WG(~isfinite(WG)) = 1;
	end;
end;

[F,dF1,dF2,dF3]  = ipctb_spm_sample_vol(VF(1),y1,y2,y3,1);
if ~isempty(flags.WF),
	WF = abs(ipctb_spm_sample_vol(flags.WF,y1,y2,y3,1))+eps;
	WF(~isfinite(WF)) = 1;
end;
% ---------------------------------------------------------------
n_main_its = 0;
...
for iter=1:256,
	...
	if length(VG)==1,
		...
		dt  = 0.0001;
		R   = eye(13);
		MM0 = inv(VG.mat)*inv(ipctb_spm_matrix(p0))*VG.mat;
		for i1=1:12,
			p1          = p0;
			p1(i1)      = p1(i1)+dt;
			MM1         = (inv(VG.mat)*inv(ipctb_spm_matrix(p1))*(VG.mat));
		...
		t2         = t2(msk);
		t3         = t3(msk);
		t          = ipctb_spm_sample_vol(VF(1), t1,t2,t3,1);
		...
		if ~isempty(flags.WF) || ~isempty(flags.WG),
			if isempty(flags.WF),
				...
				wt = ipctb_spm_sample_vol(flags.WF(1), t1,t2,t3,1)+eps;
				wt(~isfinite(wt)) = 1;
	...
	if 1,
		...
		dt = 0.0001;
		R  = eye(12+length(VG));
		MM0 = inv(M*VF.mat)*ipctb_spm_matrix(p0)*M*VF.mat;
		for i1=1:12,
			p1          = p0;
			p1(i1)      = p1(i1)+dt;
			MM1         = (inv(M*VF.mat)*ipctb_spm_matrix(p1)*M*VF.mat);
			R(1:12,i1)  = reshape((MM1(1:3,:)-MM0(1:3,:))/dt,12,1);
		end;
		% ---------------------------------------------------------------
		...
		if ~isempty(flags.WF) || ~isempty(flags.WG),
			if isempty(flags.WG),
				...
				wt = ipctb_spm_sample_vol(flags.WG(1), t1,t2,t3,1)+eps;
				wt(~isfinite(wt)) = 1;
		...
		if est_smo,
			...
			for i=1:length(VG),
				[t(:,i),dt1,dt2,dt3] = ipctb_spm_sample_vol(VG(i), t1,t2,t3,1);
			...
			for i=1:length(VG),
				t(:,i)= ipctb_spm_sample_vol(VG(i), t1,t2,t3,1);
			end;
		end;
	...
	M       = ipctb_spm_matrix(p0 + soln(1:12)')*M;
	...
	% ---------------------------------------------------------------
	ss = ss/n;
	if iter>1, ipctb_spm_chi2_plot('Set',ss); end;
	if (pss-ss)/pss < 1e-6,
		est_smo = 1;
	end;
...
figure(2);
Mt = ipctb_spm_matrix([0 0 (VG(1).dim(3)+1)/2]);
M  = (M*VF(1).mat)\VG(1).mat;
t  = zeros(VG(1).dim(1:2));
for i=1:length(VG);
	t  = t + ipctb_spm_slice_vol(VG(i),  Mt,VG(1).dim(1:2),1)*scal(i);
end;
u  = ipctb_spm_slice_vol(VF(1),M*Mt,VG(1).dim(1:2),1);
