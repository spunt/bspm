function [M,h] = ipctb_spm_maff(varargin)
...
% John Ashburner
% $Id: spm_maff.m 570 2006-07-05 14:29:24Z john $
...
d         = [size(x1) length(x3)];
g         = zeros(d);
ipctb_spm_progress_bar('Init',V.dim(3),'Loading volume','Planes loaded');
for i=1:d(3)
    g(:,:,i) = ipctb_spm_sample_vol(V,x1,x2,o*x3(i),0);
    ipctb_spm_progress_bar('Set',i);
end;
ipctb_spm_progress_bar('Clear');

% Convert the image to unsigned bytes
[mn,mx] = ipctb_spm_minmax(g);
for z=1:length(x3),
    gz         = g(:,:,z);
    buf(z).msk = gz>mn;
...
ipctb_spm_chi2_plot('Init','Registering','Log-likelihood','Iteration');
...
for iter=1:200
    ...
    for i=1:length(x3),
        ...
        for k=1:size(h0,2),
            pr(k).b = ipctb_spm_sample_priors(b0{k},y1,y2,y3,k==length(b0));
            h0(:,k) = h0(:,k) + ipctb_spm_hist(buf(i).g,pr(k).b);
        end;
    end;
    h1    = (h0+eps);
    ssh   = sum(h1(:));
    krn   = ipctb_spm_smoothkern(2,(-256:256)',0);
    h1    = conv2(h1,krn,'same');
    h1    = h1/ssh;
    h2    = log2(h1./(sum(h1,2)*sum(h1,1)));
    ll1   = sum(sum(h0.*h2))/ssh - penalty/ssh;
    ipctb_spm_chi2_plot('Set',ll1);
    ...
    for i=1:length(x3),
        ...
        dmi3  = zeros(nz,1);
        for k=1:size(h0,2),
            [pr(k).b, pr(k).db1, pr(k).db2, pr(k).db3] = ipctb_spm_sample_priors(b0{k},y1,y2,y3,k==length(b0));
            tmp  = -h2(gi,k);
        ...
        A = [dmi1.*x1m dmi2.*x1m dmi3.*x1m...
             ...
             dmi1      dmi2      dmi3];
        Alpha = Alpha + ipctb_spm_atranspa(A);
        Beta  = Beta  + sum(A,1)';
    end;
    drawnow;
    Alpha = R'*Alpha*R;
...
%sn = ipctb_spm_select(Inf,'.*seg_inv_sn.mat$');
