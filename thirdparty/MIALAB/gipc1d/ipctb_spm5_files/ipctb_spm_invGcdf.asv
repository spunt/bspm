function x = ipctb_spm_invGcdf(F,h,l,tol)
...
while length(QQ)
	b(QQ) = 2*b(QQ);
	QQ    = find(ipctb_spm_Gcdf(b,hQ,lQ) < FQ);
end

%-Interval bisection
%-----------------------------------------------------------------------
fa = ipctb_spm_Gcdf(a,hQ,lQ)-FQ;
fb = ipctb_spm_Gcdf(b,hQ,lQ)-FQ;
i  = 0;
xQ = a+(b-a)/2;
QQ = 1:length(Q);

while length(QQ) &  i<maxIt
	fxQQ        = ipctb_spm_Gcdf(xQ(QQ),hQ(QQ),lQ(QQ))-FQ(QQ);
	mQQ         = fa(QQ).*fxQQ > 0;
