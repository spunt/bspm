function varargout=ipctb_spm_help(varargin)
% SPM help and manual facilities
% FORMAT ipctb_spm_help
...
% ipctb_spm_help toolbar.
%
%-----------------------------------------------------------------------
%
% ipctb_spm_help sets up a GUI help system for the SPM package.
% `ipctb_spm_help('Topic')` or `ipctb_spm_help Topic` displays the help for a
...
%       pages.  These are contained in ASCII files named ipctb_spm_*.man.
%       These can be viewed on the MatLab command line with the `help`
%       command, e.g. `help ipctb_spm_help` prints out this manual file in
...
% $Id: spm_help.m 539 2006-05-19 17:59:30Z Darren $
...
if nargin==0 || isempty(varargin{1})
	Fhelp = ipctb_spm_figure('FindWin','Help');
	if ~isempty(Fhelp), set(Fhelp,'Visible','on')
    else ipctb_spm_help('!Topic','MenuWin'), end
	return
elseif varargin{1}(1)~='!'
	ipctb_spm_help('!Topic',varargin{:}), return
end
...
case '!drawmenuwin'
%=======================================================================
% Fhelp = ipctb_spm_help('!DrawMenuWin');
...
[SPMver, SPMc] = ipctb_spm('Ver');			%-SPM version & (c) info
...
uicontrol(Fhelp,'Style','PushButton','String',['About ',SPMver],...
	...
	'CallBack','ipctb_spm_help(''spm.man'')',...
...
modality = ipctb_spm('CheckModality');
for i = length(C):-1:1
    c = get(C(i));
    ...
    if isempty(c.Tag) || strcmp(c.Tag, 'Modality') || ~isempty(strfind(c.Tag, modality))
        if strcmp(c.Type,'uicontrol')
            ...
            if ischar(c.UserData)
                Callback = sprintf('ipctb_spm_help(''%s'')',c.UserData);
            else
                Callback = '';
            end
...
uicontrol(Fhelp,'String','Data format','ForegroundColor','b',...
	...
	'CallBack','ipctb_spm_help(''spm_format.man'')',...
...
uicontrol(Fhelp,'String','Graphics','ForegroundColor','b',...
	...
	'CallBack','ipctb_spm_help(''spm_figure'')',...
	'Tag','HelpMenu','Interruptible','on')
...
uicontrol(Fhelp,'String','Random FX','ForegroundColor','b',...
	...
	'CallBack','ipctb_spm_help(''spm_RandFX.man'')',...
	'Tag','HelpMenu','Interruptible','on')
uicontrol(Fhelp,'String','','ForegroundColor','b',...
	'ToolTipString','',...
...
uicontrol(Fhelp,'String','SPM motd','ForegroundColor','b',...
	...
	'CallBack','ipctb_spm_help(''spm_motd.man'')',...
	'Tag','HelpMenu','Interruptible','on')
...
uicontrol(Fhelp,'String',SPMc,...
	...
	'CallBack','ipctb_spm_help(''spm_LICENCE.man'')',...
	'Tag','HelpMenu','Interruptible','on')
...
elseif strcmp(Topic,'SPM_Modality')
	S = sprintf([...
		...
		'%%\n',...
		'%% See also: ipctb_spm.m ipctb_spm_defaults.m\n',...
		'%s\n%%\n%% Andrew Holmes\n'],Usep,Usep);
elseif strcmp(Topic,'MenuWin_Toolboxes_PullDown')
	S = sprintf([...
		...
		'%% SPM toolboxes are defined as the contents of sub-',...
			'directories of\n',...
		'%% fullfile(ipctb_spm(''Dir''),''toolbox'') - the ''toolbox'' ',...
		...
		'%% A toolbox called "test" would be in the "test" ',...
			'subdirectory of\n',...
		'%% ipctb_spm(''Dir''), with a single file named *test.m.) This ',...
			'M-file is\n',...
		...
		'%% If there are no toolboxes installed, then the button ',...
			'is disabled.\n',...
		'%%\n',...
		'%% See ipctb_spm.m for programmers help with toolbox handling.\n',...
		'%s\n%%\n%% Andrew Holmes\n'],...
		Usep,fullfile(ipctb_spm('Dir'),'toolbox'),Usep);
elseif strcmp(Topic,'MenuWin_Utils_PullDown')
	S = sprintf([...
		'\n%% SPM Menu window: PullDown menu of utilities...\n',...
		'%s\n%%\n',...
		'%% 1) CD:        Calls ipctb_spm_get prompting for a directory,\n',...
		...
		'%% 4) Show SPM:  Pops up all the figures for this SPM',...
			' session.\n',...
		'%%               Typing "ipctb_spm show" in the MatLab command',...
			' window has the\n',...
		'%%               same effect.\n',...
...
elseif strcmp(Topic,'QuitSPM')
	...
	doc = ipctb_spm_jobman('help',Topic);
    if ~isempty(doc),
		doc = char(doc{:});
...
if fid<0				%-No file - try 'ShortTopics'
	[S,Err] = ipctb_spm_help('!ShortTopics',Topic);
...
%-----------------------------------------------------------------------
ipctb_spm_help('!Disp',Topic,S,Fhelp);

%-Display MenuWin graphic if required
%-----------------------------------------------------------------------
if bMenuWin
	ipctb_spm_help('!DrawMenuWin')
end
...
Fhelp = ipctb_spm_figure('FindWin',F);
if isempty(Fhelp)
	Fhelp = ipctb_spm_help('!CreateHelpWin');
else
	set(Fhelp,'Visible','on');
end
...
if isempty(S)
	...
	fid = fopen(Fname,'rt');
	if fid<0
		[S,Err] = ipctb_spm_help('!ShortTopics',Fname);
	else
		S = fread(fid,'*char')';
		% Err = 0;
		fclose(fid);
...
for i = 1:(length(q) - 1)
	...
	if y<0
		...
		ipctb_spm_figure('NewPage',[hAxes(end);get(hAxes(end),'Children')])
...
case '!create'
%=======================================================================
% F = ipctb_spm_help('!Create')
%-Condition arguments

F = ipctb_spm_help('!CreateHelpWin');
ipctb_spm_help('!CreateBar',F)
ipctb_spm_figure('WaterMark',F,ipctb_spm('Ver'),'WaterMark',-45,1)
varargout = {F};
...
F = ipctb_spm_figure('FindWin','Help');
if any(F), return, end
...
F      = figure('IntegerHandle','off',...
	...
	'Resize','off',...
	'Name',sprintf('%s%s: SPMhelp',ipctb_spm('ver'),ipctb_spm('GetUser',' (%s)')),...
	'NumberTitle','off',...
	'Tag','Help',...
	'MenuBar','none',...
	'Units','Pixels',...
...
%-----------------------------------------------------------------------
ipctb_spm_figure('Clear',F,'HelpBar')
...
uicontrol(F,'String','Print' ,'Position',[x y sx sy],...
	...
	'FontSize',FS,...
	'CallBack','ipctb_spm_figure(''Print'',gcf)',...
...
uicontrol(F,'String','Clear' ,'Position',[x y sx sy],...
	...
	'FontSize',FS,...
	'CallBack','ipctb_spm_help(''!Clear'',gcf)',...
	'Interruptible','off','BusyAction','cancel',...
        'Tag','HelpBar','HandleVisibility','callback',...
        'ForegroundColor','b'); x = x+sx+dx;

uicontrol(F,'String',sprintf('About %s',ipctb_spm('Ver')),...
...
hTopic = uicontrol(F,'Style','Edit','String','',...
	...
	'CallBack','ipctb_spm_help(get(gco,''String''))',...
	'Tag','HelpBar','HandleVisibility','callback',...
...
uicontrol(F,'String','Back',   'Position',[x y sx sy],...
	...
	'FontSize',FS,...
	'CallBack','ipctb_spm_help(''!Topic'',-1)',...
	'Interruptible','off','BusyAction','cancel',...
	'Tag','HelpBar','HandleVisibility','callback')
...
hPrevTopics = uicontrol(F,'Style','PopUp',...
	...
	'CallBack',...
	'if get(gcbo,''Value'')~=1,ipctb_spm_help(get(gcbo,''Value'')),end',...
	'Tag','HelpBar','HandleVisibility','callback',...
	'HorizontalAlignment','Left',...
...
uicontrol(F,'String','Menu',   'Position',[x y sx sy],...
	...
	'FontSize',FS,...
	'CallBack','ipctb_spm_help(''MenuWin'')',...
	'Interruptible','off','BusyAction','cancel',...
	'Tag','HelpBar','HandleVisibility','callback',...
...
uicontrol(F,'String','Help',   'Position',[x y sx sy],...
	...
	'FontSize',FS,...
	'CallBack','ipctb_spm_help(''ipctb_spm_help.m'')',...
	'Interruptible','off','BusyAction','cancel',...
	'Tag','HelpBar','HandleVisibility','callback',...
...
uicontrol(F,'String','SPMweb', 'Position',[x y sx+dx/2 sy],...
	...
	'FontSize',FS,...
	'FontWeight','Bold','FontName',ipctb_spm_platform('font','courier'),...
	'CallBack',['set(gcbf,''Pointer'',''Watch''),',...
			'web(''http://www.fil.ion.ucl.ac.uk/ipctb_spm'');',...
...
ipctb_spm_help('!Topic',Topics{Topic})



case '!figkeypressfcn'
%=======================================================================
% ipctb_spm_help('!FigKeyPressFcn',h,ch)
...
if ~strcmp(get(F,'Tag'),'Help'), ipctb_spm_figure('Clear',F), return, end
HD = get(F,'UserData');
...
case '!contexthelp'
%=======================================================================
% h = ipctb_spm_help('!ContextHelp',Topic)
...
elseif ~isempty(Finter)
	...
	h = text(0.5,0.5,'?',...
		'Parent',hAx,...
		'Color','g',...
		'FontName',ipctb_spm_platform('font','helvetica'),...
		'FontSize',ipctb_spm('FontSize',14),...
		'FontWeight','bold','FontAngle','italic',...
		'HorizontalAlignment','Center',...
		'VerticalAlignment','middle',...
		'ButtonDownFcn',['ipctb_spm_help(''',Topic,''')']);
