function ipctb_spm_normalise_disp(matname,VF)
...
if nargin<1, matname = ipctb_spm_select(1,'.*_sn.mat$','Select parameter file'); end;
...
text(0,0.75, [ 'Image     : ' VF.fname],'FontSize',12,'FontWeight','Bold',...
	'Interpreter','none','Parent',ax);
%text(0,0.7, [ 'Parameters : ' ipctb_spm_str_manip(matname,'sd')],'FontSize',12,...
%	'Interpreter','none','Parent',ax);
...
ipctb_spm_orthviews('Image',t.VG(1).fname,[0.01 0.1 .48 .6]);
VN = ipctb_spm_write_sn(VF,matname);
h2 = ipctb_spm_orthviews('Image',VN,[.51 0.1 .48 .6]);
ipctb_spm_orthviews('Space',h2);
ipctb_spm_print;
drawnow;
return;
