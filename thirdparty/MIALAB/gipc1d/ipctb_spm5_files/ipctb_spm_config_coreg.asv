function opts = ipctb_spm_config_coreg
...
'It can be useful for reorienting images while preserving the original ',...
'intensities (e.g. an image consisting of labels). ',...
'Bilinear Interpolation is OK for PET, or realigned and resliced fMRI. ',...
...
x  = ipctb_spm_coreg(strvcat(job.ref), strvcat(job.source),job.eoptions);
M  = inv(ipctb_spm_matrix(x));
PO = strvcat(strvcat(job.source),strvcat(job.other));
MM = zeros(4,4,size(PO,1));
for j=1:size(PO,1),
	MM(:,:,j) = ipctb_spm_get_space(deblank(PO(j,:)));
end;
for j=1:size(PO,1),
	ipctb_spm_get_space(deblank(PO(j,:)), M*MM(:,:,j));
end;
...
x  = ipctb_spm_coreg(job.ref, job.source,job.eoptions);
M  = inv(ipctb_spm_matrix(x));
...
for i=1:numel(job.source),
    [pth,nam,ext,num] = ipctb_spm_fileparts(job.source{i});
...
vf  = cell(size(P));
for i=1:numel(P),
    [pth,nam,ext,num] = ipctb_spm_fileparts(P{i});
    vf{i} = fullfile(pth,['r', nam, ext, num]);
end;
