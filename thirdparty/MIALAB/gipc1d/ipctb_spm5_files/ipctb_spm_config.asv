function vals = ipctb_spm_config
...
% paragraph, and justified accordingly. See ipctb_spm_justify.
%
% 'values'
...
% structure (see ipctb_spm_defaults.m). Undefined 'val's are assigned
...
%
% 'filter'
% A filter for file selection.  See ipctb_spm_select for filter usage. Note
% that a filter of 'image' will allow .*\.nii and .*\.img files to be
% selected, along with their volume number. This sets the 'typ' input
% argument of ipctb_spm_select, which is not user editable.
%
% 'ufilter'
...
% John Ashburner
% $Id: spm_config.m 639 2006-09-28 17:09:17Z volkmar $

ob = struct('type','branch','tag','unused','name','Toolboxes','val',{{}});

tempo   = struct('type','repeat','name','Temporal','tag','temporal',...
          'values',{{ob,ipctb_spm_config_slice_timing}},...
          'help',{'Temporal pre-processing functions.'});

spat    = struct('type','repeat','name','Spatial','tag','spatial',...
          'values',{{ipctb_spm_config_realign,ipctb_spm_config_realign_and_unwarp,ipctb_spm_config_coreg,...
                     ipctb_spm_config_preproc,ipctb_spm_config_norm,ipctb_spm_config_smooth}},...
...
tbxdir = fullfile(ipctb_spm('Dir'),'toolbox');
