function [status, fieldsUsed] = ipctb_spm_matlab_version_chk(chk,tbx)
% ipctb_spm_matlab_version_chk Check a version number against a Toolbox version.
...
%       status = ipctb_spm_matlab_version_chk('7');
...
%       status = ipctb_spm_matlab_version_chk('7.2');
...
%       status = ipctb_spm_matlab_version_chk('6.5.0');
%       returns status == 1     : major + minor + release revision is later
%                                 for matlab
%       The statement ( ipctb_spm_matlab_version_chk('6.5.0') > 0 ) is true for
...
%
% Darren Gitelman
% $Id: ipctb_spm_matlab_version_chk.m 539 2006-05-19 17:59:30Z Darren $
