function ret = ipctb_spm_ov_quiver3d(varargin)
% function ret = ipctb_spm_ov_quiver3d(varargin)
%
% NAME   quiver3d    (Plugin to ipctb_spm_orthviews)
%
% FORMAT ret = ipctb_spm_orthviews('quiver3d', command, volhandle[, varargin])
%
% COMMANDS
% ipctb_spm_orthviews('quiver3d', 'init', volhandle, Ve1fnames, Ve2fnames,
...
%
% ipctb_spm_orthviews('quiver3d', 'redraw', volhandle)
%
% ipctb_spm_orthviews('quiver3d, 'delete', volhandle)
%_____________________________________________________________________________
% %W% Volkmar Glauche <glauche@uke.uni-hamburg.de> %E%

global st;
if isempty(st)
  error('ipctb_spm_ov_quiver3d: This routine can only be called as a plugin for ipctb_spm_orthviews!');
end;

if nargin < 2
  error('ipctb_spm_orthviews(''quiver3d'',cmd,volhandle): Not enough arguments');
end;
...
switch cmd
  case 'init'
    ...
    if nargin < 7
      error('ipctb_spm_orthviews(''quiver3d'', ''init'',...): Not enough arguments');
    end;

    Ve1 = ipctb_spm_vol(varargin{3});
    Ve2 = ipctb_spm_vol(varargin{4});
    Ve3 = ipctb_spm_vol(varargin{5});
    Vla = ipctb_spm_vol(varargin{6});
    Vmask = ipctb_spm_vol(varargin{7});
    if all([length(Ve1) length(Ve2) length(Ve3) length(Vla)] == 3)
      ...
      error('ipctb_spm_orthviews(''quiver3d'', ''init'',...): Please specify 3 images for each eigenvector!');
    end;
    if nargin > 7
  ...
  case 'redraw'
	...
	delete(q3d.qhs);
      ...
      prm = ipctb_spm_imatrix(st.Space);
      ...
      la1	 = ipctb_spm_slice_vol(q3d.la(1),inv(TM0*Mla1),TD,0);
      la1(la1==0) = NaN;
      la2	 = ipctb_spm_slice_vol(q3d.la(2),inv(TM0*Mla2),TD,0)./la1; % scale to la1
      la3	 = ipctb_spm_slice_vol(q3d.la(3),inv(TM0*Mla3),TD,0)./la1; % scale to la1
      e1   = cat(3, ipctb_spm_slice_vol(q3d.e1(1),inv(TM0*Me1x),TD,0), ...
	  ipctb_spm_slice_vol(q3d.e1(2),inv(TM0*Me1y),TD,0), ...
	  ipctb_spm_slice_vol(q3d.e1(3),inv(TM0*Me1z),TD,0));
      e2   = cat(3, la2.*ipctb_spm_slice_vol(q3d.e2(1),inv(TM0*Me2x),TD,0), ...
	  la2.*ipctb_spm_slice_vol(q3d.e2(2),inv(TM0*Me2y),TD,0), ...
	  la2.*ipctb_spm_slice_vol(q3d.e2(3),inv(TM0*Me2z),TD,0));
      e3   = cat(3, la3.*ipctb_spm_slice_vol(q3d.e3(1),inv(TM0*Me3x),TD,0), ...
	  la3.*ipctb_spm_slice_vol(q3d.e3(2),inv(TM0*Me3y),TD,0), ...
	  la3.*ipctb_spm_slice_vol(q3d.e3(3),inv(TM0*Me3z),TD,0));
      mask = ipctb_spm_slice_vol(q3d.mask,inv(TM0*Mm),TD,0);
	...
	repmat([4*nmask+[1:nmask] 5*nmask+[1:nmask]],1,4)]';
      ...
      la1	 = ipctb_spm_slice_vol(q3d.la(1),inv(CM0*Mla1),CD,0);
      la1(la1==0) = NaN;
      la2	 = ipctb_spm_slice_vol(q3d.la(2),inv(CM0*Mla2),CD,0)./la1; % scale to la1
      la3	 = ipctb_spm_slice_vol(q3d.la(3),inv(CM0*Mla3),CD,0)./la1; % scale to la1
      e1   = cat(3, ipctb_spm_slice_vol(q3d.e1(1),inv(CM0*Me1x),CD,0), ...
	  ipctb_spm_slice_vol(q3d.e1(2),inv(CM0*Me1y),CD,0), ...
	  ipctb_spm_slice_vol(q3d.e1(3),inv(CM0*Me1z),CD,0));
      e2   = cat(3, la2.*ipctb_spm_slice_vol(q3d.e2(1),inv(CM0*Me2x),CD,0), ...
	  la2.*ipctb_spm_slice_vol(q3d.e2(2),inv(CM0*Me2y),CD,0), ...
	  la2.*ipctb_spm_slice_vol(q3d.e2(3),inv(CM0*Me2z),CD,0));
      e3   = cat(3, la3.*ipctb_spm_slice_vol(q3d.e3(1),inv(CM0*Me3x),CD,0), ...
	  la3.*ipctb_spm_slice_vol(q3d.e3(2),inv(CM0*Me3y),CD,0), ...
	  la3.*ipctb_spm_slice_vol(q3d.e3(3),inv(CM0*Me3z),CD,0));
      mask = ipctb_spm_slice_vol(q3d.mask,inv(CM0*Mm),CD,0);
      mask = ((mask > q3d.thresh(1)) & (mask < q3d.thresh(2)))|isnan(la1);
      e1   = e1(qst1:qst:end,qst1:qst:end,:);
      e2   = e2(qst1:qst:end,qst1:qst:end,:);
      e3   = e3(qst1:qst:end,qst1:qst:end,:);
	...
	repmat([4*nmask+[1:nmask] 5*nmask+[1:nmask]],1,4)]';
      ...
      % sagittal: Fs, Vs, FVCs
      % laX, eX, x, y, xyz will be reused for all 3 views
      la1	 = ipctb_spm_slice_vol(q3d.la(1),inv(SM0*Mla1),SD,0);
      la1(la1==0) = NaN;
      la2	 = ipctb_spm_slice_vol(q3d.la(2),inv(SM0*Mla2),SD,0)./la1; % scale to la1
      la3	 = ipctb_spm_slice_vol(q3d.la(3),inv(SM0*Mla3),SD,0)./la1; % scale to la1
      e1   = cat(3, ipctb_spm_slice_vol(q3d.e1(1),inv(SM0*Me1x),SD,0), ...
	  ipctb_spm_slice_vol(q3d.e1(2),inv(SM0*Me1y),SD,0), ...
	  ipctb_spm_slice_vol(q3d.e1(3),inv(SM0*Me1z),SD,0));
      e2   = cat(3, la2.*ipctb_spm_slice_vol(q3d.e2(1),inv(SM0*Me2x),SD,0), ...
	  la2.*ipctb_spm_slice_vol(q3d.e2(2),inv(SM0*Me2y),SD,0), ...
	  la2.*ipctb_spm_slice_vol(q3d.e2(3),inv(SM0*Me2z),SD,0));
      e3   = cat(3, la3.*ipctb_spm_slice_vol(q3d.e3(1),inv(SM0*Me3x),SD,0), ...
	  la3.*ipctb_spm_slice_vol(q3d.e3(2),inv(SM0*Me3y),SD,0), ...
	  la3.*ipctb_spm_slice_vol(q3d.e3(3),inv(SM0*Me3z),SD,0));
      mask = ipctb_spm_slice_vol(q3d.mask,inv(SM0*Mm),SD,0);
      mask = ((mask > q3d.thresh(1)) & (mask < q3d.thresh(2)))|isnan(la1);
      e1   = e1(qst1:qst:end,qst1:qst:end,:);
      e2   = e2(qst1:qst:end,qst1:qst:end,:);
      e3   = e3(qst1:qst:end,qst1:qst:end,:);
  ...
  case 'context_menu'  
    item0 = uimenu(varargin{3}, 'Label', 'Quiver3D');
	  ...
	  ['feval(''ipctb_spm_ov_quiver3d'',''context_init'', ', ...
	  ...
	  'Visible', 'off', 'Tag', ['QUIVER3D_1_', num2str(volhandle)]);
	    ...
	    ['feval(''ipctb_spm_ov_quiver3d'',''context_edit'',', ...
	...
	item2_3 = uimenu(item2, 'Label', 'Quiver3D length', 'Callback', ...
	  ...
	  ['feval(''ipctb_spm_ov_quiver3d'',''context_delete'', ', ...
  ...
  case 'context_init'
    ...
    Ve1fnames = ipctb_spm_select(3,'evec1.*\.img','Components of 1st eigenvector');
    Ve2fnames = ipctb_spm_select(3,'evec2.*\.img','Components of 2nd eigenvector');
    Ve3fnames = ipctb_spm_select(3,'evec3.*\.img','Components of 3rd eigenvector');
    Vlafnames = ipctb_spm_select(3,'eval.*\.img','Eigenvalue images');
    Vmaskfname = ipctb_spm_select(1,'image','Mask image');
    feval('ipctb_spm_ov_quiver3d','init',volhandle,Ve1fnames,Ve2fnames,Ve3fnames,Vlafnames,Vmaskfname);
  ...
  case 'context_edit'
    ...
    switch varargin{3}
      case 'thresh'
	in = ipctb_spm_input('Mask threshold {min max}','!+1','e', ...
	    num2str(st.vols{volhandle}.quiver3d.thresh), [1 2]);
      case 'qst'
	in = ipctb_spm_input('Quiver3D distance','!+1','e', ...
	    num2str(st.vols{volhandle}.quiver3d.qst), 1);
      case 'ql'
	in = ipctb_spm_input('Quiver3D length','!+1','e', ...
  ...
  case 'context_delete'
    feval('ipctb_spm_ov_quiver3d','delete',volhandle);
    obj = findobj(0, 'Tag',  ['QUIVER3D_1_', num2str(volhandle)]);
    set(obj, 'Visible', 'off');
    obj = findobj(0, 'Tag',  ['QUIVER3D_0_', num2str(volhandle)]);
    set(obj, 'Visible', 'on');
  ...
  otherwise
    warning('ipctb_spm_orthviews(''quiver3d'',...): Unknown action string');
  end;
