function varargout=ipctb_spm_platform(varargin)
% Platform specific configuration parameters for SPM
%
% FORMAT ans = ipctb_spm_platform(arg)
...
if ~isfinite(SPM_PLATFORM.bigend),
 if isnan(SPM_PLATFORM.bigend)
  error(['I dont know if "',computer,'" is big-endian.'])
 else
  error(['I dont think that "',computer,...
...
otherwise
 error(['Dont know filesystem ',SPM_PLATFORM.filesys])
end
...
% % 	SPM_PLATFORM.user    = getenv('USERNAME');
% % 	if isempty(SPM_PLATFORM.user)
% % 		SPM_PLATFORM.user = ipctb_spm_win32utils('username'); end
% % otherwise
