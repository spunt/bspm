function varargout = ipctb_spm_DesRep(varargin)
% Design reporting utilities
% FORMAT varargout = ipctb_spm_DesRep(action,varargin)
...
%-Format arguments
%-----------------------------------------------------------------------
if nargin==0, ipctb_spm_DesRep('DesRepUI'); return, end
...
if ~isfield(SPM.xX,'nKX')
	SPM.xX.nKX = ipctb_spm_DesMtx('Sca',SPM.xX.X,SPM.xX.name);
end
...
hDesMtx = uimenu(hC,'Label','Design Matrix','Accelerator','D',...
		'CallBack',[cb,...
		'ipctb_spm_DesRep(''DesMtx'',tmp.xX,',...
...
h = uimenu(hC,'Label','Design orthogonality','Accelerator','O',...
		'CallBack',[cb,...
		'ipctb_spm_DesRep(''DesOrth'',tmp.xX)'],...
		'UserData',hC,...
		'HandleVisibility','off');
...
case 'PET'
	hFnF = uimenu(hExplore,'Label','Files and factors','Accelerator','F',...
		'CallBack',[cb,...
		'ipctb_spm_DesRep(''Files&Factors'',',...
	...
	hCovs = uimenu(hExplore,'Label','Covariates','Accelerator','C',...
		'CallBack',[cb,...
		'ipctb_spm_DesRep(''Covs'',tmp.xX,tmp.xC)'],...
		'UserData',hC,...
		'HandleVisibility','off');
...
hxvi = uimenu(hExplore, 'Label','Covariance structure', ...
    'Callback',[cb, 'ipctb_spm_DesRep(''xVi'', tmp.xVi);'], 'UserData',hC, 'HandleVisibility','off');
...
uimenu(hC,'Label','Clear','Accelerator','L','Separator','on',...
	'CallBack','ipctb_spm_results_ui(''Clear'')',...
	'HandleVisibility','off');
uimenu(hC,'Label','Help','Separator','on',...
	'CallBack','ipctb_spm_help(''ipctb_spm_DesRep'')',...
	'HandleVisibility','off');
...
[fnames,CPath] = ipctb_spm_str_manip(fnames,'c');	%-extract common path component
...
ipctb_spm_results_ui('Clear',Fgraph,0)
...
for i = 1:nScan
	...
	if y<dy
		text(0.5,0,sprintf('Page %d',ipctb_spm_figure('#page')),...
			'FontSize',FS(8),'FontAngle','italic')
		ipctb_spm_figure('NewPage',[hAx;get(hAx,'Children')])
...
if ipctb_spm_figure('#page')>1
	text(0.5,0,sprintf('Page %d/%d',ipctb_spm_figure('#page')*[1,1]),...
		'FontSize',FS(8),'FontAngle','italic')
	ipctb_spm_figure('NewPage',[hAx;get(hAx,'Children')])
end
...
if isfield(varargin{2},'V')
  ...
  set(hCovMtxIm(1),'UserData',...
      varargin{2})
  set(hCovMtxIm(1),'ButtonDownFcn','ipctb_spm_DesRep(''SurfxVi_CB'')')
...
if isfield(varargin{2},'h')
  ...
  set(hParEstIm,'ButtonDownFcn','ipctb_spm_DesRep(''SurfHPEstIm_CB'')')
else
  hPEstAx = [];
end;
ipctb_spm_figure('NewPage',[hCovMtx;get(hCovMtx,'Children');hPEstAx;get(hPEstAx,'Children');hCovMtxSc])
...
if isfield(varargin{2},'Vi')
	for k = 1:length(varargin{2}.Vi)
		...
		if k<length(varargin{2}.Vi)
			ipctb_spm_figure('NewPage',[hCovMtx(k+1);get(hCovMtx(k+1),'Children')])
		end
	end
	if ipctb_spm_figure('#page')>1
		ipctb_spm_figure('NewPage',[hCovMtx(k+1);get(hCovMtx(k+1),'Children')])
...
%=======================================================================
% ipctb_spm_DesRep('DesMtx',xX,fnames,xs)
% ipctb_spm_DesRep('DesOrth',xX)
if nargin<2, error('insufficient arguments'), end
...
elseif iX	%-No scaled DesMtx, DesMtx in .xKXs structure
	hDesMtxIm = image((ipctb_spm_DesMtx('sca',varargin{2}.xKXs.X,Xnames) + 1)*32);
else		%-No scaled DesMtx, no .xKXs, DesMtx in .X
	hDesMtxIm = image((ipctb_spm_DesMtx('sca',varargin{2}.X,     Xnames) + 1)*32);
end

STick = ipctb_spm_DesRep('ScanTick',nScan,32);
PTick = ipctb_spm_DesRep('ScanTick',nPar,32);
...
if desmtx & ~isempty(fnames)
	...
	for i = STick
		...
		text(0,i,ipctb_spm_str_manip(str,'Ca35'));
	end
end
...
if desmtx
	...
	hPEstAx   = axes('Position',[.07 .315 .6 .025],...
			'DefaultTextInterpreter','TeX');
	if iX,	est = ipctb_spm_SpUtil('IsCon',varargin{2}.xKXs);
	else,	est = ipctb_spm_SpUtil('IsCon',varargin{2}.X); end
	...
	set(hDesOIm,...
		...
		'ButtonDownFcn','ipctb_spm_DesRep(''SurfDesO_CB'')')
...
%=======================================================================
% ipctb_spm_DesRep('Covs',xX,xC)
if nargin<3, error('insufficient arguments'), end
xC     = varargin{3};	%-Struct array of covariate information
if ~isstruct(varargin{2}), error('design matrix structure required'), end

if ~length(xC), ipctb_spm('alert!','No covariates!',mfilename), return, end
...
elseif isfield(varargin{2},'xKXs') & ~isempty(varargin{2}.xKXs)
	image(ipctb_spm_DesMtx('sca',varargin{2}.xKXs.X,Xnames)*32+32)
else
	image(ipctb_spm_DesMtx('sca',varargin{2}.X,Xnames)*32+32)
end
...
for i = 1:length(xC)
	...
	%---------------------------------------------------------------
	if length(xC)>1
		ipctb_spm_figure('NewPage',[hSTitle; hAx; get(hAx,'Children');...
...
case 'scantick'
%=======================================================================
% ipctb_spm_DesRep('ScanTick',nScan,lim)
% ( Show at most 32, showing every 2nd/3rd/4th/... as necessary to pair )
% ( down to <32 items. Always show last item so #images is indicated.    )     
...
%=======================================================================
% ipctb_spm_DesRep('SurfDesMtx_CB')
% ipctb_spm_DesRep('SurfDesMtxMo_CB')
% ipctb_spm_DesRep('SurfDesMtxUp_CB')
...
if strcmp(lower(varargin{1}),'surfdesmtx_cb')
	...
	set(h,'UserData',UD)
	set(gcbf,'WindowButtonMotionFcn','ipctb_spm_DesRep(''SurfDesMtxMo_CB'')',...
		 'WindowButtonUpFcn',    'ipctb_spm_DesRep(''SurfDesMtxUp_CB'')')
end
...
%=======================================================================
% ipctb_spm_DesRep('SurfEstIm_CB')
% ipctb_spm_DesRep('SurfEstImMo_CB')
% ipctb_spm_DesRep('SurfEstImUp_CB')
...
if strcmp(lower(varargin{1}),'surfestim_cb')
	UD = struct(	'String',	get(h,'String'),...
			...
			'UserData',	get(h,'UserData'));
	set(h,'UserData',UD)
	set(gcbf,'WindowButtonMotionFcn','ipctb_spm_DesRep(''SurfEstImMo_CB'')',...
		 'WindowButtonUpFcn',    'ipctb_spm_DesRep(''SurfEstImUp_CB'')')
end
...
%=======================================================================
% ipctb_spm_DesRep('SurfDesO_CB')
% ipctb_spm_DesRep('SurfDesOMo_CB')
% ipctb_spm_DesRep('SurfDesOUp_CB')
...
if strcmp(lower(varargin{1}),'surfdeso_cb')
	UD = struct(	'String',	get(h,'String'),...
			...
			'UserData',	get(h,'UserData'));
	set(h,'UserData',UD)
	set(gcbf,'WindowButtonMotionFcn','ipctb_spm_DesRep(''SurfDesOMo_CB'')',...
		 'WindowButtonUpFcn',    'ipctb_spm_DesRep(''SurfDesOUp_CB'')')
end
...
%=======================================================================
% ipctb_spm_DesRep('SurfCon_CB')
% ipctb_spm_DesRep('SurfConOMo_CB')
% ipctb_spm_DesRep('SurfConOUp_CB')
...
if strcmp(lower(varargin{1}),'surfcon_cb')
	UD = struct(	'String',	get(h,'String'),...
			...
			'UserData',	get(h,'UserData'));
	set(h,'UserData',UD)
	set(gcbf,'WindowButtonMotionFcn','ipctb_spm_DesRep(''SurfConMo_CB'')',...
		 'WindowButtonUpFcn',    'ipctb_spm_DesRep(''SurfConUp_CB'')')
end
...
%=======================================================================
% ipctb_spm_DesRep('SurfxVi_CB')
% ipctb_spm_DesRep('SurfxViMo_CB')
% ipctb_spm_DesRep('SurfxViUp_CB')
...
if strcmp(lower(varargin{1}),'surfxvi_cb')
      UD = struct(    'String',       get(h,'String'),...
                      ...
                      'UserData',     get(h,'UserData'));
      set(h,'UserData',UD)
      set(gcbf,'WindowButtonMotionFcn','ipctb_spm_DesRep(''SurfxViMo_CB'')',...
               'WindowButtonUpFcn',    'ipctb_spm_DesRep(''SurfxViUp_CB'')')
end
...
%=======================================================================
% ipctb_spm_DesRep('SurfHPEstim_CB')
% ipctb_spm_DesRep('SurfHPEstimMo_CB')
% ipctb_spm_DesRep('SurfHPEstimUp_CB')
...
if strcmp(lower(varargin{1}),'surfhpestim_cb')
      UD = struct(    'String',       get(h,'String'),...
                      ...
                      'UserData',     get(h,'UserData'));
      set(h,'UserData',UD)
      set(gcbf,'WindowButtonMotionFcn','ipctb_spm_DesRep(''SurfHPEstimMo_CB'')',...
               'WindowButtonUpFcn',    'ipctb_spm_DesRep(''SurfHPEstimUp_CB'')')
end
