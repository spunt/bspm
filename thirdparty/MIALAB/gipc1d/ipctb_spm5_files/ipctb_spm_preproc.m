function results = ipctb_spm_preproc(varargin)
...
if nargin==0
    V = ipctb_spm_select(1,'image');
else
    V = varargin{1};
end;
if ischar(V), V = ipctb_spm_vol(V); end;
...
B         = ipctb_spm_vol(opts.tpm);
b0        = ipctb_spm_load_priors(B);
...
vx   = sqrt(sum(V(1).mat(1:3,1:3).^2));
kron = inline('ipctb_spm_krutil(a,b)','a','b');
...
B3warp = ipctb_spm_dctmtx(d(3),d2(3),z0);
B2warp = ipctb_spm_dctmtx(d(2),d2(2),y0(1,:)');
B1warp = ipctb_spm_dctmtx(d(1),d2(1),x0(:,1));
...
B3bias  = ipctb_spm_dctmtx(d(3),d3(3),z0);
B2bias  = ipctb_spm_dctmtx(d(2),d3(2),y0(1,:)');
B1bias  = ipctb_spm_dctmtx(d(1),d3(1),x0(:,1));
...
f = zeros(d);
for z=1:length(z0),
    f(:,:,z) = ipctb_spm_sample_vol(V,x0,y0,o*z0(z),0);
end;
[thresh,mx]  = ipctb_spm_minmax(f);
mn           = zeros(K,1);
...
if ~isempty(opts.regtype),
    Affine  = ipctb_spm_maff(V,{x0,y0,z0},b0,B(1).mat,Affine,opts.regtype,ff*100);
    Affine  = ipctb_spm_maff(V,{x0,y0,z0},b0,B(1).mat,Affine,opts.regtype,ff);
end;
...
for z=1:length(z0),
    ...
    buf(z).msk = ipctb_spm_sample_priors(b0{end},x1,y1,z1,1)<(1-1/512);
    fz         = f(:,:,z);
    %buf(z).msk = fz>thresh;
    if ~isempty(opts.msk),
        msk = ipctb_spm_sample_vol(VM,x0,y0,o*z0(z),0);
...
ipctb_spm_chi2_plot('Init','Processing','Log-likelihood','Iteration');
for iter=1:100,
    ...
    if finalit,
        ...
        for z=1:length(z0),
            fz         = ipctb_spm_sample_vol(V,x0,y0,o*z0(z),0);
            buf(z).msk = fz~=0;
            if ~isempty(opts.msk),
                msk = ipctb_spm_sample_vol(VM,x0,y0,o*z0(z),0);
                buf(z).msk = buf(z).msk & msk;
    ...
    for z=1:length(z0),
        ...
        for k1=1:Kb,
            tmp              = ipctb_spm_sample_priors(b0{k1},x1,y1,z1,k1==Kb);
...
if finalit, fprintf('Mix: %g\n',ll); end;
        ...
        for subit=1:40,
            ...
            for z=1:length(z0),
                ...
                b3    = B3bias(z,:)';
                Beta  = Beta  + kron(b3,ipctb_spm_krutil(wt1,B1bias,B2bias,0));
                Alpha = Alpha + kron(b3*b3',ipctb_spm_krutil(wt2,B1bias,B2bias,1));
...
if finalit, fprintf('Bia: %g\n',ll); end;
    ...
    for subit=1:20,
        ...
        for z=1:length(z0),
            ...
            for k1=1:Kb,
                [b(:,k1),db1(:,k1),db2(:,k1),db3(:,k1)] = ipctb_spm_sample_priors(b0{k1},x1,y1,z1,k1==Kb);
                s   = s   + mg1(k1)*  b(:,k1);
            ...
            b3     = B3warp(z,:)';
            Beta   = Beta - [...
                kron(b3,ipctb_spm_krutil(dp1,B1warp,B2warp,0))
                kron(b3,ipctb_spm_krutil(dp2,B1warp,B2warp,0))
                kron(b3,ipctb_spm_krutil(dp3,B1warp,B2warp,0))];

            b3b3   = b3*b3';
            A{1,1} = A{1,1} +  kron(b3b3,ipctb_spm_krutil(dp1.*dp1,B1warp,B2warp,1));
            A{1,2} = A{1,2} +  kron(b3b3,ipctb_spm_krutil(dp1.*dp2,B1warp,B2warp,1));
            A{1,3} = A{1,3} +  kron(b3b3,ipctb_spm_krutil(dp1.*dp3,B1warp,B2warp,1));
            A{2,2} = A{2,2} +  kron(b3b3,ipctb_spm_krutil(dp2.*dp2,B1warp,B2warp,1));
            A{2,3} = A{2,3} +  kron(b3b3,ipctb_spm_krutil(dp2.*dp3,B1warp,B2warp,1));
            A{3,3} = A{3,3} +  kron(b3b3,ipctb_spm_krutil(dp3.*dp3,B1warp,B2warp,1));
        ...
        for subit1 = 1:3,
            ...
            for z=1:length(z0),
                ...
                s  = zeros(buf(z).nm,1)+tiny;
                for k1=1:Kb,
                    b(:,k1) = ipctb_spm_sample_priors(b0{k1},x1,y1,z1,k1==Kb);
                    s       = s + mg1(k1)*b(:,k1);
                end;
                for k1=1:Kb,
