function varargout=ipctb_spm_XYZreg(varargin)
...
%-----------------------------------------------------------------------
[d,i] = min(ipctb_spm_XYZreg('Edist',varargin{2},varargin{3}));
...
case 'edist'
%=======================================================================
% d = ipctb_spm_XYZreg('Edist',xyz,XYZ)
if nargin<3, error('Insufficient arguments'), end
...
%=======================================================================
% [hReg,xyz] = ipctb_spm_XYZreg('InitReg',hReg,M,D,xyz)
...
%-----------------------------------------------------------------------
[xyz,d] = ipctb_spm_XYZreg('RoundCoords',xyz,M,D);
...
%=======================================================================
% ipctb_spm_XYZreg('UnInitReg',hReg)
hReg = varargin{2};
...
%=======================================================================
% xyz = ipctb_spm_XYZreg('GetCoords',hReg)
if nargin<2, hReg=ipctb_spm_XYZreg('FindReg'); else, hReg=varargin{2}; end
...
%=======================================================================
% [xyz,d] = ipctb_spm_XYZreg('SetCoords',xyz,hReg,hC,Reg)
...
%-----------------------------------------------------------------------
if hC<=0
	[xyz,d] = ipctb_spm_XYZreg('RoundCoords',xyz,RD.M,RD.D);
...
%-----------------------------------------------------------------------
if nargin<5
	RD.Reg = ipctb_spm_XYZreg('VReg',RD.Reg);
	Reg    = RD.Reg;
else
	Reg = ipctb_spm_XYZreg('VReg',varargin{5});
end
...
%=======================================================================
% nReg = ipctb_spm_XYZreg('XReg',hReg,{h,Fcn}pairs)
if nargin<4, error('Insufficient arguments'), end
hReg = varargin{2};
...
nReg = ipctb_spm_XYZreg('VReg',nReg,'Warn');

%-Set hReg registry link for registry candidates (Back registration)
%-----------------------------------------------------------------------
for i = 1:size(nReg,1)
	ipctb_spm_XYZreg('SetReg',nReg{i,1},hReg);
end
...
%-----------------------------------------------------------------------
ipctb_spm_XYZreg('SetCoords',RD.xyz,hReg,hReg,nReg);
...
%=======================================================================
% nReg = ipctb_spm_XYZreg('Add2Reg',hReg,{h,Fcn}pairs)
if nargin<4, error('Insufficient arguments'), end
hReg = varargin{2};
...
%=======================================================================
% ipctb_spm_XYZreg('SetReg',h,hReg)
if nargin<3, error('Insufficient arguments'), end
h    = varargin{2};
hReg = varargin{3};
...
%=======================================================================
% nReg = ipctb_spm_XYZreg('unXReg',hReg,hD1,hD2,hD3,...)
if nargin<3, error('Insufficient arguments'), end
hD   = [varargin{3:end}];
hReg = varargin{2};
...
hD(e)      = [];
dReg       = ipctb_spm_XYZreg('VReg',Reg(i,:));
Reg(i,:)   = [];
...
%-----------------------------------------------------------------------
for i = 1:size(dReg,1)
	ipctb_spm_XYZreg('SetReg',dReg{i,1},[]);
end
...
%=======================================================================
% nReg = ipctb_spm_XYZreg('Del2Reg',hReg,hD)
if nargin<3, error('Insufficient arguments'), end
hD   = [varargin{3:end}];
hReg = varargin{2};
...
%=======================================================================
% ipctb_spm_XYZreg('UnSetReg',h)
if nargin<2, error('Insufficient arguments'), end
ipctb_spm_XYZreg('SetReg',varargin{2},[])
...
%=======================================================================
% ipctb_spm_XYZreg('CleanReg',hReg)
%if ~strcmp(get(hReg,'Tag'),'hReg'), error('Not a registry'), end
hReg = varargin{2};
RD = get(hReg,'UserData');
RD.Reg = ipctb_spm_XYZreg('VReg',RD.Reg,'Warn');
set(hReg,'UserData',RD)
varargout = {Reg};
...
%=======================================================================
% Reg = ipctb_spm_XYZreg('VReg',Reg,Warn)
...
%=======================================================================
% hReg = ipctb_spm_XYZreg('FindReg',h)
