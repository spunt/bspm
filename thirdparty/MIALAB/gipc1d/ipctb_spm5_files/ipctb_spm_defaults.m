function ipctb_spm_defaults
% Sets the defaults which are used by SPM
%
% FORMAT ipctb_spm_defaults
...
% $Id: ipctb_spm_defaults.m 256 2005-10-17 18:57:24Z guillaume $
...
defaults.stats.fmri.t   = 30;
defaults.stats.fmri.t0  = 15;
...
defaults.segment.estimate.priors = char(...
	fullfile(ipctb_spm('Dir'),'apriori','grey.nii'),...
	fullfile(ipctb_spm('Dir'),'apriori','white.nii'),...
	fullfile(ipctb_spm('Dir'),'apriori','csf.nii'));
...
%defaults.segment.estimate.affreg.weight = fullfile(ipctb_spm('Dir'),'apriori','brainmask.nii'); 
...
defaults.preproc.tpm     = char(...
    fullfile(ipctb_spm('Dir'),'tpm','grey.nii'),...
    fullfile(ipctb_spm('Dir'),'tpm','white.nii'),...
    fullfile(ipctb_spm('Dir'),'tpm','csf.nii')); % Prior probability maps
...
defaults.imcalc.dtype  = ipctb_spm_type('int16'); % Data Type
