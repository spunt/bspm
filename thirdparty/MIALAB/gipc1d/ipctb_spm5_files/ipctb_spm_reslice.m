function ipctb_spm_reslice(P,flags)
...
if flags.mask | flags.mean,
	ipctb_spm_progress_bar('Init',P(1).dim(3),'Computing available voxels','planes completed');
...
ipctb_spm_progress_bar('Init',nread,'Reslicing','volumes completed');
...
for i = 1:prod(size(P)),
	...
	if read_vol,
		...
		if ~isfinite(flags.interp),
			v = abs(kspace3d(ipctb_spm_bsplinc(P(i),[0 0 0 ; 0 0 0]'),P(1).mat\P(i).mat));
			for x3 = 1:P(1).dim(3),
				if flags.mean,
				...
				if write_vol,
					VO = ipctb_spm_write_plane(VO,v(:,:,x3),x3);
				end;
			end;
		else,
			C = ipctb_spm_bsplinc(P(i), d);
			for x3 = 1:P(1).dim(3),

				[tmp,y1,y2,y3] = getmask(inv(P(1).mat\P(i).mat),x1,x2,x3,P(i).dim(1:3),flags.wrap);
				v              = ipctb_spm_bsplins(C, y1,y2,y3, d);
				% v(~tmp)        = 0;
				...
				if write_vol,
					...
					VO = ipctb_spm_write_plane(VO,v,x3);
				end;
			end;
		end;
		nread = nread + 1;
	end;
	ipctb_spm_progress_bar('Set',nread);
end;
...
if flags.mean
	...
	ipctb_spm_write_vol(PO,Integral);
end
