function varargout=ipctb_spm_XYZreg_Ex2(varargin)
...
xyz = ipctb_spm_XYZreg('RoundCoords',xyz,M,D);
...
hMe = uicontrol(F,'Style','Pushbutton',...
	...
	'Callback',...
	'ipctb_spm_XYZreg_Ex2(''SetCoords'',input(''Enter [x,y,z]'''' vector: ''),gcbo);',...
	'UserData',struct(...
		'hReg',	[],...
		'M',	M,...
		'D',	D,...
...
%=======================================================================
% [xyz,d] = ipctb_spm_XYZreg_Ex2('SetCoords',xyz,hMe,hC)
...
%-----------------------------------------------------------------------
if hC<=0
	[xyz,d] = ipctb_spm_XYZreg('RoundCoords',xyz,UD.M,UD.D);
...
if (~isempty(UD.hReg) & UD.hReg~=hC)
	ipctb_spm_XYZreg('SetCoords',xyz,UD.hReg,hMe);
end
