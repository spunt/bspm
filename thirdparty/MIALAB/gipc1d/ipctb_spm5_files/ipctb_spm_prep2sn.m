function [po,pin] = ipctb_spm_prep2sn(p)
...
[Y1,Y2,Y3]  =     ipctb_spm_invdef(Y1,Y2,Y3,VG(1).dim(1:3),eye(4),eye(4));
...
AA  = eye(pd)*0.01;
Ab  = zeros(pd,3);
ipctb_spm_progress_bar('init',length(x3),['Reparameterising'],'Planes completed');
mx = [0 0 0];
for z=1:length(x3),
    y1       = double(Y1(:,:,z));
    y2       = double(Y2(:,:,z));
    ...
    b3       = B3(z,:)';
    Ab(:,1)  = Ab(:,1) + kron(b3,ipctb_spm_krutil(z1,B1,B2,0));
    Ab(:,2)  = Ab(:,2) + kron(b3,ipctb_spm_krutil(z2,B1,B2,0));
    Ab(:,3)  = Ab(:,3) + kron(b3,ipctb_spm_krutil(z3,B1,B2,0));
    AA       = AA  + kron(b3*b3',ipctb_spm_krutil(w, B1,B2,1));
...
c2  = [0 0 0];
sw  =  0;
ipctb_spm_progress_bar('init',length(x3),['Procrustes (1)'],'Planes completed');
for z=1:length(x3),
    y1         = double(Y1(:,:,z));
    y2         = double(Y2(:,:,z));
    y3         = double(Y3(:,:,z));
    msk        = find(isfinite(y1));
    w          = ipctb_spm_sample_vol(B(1),x1(msk),x2(msk),o(msk)*z,0);
    swz        = sum(w(:));
    sw         = sw+swz;
...
T2 = [eye(4,3) M2*[c2 1]'];
C  = zeros(3);
ipctb_spm_progress_bar('init',length(x3),['Procrustes (2)'],'Planes completed');
for z=1:length(x3),
    y1         = double(Y1(:,:,z));
    y2         = double(Y2(:,:,z));
    y3         = double(Y3(:,:,z));
...
ipctb_spm_progress_bar('init',length(x3),['Creating Def: ' nam],'Planes completed');
for z=1:length(x3),
