function varargout = ipctb_spm_FcUtil(varargin)
% Contrast utilities
...
switch set_action,
        case {'c','c+'}
	   Fc.iX0  = set_action;
           c = ipctb_spm_sp(':', sX, varargin{5});
	   if isempty(c)
              [Fc.X1o.ukX1o Fc.X0.ukX0] = ipctb_spm_SpUtil('+c->Tsp',sX,[]);
	   ...
	   elseif size(c,1) ~= sL, 
	      error(['not contrast dim. in ' mfilename ' ' set_action]); 
       	      ...
       	      if strcmp(set_action,'c+')  
       	         if ~ipctb_spm_sp('isinspp',sX,c), c = ipctb_spm_sp('oPp:',sX,c); end;
	      end;
		 ...
		 error('trying to define a t that looks like an F'); 
	      ...
	      Fc.c   = c;
              [Fc.X1o.ukX1o Fc.X0.ukX0] = ipctb_spm_SpUtil('+c->Tsp',sX,c);
...
% 'option given for completeness - not for SPM use'
	...
	case {'X0'}
	   ...
	   Fc.iX0  = set_action;
	   X0 = ipctb_spm_sp(':', sX, varargin{5});
	   if isempty(X0),
              Fc.c         = ipctb_spm_sp('xpx',sX); 
              Fc.X1o.ukX1o = ipctb_spm_sp('cukx',sX);
              Fc.X0.ukX0   = [];	
	   elseif size(X0,1) ~= sC, 
              error('dimension of X0 wrong in Set');
	   else 
              Fc.c         = ipctb_spm_SpUtil('X0->c',sX,X0);
	      Fc.X0.ukX0   = ipctb_spm_sp('ox',sX)'*X0;	
              Fc.X1o.ukX1o = ipctb_spm_SpUtil('+c->Tsp',sX,Fc.c);
	   end
	   ...
	   if isempty(ukX0), 
              ...
              Fc.X1o.ukX1o = ipctb_spm_sp('cukx',sX);
              Fc.X0.ukX0   = [];	
	   elseif size(ukX0,1) ~= ipctb_spm_sp('rk',sX), 
              error('dimension of cukX0 wrong in Set');
	   else 
              Fc.c         = ipctb_spm_SpUtil('+X0->c',sX,ukX0);
	      Fc.X0.ukX0   = ukX0;	
              Fc.X1o.ukX1o = ipctb_spm_SpUtil('+c->Tsp',sX,Fc.c);
	   end 
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	...
	case 'iX0'
	   iX0 	      = varargin{5};
	   iX0 	      = ipctb_spm_SpUtil('iX0check',iX0,sL);
	   Fc.iX0     = iX0;
           Fc.X0.ukX0 = ipctb_spm_sp('ox',sX)' * ipctb_spm_sp('Xi',sX,iX0);
      	   if isempty(iX0), 
	      Fc.c          = ipctb_spm_sp('xpx',sX); 
	      Fc.X1o.ukX1o  = ipctb_spm_sp('cukx',sX);  
	   else 			
   	      Fc.c          = ipctb_spm_SpUtil('i0->c',sX,iX0);
   	      Fc.X1o.ukX1o  = ipctb_spm_SpUtil('+c->Tsp',sX,Fc.c);
	   end;
...
case 'x0' % ipctb_spm_FcUtil('X0',Fc,sX)
%=======================================================================
if nargin ~= 3, 
...
case 'x1o' % ipctb_spm_FcUtil('X1o',Fc,sX)
%=======================================================================
if nargin ~= 3, 
   error('too few/many input arguments - need 2');
else 
...
case 'isfcon'				%- Is it an F contrast ?
%=======================================================================
% yes_no = ipctb_spm_FcUtil('IsFcon',Fc)
...
case 'fconedf'				%- F contrast edf
%=======================================================================
% [edf_tsp edf_Xsp] = ipctb_spm_FcUtil('FconEdf', Fc, sX [, V])
...
if ~ipctb_spm_sp('isspc',sX)
	sX = ipctb_spm_sp('set',sX);	end;

if ~sf_isempty_X1o(Fc)
   [trMV, trMVMV] = ipctb_spm_SpUtil('trMV',sf_X1o(Fc,sX),V);
else
   trMV   = 0;
   trMVMV = 0;
end	
...
%=======================================================================
% hsqr = ipctb_spm_FcUtil('Hsqr',Fc, sX)
...
if sf_isempty_X1o(Fc)
	if ~sf_isempty_X0(Fc)
		...
		%- warning(' Empty X1o in ipctb_spm_FcUtil(''Hsqr'',Fc,sX) ');
		varargout = { zeros(1,ipctb_spm_sp('size',sX,2)) };
	else 	
...
if sf_isempty_X1o(Fc)
	if ~sf_isempty_X0(Fc)
		%- assumes that X0 is sX.X
		%- warning(' Empty X1o in ipctb_spm_FcUtil(''H'',Fc,sX) ');
		varargout = { zeros(ipctb_spm_sp('size',sX,2)) };
	else 	
		error(' Fc must be set ');
	end
else
...
case 'yc'     %- Fitted data corrected for confounds defined by Fc 
%=======================================================================
% Yc = ipctb_spm_FcUtil('Yc',Fc, sX, b)
...
%=======================================================================
% Y0 = ipctb_spm_FcUtil('Y0',Fc, sX, b)
...
case {'|_'}     %-  Fc orthogonalisation 
%=======================================================================
% Fc = ipctb_spm_FcUtil('|_',Fc1, sX, Fc2)
...
Fc2  = ipctb_spm_FcUtil('Set',str,'F','c+',cat(2,Fc2(:).c),sX);
...
case {'|_?'}    	%-  Are contrasts orthogonals 
%=======================================================================
% b = ipctb_spm_FcUtil('|_?',Fc1, sX [, Fc2])
...
case 'in'     %-  Fc1 is in list of  contrasts Fc2
%=======================================================================
% [iFc2 iFc1] = ipctb_spm_FcUtil('In', Fc1, sX, Fc2)
...
% or manipulated (ortho ..) with ipctb_spm_FcUtil
...
case '~unique'     %-  Fc list unique 
%=======================================================================
% idx = ipctb_spm_FcUtil('~unique', Fc, sX)
...
otherwise
%=======================================================================
error('Unknown action string in ipctb_spm_FcUtil')
...
%=======================================================================
% Fcon = ipctb_spm_FcUtil('FconFields')
...
if sf_ver(Fc) > 1, 
   b = ipctb_spm_sp('ox',sX)*Fc.X1o.ukX1o; 
else, 
   b = Fc.X1o;
end
...
if sf_ver(Fc) > 1, 
   b = ipctb_spm_sp('ox',sX)*Fc.X0.ukX0; 
else, 
   b = Fc.X0;
end
...
%-**** fprintf('v%d\n',sf_ver(Fc));
if sf_ver(Fc) > 1,    
   hsqr = ipctb_spm_sp('ox',ipctb_spm_sp('set',Fc.X1o.ukX1o))' * ipctb_spm_sp('cukx',sX);
else, 
   hsqr = ipctb_spm_sp('ox',ipctb_spm_sp('set',Fc.X1o))'*ipctb_spm_sp('x',sX);
end
%=======================================================================
% H = ipctb_spm_FcUtil('H',Fc)
...
if sf_ver(Fc) > 1, 
   ...
   % H = {c*ipctb_spm_sp('x-',ipctb_spm_sp('Set',c'*ipctb_spm_sp('xpx-',sX)*c) )*c'}
end
...
Yc =  sX.X*ipctb_spm_sp('xpx-',sX)*sf_H(Fc,sX)*b;

%=======================================================================
% Y0 = ipctb_spm_FcUtil('Y0',Fc,sX,b)
function Y0 = sf_Y0(Fc,sX,b)

Y0 = sX.X*(eye(ipctb_spm_sp('size',sX,2)) - ipctb_spm_sp('xpx-',sX)*sf_H(Fc,sX))*b;
...
c1_2  = Fc1.c - sf_H(Fc2,sX)*ipctb_spm_sp('xpx-:',sX,Fc1.c);
Fc1o  = ipctb_spm_FcUtil('Set',['(' Fc1.name ' |_ (' Fc2.name '))'], ...
		    Fc1.STAT, 'c+',c1_2,sX);

%- In the large (scans) dimension :
%- c = sX.X'*ipctb_spm_sp('r:',ipctb_spm_sp('set',Fc2.X1o),Fc1.X1o);
%- Fc1o  = ipctb_spm_FcUtil('Set',['(' Fc1.name ' |_ (' Fc2.name '))'], ...
%-						    Fc1.STAT, 'c',c,sX);
...
if isempty(Fc2)
   if length(Fc1) <= 1, b = 0; 
      ...
      c1 = cat(2,Fc1(:).c); 
      b = ~any(any( abs(triu(c1'*ipctb_spm_sp('xpx-:',sX,c1), 1)) > sX.tol));
   end
else 
   c1 = cat(2,Fc1(:).c); c2 = cat(2,Fc2(:).c); 
   b = ~any(any( abs(c1'*ipctb_spm_sp('xpx-:',sX,c2)) > sX.tol ));
end
...
%- this is ensured if ipctb_spm_FcUtil is used

function boul = sf_isnull(Fc,sX)
%
boul = ~any(any(ipctb_spm_sp('oPp:',sX,Fc.c)));

%=======================================================================
% Fc = ipctb_spm_FcUtil('Set',name, STAT, set_action, value, sX)
...
for j=1:L1
   ...
   if ~ipctb_spm_sp('isinspp',sX,Fc1(j).c), %- warning ? 
      c1  = ipctb_spm_sp('oPp:',sX,Fc1(j).c); 
   else, 
      c1 = Fc1(j).c; 
   end
   sc1 = ipctb_spm_sp('Set',c1); 
   S   = Fc1(j).STAT;
   ...
   for i =1:L2,
      if Fc2(i).STAT == S
         ...
         boul = ipctb_spm_sp('==',sc1,ipctb_spm_sp('oPp',sX,Fc2(i).c));
