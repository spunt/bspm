function VO = ipctb_spm_smoothto8bit(V,fwhm)
...
VO        = V;
VO.dt     = [ipctb_spm_type('uint8') ipctb_spm_platform('bigend')];
V0.dat    = uint8(0);
...
for i=1:V.dim(3)+r{3}.s,
	if i<=V.dim(3),
		img      = ipctb_spm_slice_vol(V,ipctb_spm_matrix([0 0 i]),V.dim(1:2),0);
