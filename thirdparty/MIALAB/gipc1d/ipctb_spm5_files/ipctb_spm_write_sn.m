function VO = ipctb_spm_write_sn(V,prm,flags,extras)
...
ipctb_spm_progress_bar('Init',numel(V),'Resampling','volumes completed');
for i=1:numel(V),
	...
	if nargout~=0,
		VO.pinfo  = [1 0]';
		VO.dt     = [ipctb_spm_type('float32') ipctb_spm_platform('bigend')];
		VO.dat    = Dat;
	end;
...
[X,Y] = ndgrid(x,y);
Tr = prm.Tr;
BX = ipctb_spm_dctmtx(prm.VG(1).dim(1),size(Tr,1),x-1);
BY = ipctb_spm_dctmtx(prm.VG(1).dim(2),size(Tr,2),y-1);
BZ = ipctb_spm_dctmtx(prm.VG(1).dim(3),size(Tr,3),z-1);
if flags.preserve,
	DX = ipctb_spm_dctmtx(prm.VG(1).dim(1),size(Tr,1),x-1,'diff');
	DY = ipctb_spm_dctmtx(prm.VG(1).dim(2),size(Tr,2),y-1,'diff');
	DZ = ipctb_spm_dctmtx(prm.VG(1).dim(3),size(Tr,3),z-1,'diff');
end;
d  = [flags.interp*[1 1 1]' flags.wrap(:)];
	...
	for j=1:length(z),   % Cycle over planes
		...
		if ~flags.preserve,
			if nargout>0,
				...
				VO = ipctb_spm_write_plane(VO,dat,j);
			end;
		else
		...
		if numel(V)<5, ipctb_spm_progress_bar('Set',i-1+j/length(z)); end;
	end;
	if nargout==0,
		if flags.preserve,
			VO = ipctb_spm_write_vol(VO,Dat);
		end;
	else
		VO.pinfo  = [1 0]';
		VO.dt     = [ipctb_spm_type('float32') ipctb_spm_platform('bigend')];
...
ipctb_spm_progress_bar('Init',numel(V),'Modulating','volumes completed');
for i=1:numel(V),
	VO          = V(i);
	VO.fname    = prepend(VO.fname,'m');
	detAff      = det(prm.VF.mat*prm.Affine/prm.VG(1).mat);
	...
	if isempty(Tr),
		for j=1:length(z),   % Cycle over planes
			dat        = ipctb_spm_slice_vol(V(i),ipctb_spm_matrix([0 0 j]),V(i).dim(1:2),0);
			Dat(:,:,j) = single(dat);
			if numel(V)<5, ipctb_spm_progress_bar('Set',i-1+j/length(z)); end;
		end;
	else
...
if numel(V)>1 && any(any(diff(t,1,1))),
	ipctb_spm_progress_bar('Init',length(z),'Computing available voxels','planes completed');
	for j=1:length(z),   % Cycle over planes
