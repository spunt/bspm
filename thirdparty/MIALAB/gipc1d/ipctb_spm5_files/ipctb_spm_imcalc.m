function Vo = ipctb_spm_imcalc(Vi,Vo,f,flags,varargin)
% Perform algebraic functions on images
% FORMAT Vo = ipctb_spm_imcalc(Vi,Vo,f,flags,Xtra_vars...)
% Vi            - vector of mapped image volumes to work on (from ipctb_spm_vol)
% Vo (input)    - handle structure containing information on output image
%                 ( pinfo field is computed for the resultant image data, )
%                 ( and can be omitted from Vo on input.  See ipctb_spm_vol     )
...
% hold          - interpolation hold (see ipctb_spm_slice_vol)
...
% treats them as NaN. NaN's are written as zero (by ipctb_spm_write_plane),
...
% $Id: ipctb_spm_imcalc.m 184 2005-05-31 13:23:32Z john $
...
%-Start progress plot
%-----------------------------------------------------------------------
ipctb_spm_progress_bar('Init',Vo.dim(3),f,'planes completed');


%-Loop over planes computing result Y
%-----------------------------------------------------------------------
for p = 1:Vo.dim(3),
	B = ipctb_spm_matrix([0 0 -p 0 0 0 1 1 1]);

	if dmtx, X=zeros(n,prod(Vo.dim(1:2))); end
	for i = 1:n
		M = inv(B*inv(Vo.mat)*Vi(i).mat);
		d = ipctb_spm_slice_vol(Vi(i),M,Vo.dim(1:2),[hold,NaN]);
		if (mask<0), d(isnan(d))=0; end;
		if (mask>0) && ~ipctb_spm_type(Vi(i).dt(1),'nanrep'), d(d==0)=NaN; end
...
%-Write output image (uses ipctb_spm_write_vol - which calls ipctb_spm_write_plane)
%-----------------------------------------------------------------------
Vo = ipctb_spm_write_vol(Vo,Y);
