function TabDat = ipctb_spm_VOI(SPM,xSPM,hReg)
...
%-Title
%-----------------------------------------------------------------------
ipctb_spm('FigName',['SPM{',xSPM.STAT,'}: Small Volume Correction']);
...
SPACE    = ipctb_spm_input('Search volume...',-1,'m',...
...
switch SPACE
	...
	case 'S' %-Sphere
	%---------------------------------------------------------------
	D          = ipctb_spm_input('radius of VOI {mm}',-2);
	...
	case 'B' %-Box
	%---------------------------------------------------------------
	D          = ipctb_spm_input('box dimensions [k l m] {mm}',-2);
	...
	case 'I' %-Mask Image
	%---------------------------------------------------------------
	Msk   = ipctb_spm_select(1,'image','Image defining search volume');
	D     = ipctb_spm_vol(Msk);
	str   = sprintf('image mask: %s',ipctb_spm_str_manip(Msk,'a30'));
	VOX   = sqrt(sum(D.mat(1:3,1:3).^2));
	FWHM  = FWHM.*(xSPM.VOX./VOX);
	XYZ   = D.mat \ [xSPM.XYZmm; ones(1, size(xSPM.XYZmm, 2))];
	j     = find(ipctb_spm_sample_vol(D, XYZ(1,:), XYZ(2,:), XYZ(3,:),0) > 0);
	XYZ   = D.mat \ [     XYZmm; ones(1, size(    XYZmm, 2))];
	k     = find(ipctb_spm_sample_vol(D, XYZ(1,:), XYZ(2,:), XYZ(3,:),0) > 0);

end

xSPM.S     = length(k);
xSPM.R     = ipctb_spm_resels(FWHM,D,SPACE);
xSPM.Z     = xSPM.Z(j);
xSPM.XYZ   = xSPM.XYZ(:,j);
...
TabDat    = ipctb_spm_list('List',xSPM,hReg,Num,Dis,str);

%-Reset title
%-----------------------------------------------------------------------
ipctb_spm('FigName',['SPM{',xSPM.STAT,'}: Results']);
