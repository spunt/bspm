function ret = ipctb_spm_quiver(varargin)
% %W% Volkmar Glauche <glauche@uke.uni-hamburg.de> %E%

global st;
if isempty(st)
  error('quiver: This routine can only be called as a plugin for ipctb_spm_orthviews!');
end;

if nargin < 2
  error('quiver: Wrong number of arguments. Usage: ipctb_spm_orthviews(''quiver'', cmd, volhandle, varargin)');
end;
...
switch cmd
  case 'init'     %function addquiver(handle, Vqfnames, Vmaskfname, varargin)
    if nargin < 4
      error('ipctb_spm_orthviews(''quiver'', ''init'',...): Not enough arguments');
    end;
    Vq = ipctb_spm_vol(varargin{3});
    Vmask = ipctb_spm_vol(varargin{4});
    if length(Vq) == 3
      st.vols{volhandle}.quiver = struct('qx',Vq(1),'qy',Vq(2),'qz',Vq(3), ...
	  'mask',Vmask, 'fa',[], 'thresh', [.1 Inf], 'ls','y.', ...
	  'qst',3,'ql',.9,'qht',[],'qhc',[],'qhs',[], 'qw', .5);
    else
      error('ipctb_spm_orthviews(''quiver'', ''init'',...): Please specify 3 images!');
    end;
    if nargin > 4
      if ~isempty(varargin{5})
	st.vols{volhandle}.quiver.fa = ipctb_spm_vol(varargin{5});
      end;
    end;
    if nargin > 5
  ...
  case 'redraw'
    ...
    if isfield(st.vols{volhandle},'quiver')
      ...
      if ~isempty(fa)
	fat = ipctb_spm_slice_vol(fa,inv(TM0*(st.Space\Mx)),TD,0)';
      else
	fat = 1;
      end;
      rqt = cat(3, ipctb_spm_slice_vol(qx,inv(TM0*(st.Space\Mx)),TD,0)', ...
	  ipctb_spm_slice_vol(qy,inv(TM0*(st.Space\My)),TD,0)', ...
	  ipctb_spm_slice_vol(qz,inv(TM0*(st.Space\Mz)),TD,0)');
      ...
      maskt = ipctb_spm_slice_vol(mask,inv(TM0*(st.Space\Mm)),TD,0)';
      xt = [1:TD(1)]-.5; 
      yt = [1:TD(2)]-.5;
      zt = zeros(size(qxt));
      ...
      if ~isempty(fa)
	fac = ipctb_spm_slice_vol(fa,inv(CM0*(st.Space\Mx)),CD,0)';
      else
	fac = 1;
      end;
      rqc = cat(3, ipctb_spm_slice_vol(qx,inv(CM0*(st.Space\Mx)),CD,0)', ...
	  ipctb_spm_slice_vol(qy,inv(CM0*(st.Space\My)),CD,0)', ...
	  ipctb_spm_slice_vol(qz,inv(CM0*(st.Space\Mz)),CD,0)');
      ...
      maskc = ipctb_spm_slice_vol(mask,inv(CM0*(st.Space\Mm)),CD,0)';
      xc = [1:CD(1)]-.5;
      yc = [1:CD(2)]-.5;
      zc = zeros(size(qxc));
      ...
      if ~isempty(fa)
	fas = ipctb_spm_slice_vol(fa,inv(SM0*(st.Space\Mx)),SD,0)';
      else
	fas = 1;
      end;   
      rqs = cat(3, ipctb_spm_slice_vol(qx,inv(SM0*(st.Space\Mx)),SD,0)', ...
	  ipctb_spm_slice_vol(qy,inv(SM0*(st.Space\My)),SD,0)', ...
	  ipctb_spm_slice_vol(qz,inv(SM0*(st.Space\Mz)),SD,0)');
      ...
      masks = ipctb_spm_slice_vol(mask,inv(SM0*(st.Space\Mm)),SD,0)';
      xs = [1:SD(1)]-.5;
      ys = [1:SD(2)]-.5;
      zs = zeros(size(qxs));
      ...
      if exist('ipctb_spm_orthviews/quiver3') == 2
	quiverfun = 'quiver3';
      else % fallback
	quiverfun = 'quiver3';
  ...
  case 'context_menu'  
    item0 = uimenu(varargin{3}, 'Label', 'Quiver');
	  ...
	  ['feval(''ipctb_spm_ov_quiver'',''context_init'', ', ...
	  ...
	  'Visible', 'off', 'Tag', ['QUIVER_1_', num2str(volhandle)]);
	    ...
	    ['feval(''ipctb_spm_ov_quiver'',''context_edit'',', ...
	...
	item2_5 = uimenu(item2, 'Label', 'Linewidth', 'Callback', ...
	  ...
	  ['feval(''ipctb_spm_ov_quiver'',''context_delete'', ', ...
  ...
  case 'context_init'
    ...
    Vqfnames = ipctb_spm_select(3,'evec1.*\.img','Components of 1st eigenvector');
    Vmaskfname = ipctb_spm_select(1,'image','Mask image');
    Vfafname = ipctb_spm_select(Inf,'fa.*\.img','Fractional anisotropy image');
    feval('ipctb_spm_ov_quiver','init',volhandle,Vqfnames,Vmaskfname,Vfafname);
  ...
  case 'context_edit'
	...
	in = ipctb_spm_input('Mask threshold {min max}','!+1','e', ...
      ...
      case 'ls'
	in = ipctb_spm_input('Line style','!+1','s', ...
	    st.vols{volhandle}.quiver.ls);
      case 'qst'
	in = ipctb_spm_input('Quiver distance','!+1','e', ...
	    num2str(st.vols{volhandle}.quiver.qst), 1);
      case 'ql'
	in = ipctb_spm_input('Quiver length','!+1','e', ...
	    num2str(st.vols{volhandle}.quiver.ql), 1);
      case 'qw'
	in = ipctb_spm_input('Linewidth','!+1','e', ...
  ...
  case 'context_delete'
    feval('ipctb_spm_ov_quiver','delete',volhandle);
    obj = findobj(0, 'Tag',  ['QUIVER_1_', num2str(volhandle)]);
    set(obj, 'Visible', 'off');
    obj = findobj(0, 'Tag',  ['QUIVER_0_', num2str(volhandle)]);
    set(obj, 'Visible', 'on');
  ...
  otherwise
    ...
    fprintf('ipctb_spm_orthviews(''quiver'',...): Unknown action string %s', cmd);
  end;
