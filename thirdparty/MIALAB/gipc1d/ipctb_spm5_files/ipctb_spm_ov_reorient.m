function ret = ipctb_spm_ov_reorient(varargin)
% Reorient tool - plugin for ipctb_spm_orthviews
%
% This tool provides the capabilities of the reorientation widget in SPMs
% "DISPLAY" for any image displayed within ipctb_spm_orthviews. The control fields
...
% $Id: ipctb_spm_ov_reorient.m 640 2006-09-29 09:53:58Z volkmar $

rev = '$Revision: 640 $';

global st;
if isempty(st)
  error('reorient: This routine can only be called as a plugin for ipctb_spm_orthviews!');
end;

if nargin < 2
  error('reorient: Wrong number of arguments. Usage: ipctb_spm_orthviews(''reorient'', cmd, volhandle, varargin)');
end;
...
switch cmd
 ...
 case 'context_menu'  
  ...
  item1 = uimenu(item0, 'Label', 'All images', 'Callback', ...
		 ['ipctb_spm_orthviews(''reorient'',''context_init'', 0);'],...
		 'Tag', ['REORIENT_0_', num2str(volhandle)]);
  item2 = uimenu(item0, 'Label', 'Current image', 'Callback', ...
		 ['ipctb_spm_orthviews(''reorient'',''context_init'', ', ...
		  num2str(volhandle), ');'],...
		 'Tag', ['REORIENT_0_', num2str(volhandle)]);
 ...
 case 'context_init'
  ...
  handles = ipctb_spm_orthviews('valid_handles');
  ...
  if volhandle == 0 
    ...
    prms = ipctb_spm_imatrix(st.vols{volhandle}.premul);
	...
	'Callback',['ipctb_spm_orthviews(''reorient'',''apply'',',...
		    num2str(volhandle), ');']);
  end;
  for k = handles
    ...
    if any(k == volhandle)
      ...
      set(objs, 'Callback', ...
	       ['ipctb_spm_orthviews(''reorient'',''context_quit'', [', ...
  ...
  for k = 1:numel(labels)
	...
	Finter, 'Style','edit', ...
	'Callback',['ipctb_spm_orthviews(''reorient'',''reorient'',[',...
		    num2str(volhandle),'])'], ... 
 ...
 case 'context_quit'
  ...
  for k = ipctb_spm_orthviews('valid_handles')
    try
 ...
 case 'apply'
  ...
  P = cellstr(ipctb_spm_select(Inf, 'image', {'Image(s) to reorient'}, '', p));
  if ~isempty(P)
    ipctb_spm_progress_bar('Init', numel(P), 'Reorient', 'Images completed');
    for k = 1:numel(P)
      M = ipctb_spm_get_space(P{k});
      ipctb_spm_get_space(P{k},st.vols{volhandle}.premul*M);
      ipctb_spm_progress_bar('Set',k);
    end;
    ipctb_spm_progress_bar('Clear');
    st.vols{volhandle}.reorient.oldpremul = eye(4);
    qu=questdlg({'Image positions are changed!', ...
		 'To make sure images are displayed correctly, it is recommended to quit and restart ipctb_spm_orthviews now.', ... 
  ...
  ipctb_spm_orthviews('reorient','context_quit', volhandle);
 ...
 case 'reorient'
  ...
  for k = volhandle
    st.vols{k}.premul = ipctb_spm_matrix(prms)* ...
 ...
 case 'redraw'
  if numel(st.vols{volhandle}.reorient.e)==10
    ...
    if ncl > 0
      todraw=ipctb_spm_orthviews('valid_handles');
      for d = 1:3
 ...
 otherwise    
  fprintf('ipctb_spm_orthviews(''reorient'', ...): Unknown action %s', cmd);
end;
