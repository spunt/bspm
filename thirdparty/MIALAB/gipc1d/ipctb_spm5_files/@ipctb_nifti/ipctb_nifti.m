function h = ipctb_nifti(varargin)
...
case 0,
    ...
    h = class(h,'ipctb_nifti');

case 1
    if ischar(varargin{1})
        if size(varargin{1},1)>1,
            h = ipctb_nifti(cellstr(varargin{1}));
            return;
        end;
        ...
        if ~isfield(vol.hdr,'magic'),
            ...
            if isfield(extras,'M') && ~isfield(extras,'mat'),
                 extras.mat = extras.M;
                 if ipctb_spm_flip_analyze_images,
        ...
        dat   = ipctb_file_array(vol.iname,dim,[dt,vol.be],offs,slope,inter);
        h     = struct('hdr',vol.hdr,'dat',dat,'extras',extras);
        h     = class(h,'ipctb_nifti');

    elseif isstruct(varargin{1})
        h     = class(varargin{1},'ipctb_nifti');
