...
case {'.'},
    ...
    for i=1:length(varargin),
        val     = varargin{i};
        obji    = class(objs(i),'ipctb_nifti');
        obji    = fun(obji,subs,val);
        objs(i) = struct(obji);
    end;
    obj = class(objs,'ipctb_nifti');
...
case {'.'},
    ...
    for ii=1:numel(objs)
        ...
        if length(subs)>1, % && ~strcmpi(subs(1).subs,{'raw','dat'}),
            val0 = subsref(class(obj,'ipctb_nifti'),subs(1));
