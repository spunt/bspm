function [u,Ps,Ts] = ipctb_spm_uc_FDR(q,df,STAT,n,Vs,Vm)
...
%-----------------------------------------------------------------------
if isstruct(Vs)
  Ts = ipctb_spm_read_vols(Vs(1));
  for i = 2:length(Vs)
    Ts = min(Ts,ipctb_spm_read_vols(Vs(i)));
  end
  if ~isempty(Vm)
    if isstruct(Vm)
      Ts(ipctb_spm_read_vols(Vm)==0) = [];
...
if isstruct(Vs)
  if      STAT == 'Z'
    Ps = (1-ipctb_spm_Ncdf(Ts)).^n;
  elseif  STAT == 'T'
    Ps = (1-ipctb_spm_Tcdf(Ts,df(2))).^n;
  elseif  STAT == 'X'
    Ps = (1-ipctb_spm_Xcdf(Ts,df(2))).^n;
  elseif  STAT == 'F'
    Ps = (1-ipctb_spm_Fcdf(Ts,df)).^n;
  end
else
  Ps = Vs;
end
...
if isempty(I)
  ...
  if isstruct(Vs)
    ...
    if      STAT == 'Z'
      u = ipctb_spm_invNcdf(1-Ps(I).^(1/n));
    elseif  STAT == 'T'
      u = ipctb_spm_invTcdf(1-Ps(I).^(1/n),df(2));
    elseif  STAT == 'X'
      u = ipctb_spm_invXcdf(1-Ps(I).^(1/n),df(2));
    elseif  STAT == 'F'
      u = ipctb_spm_invFcdf(1-Ps(I).^(1/n),df);
