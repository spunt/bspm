function ipctb_spm_render(dat,brt,rendfile)
...
if nargin < 1
	...
	num   = ipctb_spm_input('Number of sets',1,'1 set|2 sets|3 sets',[1 2 3]);
...
%-----------------------------------------------------------------------
if nargin < 3,
	rendfile = ipctb_spm_select(1,'^render.*\.mat$','Render file');
end;
...
if nargin < 2,
	brt = 1;
	if num==1,
		brt = ipctb_spm_input('Style',1,'new|old',[1 NaN], 1);
	end;
	if isfinite(brt),
		brt = ipctb_spm_input('Brighten blobs',1,'none|slightly|more|lots',[1 0.75 0.5 0.25], 1);
	end;
end;
...
if showbar, ipctb_spm_progress_bar('Init', size(dat,1)*length(rend),...
...
for i=1:length(rend),
	...
	if issparse(rend{i}.ren),
		...
		d = size(rend{i}.ren);
		B1 = ipctb_spm_dctmtx(d(1),d(1));
		B2 = ipctb_spm_dctmtx(d(2),d(2));
	...
	if showbar, ipctb_spm_progress_bar('Set', i); end;
end;
if showbar, ipctb_spm_progress_bar('Clear'); end;

if showbar, ipctb_spm_progress_bar('Init', length(dat)*length(rend),...
...
for j=1:length(dat),
	...
	for i=1:length(rend),
		...
		%-------------------------------------------------------
		dep = ipctb_spm_slice_vol(rend{i}.dep,ipctb_spm_matrix([0 0 1])*inv(M2),d2,1);
		...
		if ~isempty(msk),
			...
			X   = ipctb_spm_slice_vol(X0,ipctb_spm_matrix([0 0 1])*M2,size(rend{i}.dep),hld);
			msk = find(X<0);
			X(msk) = 0;
		else,
		...
		if showbar, ipctb_spm_progress_bar('Set', i+(j-1)*length(rend)); end;
	end;
end;
