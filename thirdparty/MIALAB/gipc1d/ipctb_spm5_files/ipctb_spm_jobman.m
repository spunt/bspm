function varargout = ipctb_spm_jobman(varargin)
% UI/Batching stuff
...
if isempty(fg), fg = ipctb_spm_figure('Create','Graphics'); end;
...
if ~isempty(help_box)
    ...
    set(help_box,'String',ipctb_spm_justify(help_box,txt));
    workaround(help_box);
end;
return;
%------------------------------------------------------------------------
...
try
    ...
    [s,ok] = ipctb_spm_select(c.num,c.filter,c.name,sel,dr,uf);
...
otherwise
    ...
    if isfield(c,'extras')
        [val,msg] = ipctb_spm_eeval(val,c.strtype,n,c.extras);
    else
        [val,msg] = ipctb_spm_eeval(val,c.strtype,n);
    end;
...
% Save a batch job
c = get(batch_box,'UserData'); ipctb_spm('Pointer','Watch');
...
[filenames sts] = ipctb_spm_select([1 Inf], 'batch', 'Load job file(s)');
if sts
...
if isfield(c,'prog')
    ...
    disp('--------------------------');
    disp(['Running "' c.name '"']);
    [Finter,unused,CmdLine] = ipctb_spm('FnUIsetup',c.name);
    ipctb_spm('Pointer','Watch');
    ipctb_spm('FigName',[c.name ': running'],Finter,CmdLine);

    if 0
        try
            feval(prog,val);
            ipctb_spm('FigName',[c.name ': done'],Finter,CmdLine);
        catch
            disp(['An error occurred when running "' c.name '"']);
            disp( '--------------------------------');
            disp(lasterr);
            disp( '--------------------------------');
            ipctb_spm('FigName',[c.name ': failed'],Finter,CmdLine);
...
ffiles = ipctb_spm_select('filter', files, ...
                    filter, uf);
%------------------------------------------------------------------------
...
if isempty(fg), fg = ipctb_spm('CreateIntWin'); end;
delete(findobj(fg,'Parent',fg));
t=uicontrol(fg,...
    'Style','listbox',...
    'Units','normalized',...
...
while(1),
    ...
    if ~isempty(help_box),
        ...
        pos = get(help_box,'position');
        pw   = floor(pos(3)/ext(3)*21-4);
        set(help_box,'String',ipctb_spm_justify(pw,hlp));
        workaround(help_box);
        ...
        if isfield(c,'prog'),
            try
                ...
                [Finter,unused,CmdLine] = ipctb_spm('FnUIsetup',c.name);
                ipctb_spm('FigName',[c.name ': setup'],Finter,CmdLine);
            catch
            end;
    ...
    case {'const','files','menu','entry'}
        ...
        case {'files'}
            ...
            if getit,
                ...
                [ci.val{1},ok] = ipctb_spm_select(num,ci.filter,ci.name,sel,dr,uf);
                if ~ok,
        ...
        case {'menu'}
            ...
            if getit,
                ...
                if isempty(dv),
                    ind   = ipctb_spm_input(ci.name,pos,'m',lab,1:length(ci.values));
                else
                    ind   = ipctb_spm_input(ci.name,pos,'m',lab,1:length(ci.values),dv);
                end;
        ...
        case {'entry'}
            ...
            if getit,
                ...
                if isfield(ci,'extras')
                    val = ipctb_spm_input(ci.name,pos,ci.strtype,val,n1,ci.extras);
                else
                    val = ipctb_spm_input(ci.name,pos,ci.strtype,val,n1);
                end;
                ci.val{1} = val;
            end;
        end;
    ...
    case {'repeat'},
        ...
        tmp = ipctb_spm_input(ci.name,pos,'m',lab,0:length(ci.values));
        if tmp,
    ...
    case {'choice'}
        ...
        tmp = ipctb_spm_input(ci.name,pos,'m',lab,1:length(ci.values));
...
if isfield(c,'name'),
    ...
    %end;
    if isfield(c,'help');
        hlp = ipctb_spm_justify(wid,c.help);
        doc = {doc{:},hlp{:}};
    end;
    ...
    case {'files'},
        if length(c.num)==1 && isfinite(c.num(1)) && c.num(1)>=0,
            tmp = ipctb_spm_justify(wid,sprintf('A "%s" file is selected by the user.',c.filter));
        else
            tmp = ipctb_spm_justify(wid,sprintf('"%s" files are selected by the user.\n',c.filter));
        end;
        doc = {doc{:}, '', tmp{:}, ''};
    ...
    case {'entry'},
        ...
        tmp = ipctb_spm_justify(wid,sprintf('%s are entered.',d));
        doc = {doc{:}, '', tmp{:}, ''};
    end;
