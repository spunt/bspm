function varargout = ipctb_spm_get(varargin)
...
CmdLine = ipctb_ica_spm('CmdLine',CmdLine);
...
if CmdLine>0
	...
	%---------------------------------------------------------------
	P = ipctb_spm_get('CmdLine',n,char(Prompt),[],NewWDir);
	...
	%---------------------------------------------------------------
	%-Set up FileSelWin
	[F,cF] = ipctb_spm_get('Initialise','on',n,char(Prompt),Filter,NewWDir);
...
if (exist('ipctb_ica_spm.m')==2), LastDirs=strvcat(LastDirs,ipctb_ica_spm('Dir')); end
...
%-----------------------------------------------------------------------
WS = ipctb_ica_spm('WinScale');				%-Window scaling factors
FS = ipctb_ica_spm('FontSizes');				%-Scaled font sizes
PF = ipctb_spm_platform('fonts');			%-Font names (for this platform)
S0 = get(0,'ScreenSize');			%-Screen size

F  = figure('IntegerHandle','off',...
	'Tag','SelFileWin',...
	'Name',sprintf('%s%s: SPMget',ipctb_ica_spm('ver'),ipctb_ica_spm('GetUser',' (%s)')),...
	'NumberTitle','off',...
...
if strcmp(ipctb_spm_platform('filesys'),'win')
	off=100;
	uicontrol(F,'Style','PopUp','Tag','DrivesPopup',...
		'ToolTipString','drives',...
		'HorizontalAlignment','Left',...
		'ForegroundColor','r',...
		'String',ipctb_spm_get('DrivesPullDownStr'),...
		'Callback',['ipctb_spm_get(''cd''),'...
		    'set(gcbo,''String'',ipctb_spm_get(''DrivesPullDownStr''))'],...
...
if exist('ipctb_spm_help.m')==2
	uicontrol(F,'Style','Pushbutton','String','?',...
		'ToolTipString','help on using ipctb_spm_get',...
		'ForegroundColor','g',...
		'Callback','ipctb_spm_help(''ipctb_spm_get.m'')',...
...
set(F,'KeyPressFcn',...
	'ipctb_spm_get(''FigKeyPressFcn'',get(gcbf,''CurrentCharacter''),gcbf)')
...
if any(abs(ch)==[10,11,19]) % ^J,^K,^S
	...
	if abs(ch)==19 & hPos
		%- ^S - Select
		switch get(h,'Tag')
			case 'IName', ipctb_spm_get('Add',h)
			case 'SelIName', ipctb_spm_get('Delete',h)
			case 'DirName', ipctb_spm_get('cd',get(h,'UserData'))
		end
		return
	end
...
if any(abs(ch)==[32,4,16,2,27]) % Space,^D,^P,^B,Esc
	...
	case 16		%- ^P - Change to MatLab pwd
		ipctb_spm_get('cd',pwd)
	case 2		%- ^B - Keyboard
		ipctb_spm_get('GUI2CmdLine')
	case 27		%- Esc - Reset
		ipctb_spm_get('Reset')
	end
end
...
elseif abs(ch)==13	%- Return - Goto topic / apply filter
	if EditWDir, ipctb_spm_get('cd',str)
		else, ipctb_spm_get('dir'), end
	return
...
case {'off','reset'}
	...
	return
case 'on'
	if isempty(F), 	F=ipctb_spm_get('CreateFig'); end	%-Create SelFileWin
	varargout = {F,cF};				%-Return figure handles
	...
	set(findobj(F,'Tag','Prompt'),...
		...
		'UserData',n)
							%-Reset prompt, & n
	ipctb_spm_get('StatusLine',0,n,F)			%-Reset StatusLine
	...
	if ~isempty(WDir)
		ipctb_spm_get('cd',WDir)			%-Change directory
	else
		ipctb_spm_get('dir')				%-Show current dir.
	end
...
%-----------------------------------------------------------------------
NewDir = ipctb_spm_get('CPath',NewDir,WDir);
...
%-----------------------------------------------------------------------
[Files,Dirs] = ipctb_spm_list_files(WDir,Filter);
if isempty(Dirs)
...
if size(Dirs,1)
	...
	h = text(0,y,deblank(Dirs(i,:)),...
		...
		'ButtonDownFcn','ipctb_spm_get(''cd'',get(gcbo,''UserData''))',...
...
if ~NoComp & size(Items,1)>32
	...
	if Filter(end)=='*'
		[IName,ItemPos] = ipctb_spm_get('FileSummary',Items);
	else
		[IName,ItemPos] = ipctb_spm_get('FileSummary',Items,'front',Filter);
	end
	text(0.45,y0,'Summary View - Click to expand',...
		'Parent',hAxes,...
		'Color','w',...
		'FontSize',ipctb_ica_spm('FontSize',9),...
		'HorizontalAlignment','Center',...
		'FontAngle','Italic',...
		'ButtonDownFcn','ipctb_spm_get(''dir'',[],[],1)',...
		'HandleVisibility','off');
else
...
if size(IName,1)
	...
	h = text(0.4,y,cIName,...
		...
		'UserData',cItems,...
		'Color','k',...
		'ButtonDownFcn','ipctb_spm_get(''Add'')',...
		'HandleVisibility','on');
	if nItems>1;
		h = text(0.39,y-3,int2str(sum(ItemPos(i,:)>0)),...
			'Parent',hAxes,...
			'Color','w',...
			'FontSize',ipctb_ica_spm('FontSize',8),...
			'FontAngle','Italic',...
			'HorizontalAlignment','right',...
			'UserData',cIName,...
			'ButtonDownFcn',...
			'ipctb_spm_get(''dir'',[],get(gcbo,''UserData''),1)',...
			'HandleVisibility','off');
	end
	y = y - dy;
    end
...
if size(S,2)<=3
	...
	tS            = S;
	tS(:,1:3)     = [];
	[null,tI]     = ipctb_spm_get('StrSort',tS);
	[null,hI]     = ipctb_spm_get('StrSort',hS(tI,:));
	I             = tI(hI);
	sS            = S(I,:);	
end
varargout = {sS,I};
...
elseif strcmp(Cend,'end')
	fFnames           = ipctb_spm_get('strfliplr',Fnames);
	fFilter           = fliplr(Filter);
	[FSpecs,FnamePos] = ipctb_spm_get('FileSummary',fFnames,'front',fFilter,len);
	FSpecs            = ipctb_spm_get('strfliplr',FSpecs);
elseif strcmp(Cend,'both')
	[hSpecs,hI] = ipctb_spm_get('FileSummary',Fnames,'front');
	FSpecs      = '';
	FnamePos    = [];
	for i = 1:size(hSpecs,1)
		...
		[tSpecs,tI] = ipctb_spm_get('FileSummary',Cfnames,'end',Filter,len);
...
for h = H'
	...
	set(h,'String',tmp,'Color','b',...
		'Tag','SelIName',...
		'ButtonDownFcn','ipctb_spm_get(''Delete'')')
end
...
%-Update StatusLine
%-----------------------------------------------------------------------
ipctb_spm_get('StatusLine',size(P,1),n,F)
...
h = uicontrol(F,'Style','Text',...
	'String',Prompt,...
	'ToolTipString',Prompt,...
	'FontName',ipctb_spm_platform('font','times'),...
	'FontWeight','Bold',...
	'FontAngle','Italic',...
	'FontSize',ipctb_ica_spm('FontSize',16),...
	'ForegroundColor','k',...
	'HorizontalAlignment','Center',...
	'Position',[010 370 380 022].*WS);
Handles = [Handles, h];
...
PNew = ipctb_spm_get('CmdLine',n,Prompt,P,WDir,1);

if ~strcmp(P(:),PNew(:))
	set(findobj(F,'Tag','P'),'UserData',PNew);
	ipctb_spm_get('dir')
	ipctb_spm_get('StatusLine',size(PNew,1),n,F)
end
...
h = uicontrol(F,'Style','Pushbutton','String','Cancel',...
	'ForegroundColor','r',...
	'Callback','ipctb_spm_get(''EditDone'',''Cancel'')',...
	'Position',[220 347 070 022].*WS);
EditHandles = [EditHandles, h];

h = uicontrol(F,'Style','Pushbutton','String','OK',...
	'ForegroundColor','m',...
	'Callback','ipctb_spm_get(''EditDone'',''OK'')',...
...
if (nP<n.mn | nP>n.mx)
	...
	ipctb_ica_spm('Beep')
	msgbox(str,sprintf('%s%s: %s...',ipctb_ica_spm('ver'),...
		ipctb_ica_spm('GetUser',' (%s)'),mfilename),'error','non-modal')
	drawnow
...
case 'cpath'
%=======================================================================
% cpath = ipctb_spm_get('cpath',path,cwd)
if nargin<3, cwd=pwd; else, cwd=ipctb_spm_get('CPath',varargin{3}); end
...
rootlen = ipctb_spm_platform('rootlen');	%-Length of root paths ('/' & 'C:\' &c)
...
%=======================================================================
% str = ipctb_spm_get('DrivesPullDownStr')
drivestr  = ipctb_spm_win32utils('drives');
n         = length(drivestr);
varargout = ...
...
dir = ipctb_spm_get('CPath',dir);
[P,null] = ipctb_spm_list_files(dir,fil);
...
switch (ipctb_spm_platform('filesys'))
case 'unx'
	rootf = strcmp(path,'/');
case 'win'
