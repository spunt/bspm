function varargout = ipctb_spm_input(varargin)
...
% created (`ipctb_spm('CreateIntWin')`). If figures are available, then the
...
if ~isempty(Prompt) & ischar(Prompt) & Prompt(1)=='!'
	...
	[CmdLine,YPos] = ipctb_spm_input('!CmdLine',YPos);

	if ~CmdLine	%-Setup for GUI use
		%-Locate (or create) figure to work in
		Finter = ipctb_spm_input('!GetWin');
	
		%-Find out which Y-position to use, setup for use
		YPos = ipctb_spm_input('!SetNextPos',YPos,Finter,CmdLine);
	
		%-Determine position of objects
		[FRec,QRec,PRec,RRec]=ipctb_spm_input('!InputRects',YPos,'',Finter);
	end
end
...
%-----------------------------------------------------------------------
	ipctb_spm_input('!PrntPrmpt',[Prompt,strN,strM],TTstr)
...
%-----------------------------------------------------------------------
	...
	hM = ipctb_spm_input('!InptConMen',Finter,[hPrmpt,hDef,h]);
	...
	ipctb_spm_input('!PointerJumpBack',PLoc,cF)
	drawnow
...
%=======================================================================
	...
	if CmdLine
		...
		ipctb_spm_input('!PrntPrmpt',Prompt,'',Title)
		%-Build prompt
		%-------------------------------------------------------
		if ~isempty(Labs) 
	...
	elseif strcmp(lower(Type),'bd')
		...
		k = min(find(strcmp(tmp,...
			questdlg(Prompt,sprintf('%s%s: %s...',ipctb_spm('ver'),...
				ipctb_spm('GetUser',' (%s)'),Title),tmp{:}))));
...
if CmdLine
	...
	%-Print banner prompt
	%---------------------------------------------------------------
	ipctb_spm_input('!PrntPrmpt',Prompt)		%-Display question prompt
	...
	%-Bring window to fore & jump pointer to default button
	[PLoc,cF] = ipctb_spm_input('!PointerJump',RRec,Finter,RRec(3)*0.95);
...
%=======================================================================
	...
	if CmdLine
		ipctb_spm_input('!PrntPrmpt',Prompt)
		nLabels = size(Labels,1);
		...
		if nLabels==1
			...
			%-Figure ContextMenu for shortcuts
			hM = ipctb_spm_input('!InptConMen',Finter,[hPopUp,H]);

			%-Bring window to fore & jump pointer to menu widget
			[PLoc,cF] = ipctb_spm_input('!PointerJump',RRec,Finter);
	
			%-Callback for KeyPresses
			cb=['ipctb_spm_input(''!PullDownKeyPressFcn'',',...
				'findobj(gcf,''Tag'',''',Tag,'''),',...
...
switch lower(Type), case 'm!'
	if XCB, UD.cb=cb; cb = {'ipctb_spm_input(''!m_cb'')'}; end
...
case 'b!'
	...
	for i=1:nLabels
		...
		if XCB, UD.cb=tcb; tcb = {'ipctb_spm_input(''!m_cb'')'}; end
		h = uicontrol(Finter,'Style','Pushbutton',...
			'String',deblank(Labels(i,:)),...
			'ToolTipString','',...
			'Tag',Tag,...
	...
	if length(Prompt)>56
		...
		uicontrol(Finter,'Style','PushButton','String','>',...
			...
			'CallBack',[...
			 ...
			 'ipctb_spm_input(''!dScroll'',get(gcbo,''UserData'')),',...
			 'set(gcbo,''Visible'',''on'')'],...
...
if ischar(i)
	if i(1)=='0' & all(ismember(unique(i(:)),setstr(abs('0'):abs('9'))))
		...
		i = [zeros(1,z), ipctb_spm_input('!iCond',i(z+1:end))'];
	else
...
Finter = ipctb_spm_figure('FindWin',F);
if isempty(Finter)
	if any(get(0,'Children'))
		if isempty(get(gcf,'Tag')), Finter = gcf;
		else, Finter = ipctb_spm('CreateIntWin'); end
	else, Finter = ipctb_spm('CreateIntWin'); end
end
varargout = {Finter};
...
cF = ipctb_spm_figure('FindWin',cF);
if ~isempty(cF), set(0,'CurrentFigure',cF); end


case '!prntprmpt'
%=======================================================================
% ipctb_spm_input('!PrntPrmpt',Prompt,TipStr,Title)
%-Print prompt for CmdLine questioning
...
case '!inputrects'
%=======================================================================
% [Frec,QRec,PRec,RRec,Sz,Se] = ipctb_spm_input('!InputRects',YPos,rec,F)
...
case '!deleteinputobj'
%=======================================================================
% ipctb_spm_input('!DeleteInputObj',F)
if nargin<2, F='Interactive'; else, F=varargin{2}; end
h = ipctb_spm_input('!FindInputObj',F);
delete(h(h>0))
...
if nargin<3, F='Interactive'; else, F=varargin{3}; end
if nargin<2, YPos='+1'; else, YPos=varargin{2}; end
if nargin<4, [CmdLine,YPos]=ipctb_spm_input('!CmdLine',YPos);
...
if nargout<3
	CPos = ipctb_spm_input('!CurrentPos',F);
	hCPos = [];
else
	[CPos,hCPos] = ipctb_spm_input('!CurrentPos',F);
end

if CmdLine
	NPos = 0;
else
	MPos = ipctb_spm_input('!MaxPos',F);
	if ischar(YPos)
		%-Relative YPos
...
[NPos,CPos,hCPos] = ipctb_spm_input('!NextPos',YPos,F,CmdLine);
...
if nargin<3
	...
	F = ipctb_spm_figure('FindWin',F);
	if isempty(F)
		FRec3=ipctb_spm('WinSize','Interactive')*[0;0;0;1];
	else
		%-Get figure size
		Units = get(F,'Units');
		set(F,'Units','pixels')
...
Se   = round(25*min(ipctb_spm('WinScale')));
...
case 'c'
	...
	[p,msg] = ipctb_spm_input('!iCond',str,n,m);
case 'x'
	...
	if ischar(p)
		...
		elseif ~isempty(X) & ~ipctb_spm_SpUtil('allCon',X,p')
