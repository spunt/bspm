function [Y,xY] = ipctb_spm_regions(xSPM,SPM,hReg,xY)
...
if ~length(xSPM.XYZmm)
	ipctb_spm('alert!','No suprathreshold voxels!',mfilename,0);
	Y = []; xY = [];
	return
end
try
	...
	[xyz,i] = ipctb_spm_XYZreg('NearestXYZ',...
		      ipctb_spm_XYZreg('GetCoords',hReg),xSPM.XYZmm);
	xY.xyz  = xyz;
end
...
%-----------------------------------------------------------------------
ipctb_spm_XYZreg('SetCoords',xyz,hReg);


%-Get adjustment options and VOI name
%-----------------------------------------------------------------------
ipctb_spm_input(sprintf('at [%3.0f %3.0f %3.0f]',xY.xyz),1,'d',...
	'VOI time-series extraction')

if ~isfield(xY,'name')
	xY.name = ipctb_spm_input('name of region','!+1','s','VOI');
end
...
if ~isfield(xY,'Ic')
	...
	i     = ipctb_spm_input('adjust data for (select contrast)','!+1','m',Con);
	xY.Ic = q(i);
end
...
if isfield(SPM,'Sess')
	...
	if ~isfield(xY,'Sess')
		s         = length(SPM.Sess);
		if s > 1
			s = ipctb_spm_input('which session','!+1','n1',s,s);
		end
		xY.Sess   = s;
	end
end
...
%-----------------------------------------------------------------------
if ~isfield(xY,'def')
	xY.def    = ipctb_spm_input('VOI definition...','!+1','b',...
...
switch xY.def
	...
	%---------------------------------------------------------------
	if ~isfield(xY,'spec')
		xY.spec = ipctb_spm_input('VOI radius (mm)','!+0','r',0,1,[0,Inf]);
	end
	...
	%---------------------------------------------------------------
	if ~isfield(xY,'spec')
		xY.spec = ipctb_spm_input('box dimensions [x y z] {mm}',...
			'!+0','r','0 0 0',3);
	end
	...
	%---------------------------------------------------------------
	if ~isfield(xY,'spec')
		xY.spec = ipctb_spm_vol(ipctb_spm_select(1,'image','Specify Mask'));
	else
	  if ~isstruct(xY.spec)
	    xY.spec = ipctb_spm_vol(xY.spec);
	  end;
	end;
	mXYZ=inv(xY.spec.mat)*[xSPM.XYZmm;ones(1,size(xSPM.XYZmm,2))];
	tmpQ = ipctb_spm_sample_vol(xY.spec,mXYZ(1,:),mXYZ(2,:),mXYZ(3,:),0);
	...
	case 'cluster'
	%---------------------------------------------------------------
	[x i] = ipctb_spm_XYZreg('NearestXYZ',xyz,xSPM.XYZmm);
	A     = ipctb_spm_clusters(xSPM.XYZ);
	Q     = find(A == A(i));
end
...
%-----------------------------------------------------------------------
y        = ipctb_spm_get_data(SPM.xY.VY,xSPM.XYZ(:,Q));
y        = ipctb_spm_filter(SPM.xX.K,SPM.xX.W*y);
...
if xY.Ic
	...
	%---------------------------------------------------------------
	beta  = ipctb_spm_get_data(SPM.Vbeta,xSPM.XYZ(:,Q));

	%-subtract Y0 = XO*beta,  Y = Yc + Y0 + e
	%---------------------------------------------------------------
	y     = y - ipctb_spm_FcUtil('Y0',SPM.xCon(xY.Ic),SPM.xX.xKXs,beta);
...
[m n]   = size(y);
if m > n
	[v s v] = svd(ipctb_spm_atranspa(y));
	s       = diag(s);
	v       = v(:,1);
	u       = y*v/sqrt(s(1));
else
	[u s u] = svd(ipctb_spm_atranspa(y'));
	s       = diag(s);
	u       = u(:,1);
	v       = y'*u/sqrt(s(1));
end
...
figure(Fgraph);
subplot(2,2,3)
ipctb_spm_dcm_display(xY,[],[],[[1 0 0];[0 1 0]]',64)
