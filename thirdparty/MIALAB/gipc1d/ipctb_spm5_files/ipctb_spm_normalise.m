function params = ipctb_spm_normalise(VG,VF,matname,VWG,VWF,flags)
...
if ischar(VF), VF = ipctb_spm_vol(VF); end;
...
if nargin<4, VWG = ''; end;
if nargin<5, VWF = ''; end;
if ischar(VWG), VWG=ipctb_spm_vol(VWG); end;
if ischar(VWF), VWF=ipctb_spm_vol(VWF); end;                                                                     
...
VF1 = ipctb_spm_smoothto8bit(VF,flags.smosrc);

% Rescale images so that globals are better conditioned
VF1.pinfo(1:2,:) = VF1.pinfo(1:2,:)/ipctb_spm_global(VF1);
for i=1:numel(VG),
    VG1(i) = ipctb_spm_smoothto8bit(VG(i),flags.smoref);
    VG1(i).pinfo(1:2,:) = VG1(i).pinfo(1:2,:)/ipctb_spm_global(VG(i));
end;
...
M         = eye(4); %ipctb_spm_matrix(prms');
ipctb_spm_chi2_plot('Init','Affine Registration','Mean squared difference','Iteration');
[M,scal]  = ipctb_spm_affreg(VG1, VF1, aflags, M);
 
fprintf('Fine Affine Registration..\n');
aflags.WG  = VWG;
aflags.WF  = VWF;
aflags.sep = aflags.sep/2;
[M,scal]   = ipctb_spm_affreg(VG1, VF1, aflags, M,scal);
...
flags.version = 'ipctb_spm_normalise.m 2.12 04/11/26';
flags.date    = date;

params = struct('Affine',Affine, 'Tr',Tr, 'VF',VF, 'VG',VG, 'flags',flags);

if flags.graphics, ipctb_spm_normalise_disp(params,VF); end;
...
% VG        - Template volumes (see ipctb_spm_vol).
...
%-----------------------------------------------------------------------
stabilise = 8;
basX = ipctb_spm_dctmtx(VG(1).dim(1),k(1))*stabilise;
basY = ipctb_spm_dctmtx(VG(1).dim(2),k(2))*stabilise;
basZ = ipctb_spm_dctmtx(VG(1).dim(3),k(3))*stabilise;

dbasX = ipctb_spm_dctmtx(VG(1).dim(1),k(1),'diff')*stabilise;
dbasY = ipctb_spm_dctmtx(VG(1).dim(2),k(2),'diff')*stabilise;
dbasZ = ipctb_spm_dctmtx(VG(1).dim(3),k(3),'diff')*stabilise;
...
for iter=1:nits,
	...
	[Alpha,Beta,Var,fw] = ipctb_spm_brainwarp(VG,VF,Affine,basX,basY,basZ,dbasX,dbasY,dbasZ,T,fwhm,VWG, VWF);
