function varargout = ipctb_spm_list(varargin)
...
% FORMAT TabDat = spm_list('list',SPM,hReg)
    ...
    %----------------------------------------------------------------------
    xyzmm     = ipctb_spm_results_ui('GetCoords');
    ...
    if STAT ~= 'P'
        %-----------------------------------------------------------------------
        Pz              = ipctb_spm_P(1,0,u,df,STAT,1,n,S);
        Pu              = ipctb_spm_P(1,0,u,df,STAT,R,n,S);
        Qu              = ipctb_spm_P_FDR(u,df,STAT,n,QPs);
        [P Pn Em En EN] = ipctb_spm_P(1,k,u,df,STAT,R,n,S);
    ...
    [N Z XYZ A] = ipctb_spm_max(zscores,varargin{2}.XYZ);
    Z           = Z - minz - 1;
    ...
    if STAT~='P'
        if isfield(varargin{2},'VRvp')
            V2R = ipctb_spm_get_data(varargin{2}.VRvp,XYZ);
        else
            V2R = v2r;
        end
        N           = N.*V2R;
    ...
    if STAT ~= 'P'
        Pc    = ipctb_spm_P(c,k,u,df,STAT,R,n,S);            %-Set-level p-value
    else
        Pc    = [];
        set(Hp,'Visible','off')
    end
    ...
    while prod(size(find(isfinite(Z))))
        ...
        if y < min(Num + 1,3)*dy
            ...
            %--------------------------------------------------------------
            h     = text(0.5,-5*dy,...
                sprintf('Page %d',ipctb_spm_figure('#page',Fgraph)),...
                'FontName',PF.helvetica,'FontAngle','Italic',...
                'FontSize',FS(8));

            ipctb_spm_figure('NewPage',[hPage,h])
            hPage = [];
            y     = y0;
        end
        ...
        if STAT ~= 'P'
            Pz      = ipctb_spm_P(1,0,   U,df,STAT,1,n,S);  % uncorrected p value
            Pu      = ipctb_spm_P(1,0,   U,df,STAT,R,n,S);  % FWE-corrected {based on Z)
            Qu      = ipctb_spm_P_FDR(   U,df,STAT,n,QPs);  % FDR-corrected {based on Z)
            [Pk Pn] = ipctb_spm_P(1,N(i),u,df,STAT,R,n,S);  % [un]corrected {based on k)

            if Pz < tol				                  % Equivalent Z-variate
                Ze  = Inf;	 			              % (underflow => can't compute)
            else
                Ze  = ipctb_spm_invNcdf(1 - Pz);
            end
        else
            Pz	= [];
            Pu      = [];
        ...
        h     = text(tCol(11),y,sprintf(TabDat.fmt{11},XYZmm(:,i)),...
            ...
            'ipctb_spm_mip_ui(''SetCoords'',get(gcbo,''UserData''),hMIPax);'],...
            'Interruptible','off','BusyAction','Cancel',...
            'UserData',XYZmm(:,i));

        HlistXYZ = [HlistXYZ, h];
        if ipctb_spm_XYZreg('Edist',xyzmm,XYZmm(:,i))<tol & ~isempty(hReg)
            set(h,'Color','r')
        end
        hPage  = [hPage, h];
        ...
        for i = 1:length(q)
            ...
            if min(sqrt(sum((XYZmm(:,D)-XYZmm(:,d)*ones(1,size(D,2))).^2)))>Dis;
                ...
                if length(D) < Num
                    ...
                    %------------------------------------------------------
                    if STAT ~= 'P'
                        Pz    = ipctb_spm_P(1,0,Z(d),df,STAT,1,n,S);
                        Pu    = ipctb_spm_P(1,0,Z(d),df,STAT,R,n,S);
                        Qu    = ipctb_spm_P_FDR(Z(d),df,STAT,n,QPs);
                        if Pz < tol
                            Ze = Inf;
                        else,   Ze = ipctb_spm_invNcdf(1 - Pz); end
                    else
                        Pz    = [];
                        Pu    = [];
                        Qu    = [];
                        Ze    = ipctb_spm_invNcdf(Z(d));
                    end
                    ...
                    h     = text(tCol(11),y,...
                        ...
                        'ButtonDownFcn',[...
                        'hMIPax = findobj(''tag'',''hMIPax'');',...
                        'ipctb_spm_mip_ui(''SetCoords'',',...
                        'get(gcbo,''UserData''),hMIPax);'],...
                        'Interruptible','off','BusyAction','Cancel',...
                        'UserData',XYZmm(:,d));

                    HlistXYZ = [HlistXYZ, h];
                    if ipctb_spm_XYZreg('Edist',xyzmm,XYZmm(:,d))<tol & ...
                            ~isempty(hReg)
                        set(h,'Color','r')
                    end
                    hPage = [hPage, h];
    ...
    %----------------------------------------------------------------------
    if ipctb_spm_figure('#page',Fgraph)>1
        h = text(0.5,-5*dy,sprintf('Page %d/%d',ipctb_spm_figure('#page',Fgraph)*[1,1]),...
    ...
    uimenu(h,'Separator','on','Label','Print text table',...
        'Tag','TD_TxtTab',...
        'CallBack',...
        'ipctb_spm_list(''txtlist'',get(get(gcbo,''Parent''),''UserData''),3)',...
        'Interruptible','off','BusyAction','Cancel');
    ...
    uimenu(h,'Separator','on','Label','help',...
        'Tag','TD_Xdat',...
        'CallBack','ipctb_spm_help(''ipctb_spm_list'')',...
        'Interruptible','off','BusyAction','Cancel');

    %-Setup registry
    %----------------------------------------------------------------------
    set(hAx,'UserData',struct('hReg',hReg,'HlistXYZ',HlistXYZ))
    ipctb_spm_XYZreg('Add2Reg',hReg,hAx,'ipctb_spm_list');
    ...
    %======================================================================
    % FORMAT TabDat = ipctb_spm_list('listcluster',SPM,hReg)
        ...
        if length(SPM.Z)
            ...
            %--------------------------------------------------------------
            [xyzmm,i] = ipctb_spm_XYZreg('NearestXYZ',...
                ipctb_spm_results_ui('GetCoords'),SPM.XYZmm);
            ipctb_spm_results_ui('SetCoords',SPM.XYZmm(:,i));

            %-Find selected cluster
            %--------------------------------------------------------------
            A         = ipctb_spm_clusters(SPM.XYZ);
            j         = find(A == A(i));
            SPM.Z     = SPM.Z(j);
            SPM.XYZ   = SPM.XYZ(:,j);
        ...
        %------------------------------------------------------------------
        varargout = {ipctb_spm_list('list',SPM,hReg,Num,Dis)};


    %======================================================================
    case 'txtlist'                                 %-Print ASCII text table
    %======================================================================
    % FORMAT ipctb_spm_list('TxtList',TabDat,c)
    ...
    case 'setcoords'                                   %-Co-ordinate change
        %==================================================================
        % FORMAT ipctb_spm_list('SetCoords',xyz,hAx,hReg)
        ...
        [null,i,d] = ipctb_spm_XYZreg('NearestXYZ',xyz,XYZ);
        if d<eps
