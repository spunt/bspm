function varargout=ipctb_spm_figure(varargin)
...
F = ipctb_spm_figure('CreateWin',Tag,Name,Visible);
ipctb_spm_figure('CreateBar',F);
ipctb_spm_figure('FigContextMenu',F);
varargout = {F};
...
F = ipctb_spm_figure('FindWin',Tag);

if isempty(F)
	if ischar(Tag)
		switch Tag, case 'Graphics'
			F = ipctb_spm_figure('Create','Graphics','Graphics');
		case 'Interactive'
			F = ipctb_spm('CreateIntWin');
		end
	end
else
...
ipctb_spm_figure('Print',F,psname);
...
F = ipctb_spm_figure('ParentFig',h(1));
...
if isempty(hNextPage)
	...
	hNextPage = uicontrol(F,'Style','Pushbutton',...
		...
		'Callback','ipctb_spm_figure(''TurnPage'',''+1'',gcbf)',...
...
case 'turnpage'
%=======================================================================
% ipctb_spm_figure('TurnPage',move,F)
...
set(F,'Units',Units)
text(0.5,0.5,str,...
	'FontSize',ipctb_spm('FontSize',80),...
	'FontWeight','Bold',...
...
Rect = ipctb_spm('WinSize','Graphics','raw').*WS;	%-Graphics window rectangle
...
F    = figure(...
	...
	'Toolbar','none');
if ~isempty(Name)
	set(F,'Name',sprintf('%s%s: %s',ipctb_spm('ver'),...
		ipctb_spm('GetUser',' (%s)'),Name),'NumberTitle','off')
end
...
warning('ipctb_spm_figure(''GetWinScale''... is Grandfathered: use spm(''WinScale''')
varargout = {ipctb_spm('WinScale')};


case 'fontsizes'
%=======================================================================
% FS = ipctb_spm_figure('FontSizes',FS)
warning('ipctb_spm_figure(''FontSizes''... is Grandfathered: use spm(''FontSizes''')
if nargin<2, FS=[08,09,11,13,14,6:36]; else FS=varargin{2}; end
varargout = {round(FS*min(ipctb_spm('WinScale')))};
...
uimenu(t1,'Label','Gray',     'CallBack','ipctb_spm_figure(''ColorMap'',''gray'')');
uimenu(t1,'Label','Hot',      'CallBack','ipctb_spm_figure(''ColorMap'',''hot'')');
uimenu(t1,'Label','Pink',     'CallBack','ipctb_spm_figure(''ColorMap'',''pink'')');
uimenu(t1,'Label','Jet','CallBack','ipctb_spm_figure(''ColorMap'',''jet'')');
uimenu(t1,'Label','Gray-Hot', 'CallBack','ipctb_spm_figure(''ColorMap'',''gray-hot'')');
uimenu(t1,'Label','Gray-Cool','CallBack','ipctb_spm_figure(''ColorMap'',''gray-cool'')');
uimenu(t1,'Label','Gray-Pink','CallBack','ipctb_spm_figure(''ColorMap'',''gray-pink'')');
uimenu(t1,'Label','Gray-Jet', 'CallBack','ipctb_spm_figure(''ColorMap'',''gray-jet'')');
t1=uimenu(t0,'Label','Effects');
uimenu(t1,'Label','Invert','CallBack','ipctb_spm_figure(''ColorMap'',''invert'')');
uimenu(t1,'Label','Brighten','CallBack','ipctb_spm_figure(''ColorMap'',''brighten'')');
uimenu(t1,'Label','Darken','CallBack','ipctb_spm_figure(''ColorMap'',''darken'')');
uimenu( F,'Label','Clear','HandleVisibility','off','CallBack','ipctb_spm_figure(''Clear'',gcbf)');
t0=uimenu( F,'Label','SPM-Print','HandleVisibility','off');
%uimenu( F,'Label','SPM-Print','HandleVisibility','off','CallBack','ipctb_spm_figure(''Print'',gcbf)');
t1=uimenu(t0,'Label','default print file','HandleVisibility','off','CallBack','ipctb_spm_figure(''Print'',gcbf)');
t1=uimenu(t0,'Label','other print file','HandleVisibility','off','CallBack','ipctb_spm_figure(''PrintTo'',ipctb_spm_figure(''FindWin'',''Graphics''))');
...
set(0,'ShowHiddenHandles',cSHH)
ipctb_spm_jobman('pulldown');
%=======================================================================


case 'figcontextmenu'
%=======================================================================
% h = ipctb_spm_figure('FigContextMenu',F)
if nargin<2
	F = get(0,'CurrentFigure');
	if isempty(F), error('no figure'), end
else
	F = ipctb_spm_figure('FindWin',varargin{2});
...
if nargin<2, F=gcbf; else F=ipctb_spm_figure('FindWin',varargin{2}); end
if isempty(F)
    return
end
