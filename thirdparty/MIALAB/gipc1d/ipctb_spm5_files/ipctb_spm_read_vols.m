function [Y,XYZ] = ipctb_spm_read_vols(V,mask)
...
%
% For image data types without a representation of NaN (see ipctb_spm_type),
...
for i=1:n, for p=1:V(1).dim(3)
	Y(:,:,p,i) = ipctb_spm_slice_vol(V(i),ipctb_spm_matrix([0 0 p]),V(i).dim(1:2),0);
end, end
...
if mask
	...
	for i=1:n, im(i)=~ipctb_spm_type(V(i).dt(1),'NaNrep'); end
	%-Mask
	Y(Y(:,:,:,im)==0)=NaN;
end
