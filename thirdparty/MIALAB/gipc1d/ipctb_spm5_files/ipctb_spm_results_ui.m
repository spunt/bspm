function varargout = ipctb_spm_results_ui(varargin)
...
% $Id: spm_results_ui.m 656 2006-10-18 19:00:10Z karl $
...
% WS      - WinScale  [Default ipctb_spm('WinScale') ]
% FS      - FontSizes [Default ipctb_spm('FontSizes')]
%
...
%-Initialise 
%-----------------------------------------------------------------------
SPMid      = ipctb_spm('FnBanner',mfilename,SCCSid);
[Finter,Fgraph,CmdLine] = ipctb_spm('FnUIsetup','Stats: Results');
FS         = ipctb_spm('FontSizes');

% clear satfig if it exists
%-----------------------------------------------------------------------
hSat       = findobj('tag','Satellite');
ipctb_spm_figure('clear',hSat);

%-Get thresholded xSPM data and parameters of design
%=======================================================================
[SPM,xSPM] = ipctb_spm_getSPM;
if isempty(xSPM) 
    varargout = {[],[],[]};
    return;
end
...
%=======================================================================
ipctb_spm('FigName',['SPM{',xSPM.STAT,'}: Results'],Finter,CmdLine);


%-Setup results GUI
%-----------------------------------------------------------------------
ipctb_spm_figure('Clear',Finter)
hReg      = ipctb_spm_results_ui('SetupGUI',M,DIM,xSPM,Finter);

%-Setup design interrogation menu
%-----------------------------------------------------------------------
hDesRepUI = ipctb_spm_DesRep('DesRepUI',SPM);
figure(Finter)
...
hMIPax = ipctb_spm_mip_ui(xSPM.Z,xSPM.XYZmm,M,DIM,hMIPax);

ipctb_spm_XYZreg('XReg',hReg,hMIPax,'ipctb_spm_mip_ui');
if xSPM.STAT == 'P'
	str = xSPM.STATstr;
else
...
h     = text(0,24,'SPMresults:','Parent',hResAx,...
	'FontWeight','Bold','FontSize',FS(14));
text(get(h,'Extent')*[0;0;1;0],24,ipctb_spm_str_manip(SPM.swd,'a30'),'Parent',hResAx)
try
...
for ii = nCon:-1:1
	...
	set(gca,'Tag','ConGrphAx',...
		'Box','off','TickDir','out',...
		'XTick',ipctb_spm_DesRep('ScanTick',nPar,10) - 0.5,'XTickLabel','',...
		'XLim',	[0,nPar],...
	...
	set(gca,'Tag','ConGrphAx',...
		'Box','on','TickDir','out',...
		'XTick',ipctb_spm_DesRep('ScanTick',nPar,10),'XTickLabel','',...
		'XLim',	[0,nPar]+0.5,...
...
M      = varargin{2};
DIM    = varargin{3};
Finter = ipctb_spm_figure('GetWin',Finter);
...
hReg    = uicontrol(Finter,'Style','Frame','Position',[001 001 400 190].*WS,...
		'BackgroundColor',ipctb_spm('Colour'));
hFResUi = uicontrol(Finter,'Style','Frame','Position',[008 007 387 178].*WS);

%-Initialise registry in hReg frame object
%-----------------------------------------------------------------------
[hReg,xyz] = ipctb_spm_XYZreg('InitReg',hReg,M,DIM,[0;0;0]);

%-Setup editable XYZ widgets & cross register with registry
%-----------------------------------------------------------------------
hFxyz      = ipctb_spm_results_ui('DrawXYZgui',M,DIM,varargin{4},xyz,Finter);
ipctb_spm_XYZreg('XReg',hReg,hFxyz,'ipctb_spm_results_ui');

%-Set up buttons for results functions
%-----------------------------------------------------------------------
ipctb_spm_results_ui('DrawButts',hReg,DIM,Finter,WS,FS);
...
hReg = varargin{2};
DIM  = varargin{3};
if nargin<4,  Finter = ipctb_spm_figure('FindWin','Interactive');
	else, Finter = varargin{4}; end
if nargin < 5, WS = ipctb_spm('WinScale');	else,	WS = varargin{5}; end
if nargin < 6, FS = ipctb_spm('FontSizes');	else,	FS = varargin{6}; end
...
uicontrol(Finter,'Style','PushButton','String','volume','FontSize',FS(10),...
	...
	'Callback','ipctb_spm_list(''List'',xSPM,hReg);',...
...
uicontrol(Finter,'Style','PushButton','String','cluster','FontSize',FS(10),...
	...
	'Callback','ipctb_spm_list(''ListCluster'',xSPM,hReg);',...
	'Interruptible','on','Enable','on',...
...
uicontrol(Finter,'Style','PushButton','String','S.V.C.','FontSize',FS(10),...
	...
	'Callback','ipctb_spm_VOI(SPM,xSPM,hReg);',...
	'Interruptible','on','Enable','on',...
...
uicontrol(Finter,'Style','PushButton','String','VOI',...
	...
	'Callback','[Y,xY] = ipctb_spm_regions(xSPM,SPM,hReg)',...
	'Interruptible','on','Enable','on',...
...
% Hemodynamic response function not needed for IPC
...
tmp  = { 'ipctb_spm_transverse(''set'',xSPM,hReg)',...
	 'ipctb_spm_sections(xSPM,hReg)',...
	['ipctb_spm_render(	struct(	''XYZ'',	xSPM.XYZ,',...
				'''t'',		xSPM.Z'',',...
				'''mat'',	xSPM.M,',...
				'''dim'',	xSPM.DIM))'],...
	['global prevsect;','ipctb_spm_sections(xSPM,hReg,prevsect)']};

global defaults

if strcmp(defaults.units{3},'mm')
    uicontrol(Finter,'Style','PopUp','String',str,'FontSize',FS(10),...
	    'ToolTipString',cat(2,tstr{:}),...
	    'Callback','ipctb_spm(''PopUpCB'',gcbo)',...
	    'UserData',tmp,...
	    'Interruptible','on','Enable','on',...
...
hExit  = uicontrol(Finter,'Style','PushButton','String','exit',...
	...
	'Callback',['ipctb_spm_clf(''Interactive''), ipctb_spm_clf(''Graphics''),'...
	'close(ipctb_spm_figure(''FindWin'',''Satellite'')),'...
	 'clear'],...
	'Interruptible','on','Enable','on',...
...
hHelp  = uicontrol(Finter,'Style','PushButton','String','?',...
	...
	'Callback','ipctb_spm_help(''ipctb_spm_results_ui'')',...
	'Interruptible','on','Enable','on',...
...
if nargin<6,  Finter=ipctb_spm_figure('FindWin','Interactive');
	else, Finter=varargin{6}; end
if nargin < 5, xyz=[0;0;0]; else, xyz=varargin{5}; end
if nargin < 4, error('Insufficient arguments'), end
DIM     = varargin{3};
M       = varargin{2};
xyz     = ipctb_spm_XYZreg('RoundCoords',xyz,M,DIM);
...
hX   = uicontrol(Finter,'Style','Edit','String',sprintf('%.2f',xyz(1)),...
	...
	'Tag','hX',...
	'Callback','ipctb_spm_results_ui(''EdWidCB'')');
...
if ischar(o) | length(o)>1
	...
	nxyz(d) = o;
	nxyz = ipctb_spm_XYZreg('RoundCoords',nxyz,UD.M,UD.DIM);
end

if abs(xyz(d)-nxyz(d))>0
	UD.xyz = nxyz; set(hFxyz,'UserData',UD)
	if ~isempty(UD.hReg), ipctb_spm_XYZreg('SetCoords',nxyz,UD.hReg,hFxyz); end
	set(hC,'String',sprintf('%.3f',nxyz(d)))
	ipctb_spm_results_ui('UpdateSPMval',UD)
end
...
i  = ipctb_spm_XYZreg('FindXYZ',UD.xyz,UD.XYZ);
...
hFxyz     = ipctb_spm_results_ui('FindXYZframe',hFxyz);
...
if nargin<3, hFxyz=ipctb_spm_results_ui('FindXYZframe'); else, hFxyz=varargin{3}; end
...
%-----------------------------------------------------------------------
if hC <= 0
	[xyz,d] = ipctb_spm_XYZreg('RoundCoords',xyz,UD.M,UD.DIM);
...
if (~isempty(UD.hReg) & UD.hReg~=hC)
	ipctb_spm_XYZreg('SetCoords',xyz,UD.hReg,hFxyz);
end
...
hText = uicontrol(Finter,'Style','Text','String','plot controls',...
	...
	'FontName',ipctb_spm_platform('font','times'),'FontWeight','Normal',...
...
h4 = uicontrol(Finter,'Style','PopUp',...
	...
	'Callback','ipctb_spm_results_ui(''PlotUiCB'')',...
	'Interruptible','on','Enable','on',...
...
set(hGraphUIbg,'UserData',...
	...
	'DeleteFcn','ipctb_spm_results_ui(''Delete'',get(gcbo,''UserData''))')
...
case 'Title'
	h = get(hAx,'Title');
	set(h,'String',ipctb_spm_input('Enter title:',-1,'s+',get(h,'String')))
case 'Xlabel'
	h = get(hAx,'Xlabel');
	set(h,'String',ipctb_spm_input('Enter X axis label:',-1,'s+',get(h,'String')))
case 'Ylabel'
	h = get(hAx,'Ylabel');
	set(h,'String',ipctb_spm_input('Enter Y axis label:',-1,'s+',get(h,'String')))
case 'LineWidth'
	lw = ipctb_spm_input('Enter LineWidth',-1,'e',get(hAx,'LineWidth'),1);
	set(hAx,'LineWidth',lw)
case 'XLim'
	XLim = ipctb_spm_input('Enter XLim',-1,'e',get(hAx,'XLim'),[1,2]);
	set(hAx,'XLim',XLim)
case 'YLim'
	YLim = ipctb_spm_input('Enter YLim',-1,'e',get(hAx,'YLim'),[1,2]);
	set(hAx,'YLim',YLim)
case 'handle'
	varargout={hAx};
otherwise
...
%=======================================================================
% Fgraph = ipctb_spm_results_ui('Clear',F,mode)
...
%-----------------------------------------------------------------------
%ipctb_spm_input('!DeleteInputObj')
...
%=======================================================================
% hMP = ipctb_spm_results_ui('LaunchMP',M,DIM,hReg,hBmp)
...
if ishandle(hMP)
	figure(ipctb_spm_figure('ParentFig',hMP))
	varargout = {hMP};
	return
end

%-Initialise and cross-register MultiPlanar toolbox
hMP = ipctb_spm_XYZreg_Ex2('Create',M,DIM);
ipctb_spm_XYZreg('Xreg',hReg,hMP,'ipctb_spm_XYZreg_Ex2');

%-Setup automatic deletion of MultiPlanar on deletion of results controls
set(hBmp,'Enable','on','UserData',hMP)
set(hBmp,'DeleteFcn','ipctb_spm_results_ui(''delete'',get(gcbo,''UserData''))')
