function varargout = ipctb_spm_sp(varargin)
...
otherwise,
	...
	[ok,str] = ipctb_spm_sp('issetspc',varargin{2}); 
...
case 3
   Y = varargin{3};
   if isempty(Y), varargout = {ipctb_spm_sp(action,sX)}; return, end
...
otherwise
   error(['too many input argument in ipctb_spm_sp ' action]);
...
otherwise
   error('too many input argument in ipctb_spm_sp');
...
case 4
	...
	if ~isnumeric(n), error('~isnumeric(n)'), end;
	Y = varargin{4};
	if isempty(Y), varargout = {ipctb_spm_sp(action,sX,n)}; return, end
	if size(Y,1) ~= sf_s2(sX), error('Dim dont match'); end;
...
case 2	
	switch lower(action), 		
	case {'nopp'}
		varargout = {ipctb_spm_sp('nop',sf_transp(sX))};
	case {'nopp:'}
		varargout = {ipctb_spm_sp('nop:',sf_transp(sX))};
	end %-
case 3	
	switch lower(action), 		
	case {'nopp'}
		varargout = {ipctb_spm_sp('nop',sf_transp(sX),varargin{3})};
	case {'nopp:'}
		varargout = {ipctb_spm_sp('nop:',sf_transp(sX),varargin{3})};
	end %-
otherwise
...
%=======================================================================
% b = ipctb_spm_sp('isinsp',x,c[,tol])
% b = ipctb_spm_sp('isinspp',x,c[,tol])
...
%=======================================================================
% b = ipctb_spm_sp('eachinsp',x,c[,tol])
% b = ipctb_spm_sp('eachinspp',x,c[,tol])
%-Check whether vectors are in row/column space of X
...
if isempty(sX.X)
   if isempty(X2), 
	...
	x2 = ipctb_spm_sp('Set',X2);
	maxtol = max(sX.tol,x2.tol);
	varargout = { all( ipctb_spm_sp('isinsp',sX,X2,maxtol)) & ...
   	all( ipctb_spm_sp('isinsp',x2,sX.X,maxtol) ) };
