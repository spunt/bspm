function ima = ipctb_spm_erode(varargin)
...
if exist('ipctb_spm_dilate_erode')~=3 
   error('ipctb_spm_dilate_erode.c not compiled - see Makefile');
end

if nargin < 1 | nargin > 2 | nargout ~= 1
   error('FORMAT: ima=ipctb_spm_erode(ima) or ima=ipctb_spm_erode(ima,kernel)');
end
...
if nargin > 1
   ...
   elseif length(size(varargin{1})) == 3
      ...
      error('ipctb_spm_erode: ima must be 2- or 3-dimensional');
   end
end

ima = ipctb_spm_dilate_erode(varargin{1},kernel,'erode');
