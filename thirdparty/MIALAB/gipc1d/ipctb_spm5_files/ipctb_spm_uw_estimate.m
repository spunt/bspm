function ds = ipctb_spm_uw_estimate(P,par)
%
...
if nargin < 1 | isempty(P), P = ipctb_spm_select(Inf,'image'); end
if ~isstruct(P), P = ipctb_spm_vol(P); end
...
old_P = P;
if ds.fwhm ~= 0
   ipctb_spm_uw_show('SmoothStart',length(P));
   for i=1:length(old_P)
      ipctb_spm_uw_show('SmoothUpdate',i);
      sfname(i,:) = [tempname '.img,1,1'];
      to_smooth = sprintf('%s,%d,%d',old_P(i).fname,old_P(i).n);
      ipctb_spm_smooth(to_smooth,sfname(i,:),ds.fwhm);
   end
   P = ipctb_spm_vol(sfname);
   ipctb_spm_uw_show('SmoothEnd');
end
...
M          = ipctb_spm_matrix([-xs(1)/ss(1)+1 -ys(1)/ss(2)+1 -zs(1)/ss(3)+1])*inv(diag([ss 1]))*eye(4);
nx         = length(xs);
ny         = length(ys);
nz         = length(zs);
[x,y,z]    = ndgrid(xs,ys,zs);
Bx         = ipctb_spm_dctmtx(P(1).dim(1),ds.order(1),x(:,1,1));
By         = ipctb_spm_dctmtx(P(1).dim(2),ds.order(2),y(1,:,1));
Bz         = ipctb_spm_dctmtx(P(1).dim(3),ds.order(3),z(1,1,:));
dBy        = ipctb_spm_dctmtx(P(1).dim(2),ds.order(2),y(1,:,1),'diff');
...
dispP.dim   = [nx ny nz];
dispP.dt    = [64 ipctb_spm_platform('bigend')];
dispP.pinfo = [1 0]';
p          = ipctb_spm_imatrix(dispP.mat); p = p.*[zeros(1,6) ss 0 0 0];
p(1)       = -mean(1:nx)*p(7);
p(2)       = -mean(1:ny)*p(8);
p(3)       = -mean(1:nz)*p(9);
dispP.mat  = ipctb_spm_matrix(p); clear p;
...
if isfield(ds,'sfP') & ~isempty(ds.sfP)
   T = ds.sfP.mat\ds.M;
   txyz = xyz*T(1:3,:)';
   c = ipctb_spm_bsplinc(ds.sfP,ds.hold);
   ds.sfield = ipctb_spm_bsplins(c,txyz(:,1),txyz(:,2),txyz(:,3),ds.hold);
   ds.sfield = ds.sfield(:);
   clear c txyz;
else
   ds.sfield = [];
...
%
for iter=1:ds.noi
   ipctb_spm_uw_show('NewIter',iter);
   ...
   if iter > 1 & yty > ssq(iter-1)
      ...
      for i=1:nof
         def(:,i)   = ipctb_spm_get_def(Bx, By,Bz,beta((i-1)*prod(ds.order)+1:i*prod(ds.order)));
         ddefa(:,i) = ipctb_spm_get_def(Bx,dBy,Bz,beta((i-1)*prod(ds.order)+1:i*prod(ds.order)));
      end
      ...
      ipctb_spm_uw_show('EndInv');
      tmp = dispP.mat;
      dispP.mat = P(1).mat;
      ipctb_spm_uw_show('FinIter',ssq,def,ds.fot,ds.sot,dispP,ds.q);
      dispP.mat = tmp;
   end
   clear AtA
end
...
% anyone?
%
ep = [1 1 1 180/pi 180/pi 180/pi zeros(1,6)] .* ipctb_spm_imatrix(mT);
...
for i=1:length(P)
   p = [1 1 1 180/pi 180/pi 180/pi zeros(1,6)] .*...
      ipctb_spm_imatrix(inv(P(i).mat) * P(1).mat);
...
if ds.jm, ipctb_spm_uw_show('StartAtA',10);
else ipctb_spm_uw_show('StartAtA',6); end

nof   = prod(size(ds.fot)) + size(ds.sot,1);

AxtAx = uwAtA1(dx.*dx,Bx,By,Bz); ipctb_spm_uw_show('NewAtA',1);
AytAy = uwAtA1(dy.*dy,Bx,By,Bz); ipctb_spm_uw_show('NewAtA',2);
AztAz = uwAtA1(dz.*dz,Bx,By,Bz); ipctb_spm_uw_show('NewAtA',3);
AxtAy = uwAtA1(dx.*dy,Bx,By,Bz); ipctb_spm_uw_show('NewAtA',4);
AxtAz = uwAtA1(dx.*dz,Bx,By,Bz); ipctb_spm_uw_show('NewAtA',5);
AytAz = uwAtA1(dy.*dz,Bx,By,Bz); ipctb_spm_uw_show('NewAtA',6);

if ds.jm
   AjtAj = uwAtA1(ref.*ref,Bx,dBy,Bz); ipctb_spm_uw_show('NewAtA',7);
   AxtAj = uwAtA2( dx.*ref,Bx, By,Bz,Bx,dBy,Bz); ipctb_spm_uw_show('NewAtA',8);
   AytAj = uwAtA2( dy.*ref,Bx, By,Bz,Bx,dBy,Bz); ipctb_spm_uw_show('NewAtA',9);
   AztAj = uwAtA2( dz.*ref,Bx, By,Bz,Bx,dBy,Bz); ipctb_spm_uw_show('NewAtA',10);
end
...
for sl =1:nz
   ...
   ipctb_spm_krutil(Bz(sl,:)'*Bz(sl,:),ipctb_spm_krutil(tmp,Bx,By,1),AtA);
%    AtA  = AtA + kron(Bz(sl,:)'*Bz(sl,:),ipctb_spm_krutil(tmp,Bx,By,1));
end
return
%_______________________________________________________________________
...
for sl =1:nz
   tmp  = reshape(y((sl-1)*nx*ny+1:sl*nx*ny),nx,ny);
   ipctb_spm_krutil(Bz1(sl,:)'*Bz2(sl,:),ipctb_spm_krutil(tmp,Bx1,By1,Bx2,By2),AtA);
%   AtA  = AtA + kron(Bz1(sl,:)'*Bz2(sl,:),ipctb_spm_krutil(tmp,Bx1,By1,Bx2,By2));
end
return
%_______________________________________________________________________
...
ipctb_spm_uw_show('StartAty',length(P));
for scan = 1:length(P)
   ipctb_spm_uw_show('NewAty',scan);
   T    = P(scan).mat\ds.M;
   txyz = xyz*T(1:3,:)';
   if ~(all(beta == 0) & isempty(ds.sfield))
      [idef,jac] = ipctb_spm_get_image_def(scan,ds,def,ddefa);
      txyz(:,2)  = txyz(:,2)+idef;
   end;
   c    = ipctb_spm_bsplinc(P(scan),ds.hold);
   y    = sf(scan) * ipctb_spm_bsplins(c,txyz(:,1),txyz(:,2),txyz(:,3),ds.hold);
   ...
   y_diff(indx) = 0;
   iTcol        = inv(T(1:3,1:3));
   tmpAty       = ipctb_spm_get_def(Bx',By',Bz',([dx dy dz]*iTcol(:,2)).*y_diff);
   if ds.jm ~= 0
      tmpAty = tmpAty + ipctb_spm_get_def(Bx',dBy',Bz',ref.*y_diff);
   end
   for i=1:nof
...
%
ipctb_spm_uw_show('StartRef',length(P));
...
for i=1:length(P)
   ipctb_spm_uw_show('NewRef',i);
   T    = P(i).mat\ds.M;
   txyz = xyz*T(1:3,:)';
   if ~(all(beta == 0) & isempty(ds.sfield))
      [idef,jac] = ipctb_spm_get_image_def(i,ds,def,ddefa);
      txyz(:,2)  = txyz(:,2)+idef;
   end;
   c     = ipctb_spm_bsplinc(P(i),ds.hold);
   f     = ipctb_spm_bsplins(c,txyz(:,1),txyz(:,2),txyz(:,3),ds.hold);
...
gl             = ipctb_spm_global(ref);
sf             = (100 * (sum(ref(indx).*msk(indx)) / sum(msk(indx))) / gl) * sf;
ref            = (100 / gl) * ref;
dispP.dat      = ref;
c              = ipctb_spm_bsplinc(ref,ds.hold);
txyz           = xyz*M(1:3,:)';
[ref,dx,dy,dz] = ipctb_spm_bsplins(c,txyz(:,1),txyz(:,2),txyz(:,3),ds.hold);
ref            = ref.*msk;
dx             =  dx.*msk;
dy             =  dy.*msk;
dz             =  dz.*msk;
...
if rem ~= 0
   iDtD = inv(ipctb_spm_atranspa(D));
   for i=2:length(P)
      P(i).mat = inv(ipctb_spm_matrix((iDtD*Dty{i})'))*P(i).mat;
   end
   [ds.q,ds.p0] = make_q(P,ds.fot,ds.sot,ds.exp_round);
end
ipctb_spm_uw_show('EndRef');
return
%_______________________________________________________________________
...
[idef,jac] = ipctb_spm_get_image_def(1,ds,def,ddefa);
T    = P(1).mat\ds.M;
txyz = xyz*T';
c    = ipctb_spm_bsplinc(P(1),ds.hold);
[m_ref,dx,dy,dz] = ipctb_spm_bsplins(c,txyz(:,1),...
...
if ~isempty(ds.fwhm) && ds.fwhm > 0
   for i=1:length(P)
      ipctb_spm_unlink(P(i).fname);
      [fpath,fname,ext] = fileparts(P(i).fname);
      ipctb_spm_unlink(fullfile(fpath,[fname '.hdr']));
      ipctb_spm_unlink(fullfile(fpath,[fname '.mat']));
   end
end
return;
%_______________________________________________________________________
...
%
ipctb_spm_uw_show('StartRefit',1);
rsP     = P(1);
p       = ipctb_spm_imatrix(rsP.mat);
p       = [zeros(1,6) p(7:9) 0 0 0];
p(1)    = -mean(1:rsP.dim(1))*p(7);
p(2)    = -mean(1:rsP.dim(2))*p(8);
p(3)    = -mean(1:rsP.dim(3))*p(9);
rsP.mat = ipctb_spm_matrix(p); clear p;
[x,y,z] = ndgrid(1:rsP.dim(1),1:rsP.dim(2),1:rsP.dim(3));
xyz     = [x(:) y(:) z(:) ones(numel(x),1)];
txyz    = ((inv(dispP.mat)*rsP.mat)*xyz')';
Bx      = ipctb_spm_dctmtx(rsP.dim(1),ds.order(1));
By      = ipctb_spm_dctmtx(rsP.dim(2),ds.order(2));
Bz      = ipctb_spm_dctmtx(rsP.dim(3),ds.order(3));
...
for i=1:nof
   ...
   c           = ipctb_spm_bsplinc(dispP,ds.hold);
   field       = ipctb_spm_bsplins(c,txyz(:,1),txyz(:,2),txyz(:,3),ds.hold);
   indx        = isnan(field);
   wgt         = ones(size(field));
   wgt(indx)   = 0;
   field(indx) = 0;
   ...
   for sl = 1:nz
      ...
      ipctb_spm_krutil(Bz(sl,:)'*Bz(sl,:),ipctb_spm_krutil(tmp2,Bx,By,1),AtA);
%      AtA   = AtA + kron(Bz(sl,:)'*Bz(sl,:),ipctb_spm_krutil(tmp2,Bx,By,1));
      Aty   = Aty + kron(Bz(sl,:)',ipctb_spm_krutil(tmp1,Bx,By,0));
   end
   beta((i-1)*prod(ds.order)+1:i*prod(ds.order)) = AtA\Aty;
end
ipctb_spm_uw_show('EndRefit');
return;
%_______________________________________________________________________
...
%
ipctb_spm_uw_show('MaskStart',length(P));
...
for i=1:length(P)
   ...
   msk  = msk & tmsk;
   ipctb_spm_uw_show('MaskUpdate',i);
end
%
...
omsk = ipctb_spm_erode(omsk(:),dim+[4 4 4]);
