function ipctb_spm_preproc_write(p,opts)
% Write out VBM preprocessed data
% FORMAT ipctb_spm_preproc_write(p,opts)
% p    - results from ipctb_spm_prep2sn
...
% John Ashburner
% $Id: ipctb_spm_preproc_write.m 539 2006-05-19 17:59:30Z Darren $
...
if numel(p)>0,
    b0  = ipctb_spm_load_priors(p(1).VG);
end;
for i=1:numel(p),
...
x3  = 1:d(3);
d3  = [size(bsol) 1];
B1  = ipctb_spm_dctmtx(d(1),d2(1));
B2  = ipctb_spm_dctmtx(d(2),d2(2));
B3  = ipctb_spm_dctmtx(d(3),d2(3));
bB3 = ipctb_spm_dctmtx(d(3),d3(3),x3);
bB2 = ipctb_spm_dctmtx(d(2),d3(2),x2(1,:)');
bB1 = ipctb_spm_dctmtx(d(1),d3(1),x1(:,1));
...
if opts.biascor,
    ...
    VB = ipctb_spm_create_vol(VB);
end;

lkp = []; for k=1:Kb, lkp = [lkp ones(1,p.flags.ngaus(k))*k]; end;

ipctb_spm_progress_bar('init',length(x3),['Working on ' nam],'Planes completed');
M = p.VG(1).mat\p.flags.Affine*p.VF.mat;

for z=1:length(x3),

    % Bias corrected image
    f          = ipctb_spm_sample_vol(p.VF,x1,x2,o*x3(z),0);
    cr         = exp(transf(bB1,bB2,bB3(z,:),bsol)).*f;
    if opts.biascor,
        % Write a plane of bias corrected data
        VB = ipctb_spm_write_plane(VB,cr,z);
    end;
    ...
    if any(sopts(:)),
        ...
        bt         = zeros([d(1:2) Kb]);
        for k1=1:Kb,
            bt(:,:,k1) = ipctb_spm_sample_priors(b0{k1},t1,t2,t3,k1==Kb);
        end;
        b = zeros([d(1:2) K]);
        for k=1:K,
            b(:,:,k) = bt(:,:,lkp(k))*mg(k);
    ...
    for z=1:length(x3),
        for k1=1:size(sopts,1),
            if sopts(k1,3),
                ...
                ipctb_spm_write_plane(VO(k1),tmp,z);
            end;
        end;
    end;
end;
...
for k1=1:size(sopts,1),
    if any(sopts(k1,1:2)),
        ...
        if sopts(k1,2),
            ipctb_spm_write_sn(VT,p,so);
        end;
        so.preserve = 1;
        if sopts(k1,1),
           VN       = ipctb_spm_write_sn(VT,p,so);
           VN.fname = fullfile(pth,['mwc', num2str(k1), nam, ext]);
           ipctb_spm_write_vol(VN,VN.dat);
        end;
    end;
end;
return;
...
lim = ceil(2*fwhm);
s  = fwhm/sqrt(8*log(2));
x  = [-lim(1):lim(1)]; x = ipctb_spm_smoothkern(fwhm(1),x); x  = x/sum(x);
y  = [-lim(2):lim(2)]; y = ipctb_spm_smoothkern(fwhm(2),y); y  = y/sum(y);
z  = [-lim(3):lim(3)]; z = ipctb_spm_smoothkern(fwhm(3),z); z  = z/sum(z);
i  = (length(x) - 1)/2;
j  = (length(y) - 1)/2;
k  = (length(z) - 1)/2;
ipctb_spm_conv_vol(dat,dat,x,y,z,-[i j k]);
return;
%=======================================================================
...
%-----------------------------------------------------------------------
niter = 32;
ipctb_spm_progress_bar('Init',niter,'Extracting Brain','Iterations completed');
for j=1:niter,
        ...
        ipctb_spm_conv_vol(b,b,kx,ky,kz,-[1 1 1]);
        ipctb_spm_progress_bar('Set',j);
end;
th = 0.05;
for i=1:size(b,3),
        gp       = double(g(:,:,i))/255;
