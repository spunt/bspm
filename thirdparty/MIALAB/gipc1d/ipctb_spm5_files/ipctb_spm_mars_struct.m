function varargout = ipctb_spm_mars_struct(action, varargin)
...
% 
% $Id: mars_struct.m 253 2005-10-13 15:31:34Z guillaume $
...
switch lower(action)  
  ...
  for si=1:maxlen
	...
	if isempty(getfield(a(si), fn)) | ... % a field is empty
	  ...
	  if isstruct(fval) & isstruct(bfc)
	    fval = ipctb_spm_mars_struct('fillafromb',fval,bfc);
	  end
	end
       case 2 % b~a
 ...
 case 'strip'
  [c d] = ipctb_spm_mars_struct('split', a, b);
  varargout = {d};
 ...
 case 'ffillmerge'
  [a b] = ipctb_spm_mars_struct('ffillsplit', a, b);
  varargout = {ipctb_spm_mars_struct('merge', a, b)};
  
 case 'splitmerge'
  [a c] = ipctb_spm_mars_struct('split', a, b);
  varargout = {ipctb_spm_mars_struct('merge', a, b) c};
  
 case 'isthere'
  if isempty(a), varargout = {0}; return, end
  c = ipctb_spm_mars_struct('getifthere', varargin{:});
  varargout = {~isempty(c)};
