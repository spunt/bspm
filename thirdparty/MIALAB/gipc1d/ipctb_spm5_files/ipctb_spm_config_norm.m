function opts = ipctb_spm_config_norm
% Configuration file for normalise jobs
%_______________________________________________________________________
% Copyright (C) 2005 Wellcome Department of Imaging Neuroscience

% John Ashburner
% $Id: ipctb_spm_config_norm.m 512 2006-05-05 08:14:50Z volkmar $
...
eoptions.help = {'Various settinds for estimating warps.'};
...
for i=1:length(job.subj),
	[pth,nam,ext,ind] = ipctb_spm_fileparts(strvcat(job.subj(i).source{:}));
	matname = fullfile(pth,[nam '_sn.mat']);
	ipctb_spm_normalise(strvcat(job.eoptions.template{:}),...
...
for i=1:length(job.subj),
	ipctb_spm_write_sn(strvcat(job.subj(i).resample{:}),...
...
for i=1:length(job.subj),
    [ pth,nam ] = ipctb_spm_fileparts(deblank(job.subj(i).source{:}));
    matname     = fullfile(pth,[nam,'_sn.mat']);
    ipctb_spm_normalise(strvcat(job.eoptions.template{:}),...
        strvcat(job.subj(i).source{:}), matname,...
        strvcat(job.eoptions.weight{:}), strvcat(job.subj(i).wtsrc{:}), eflags);
    ipctb_spm_write_sn(strvcat(job.subj(i).resample{:}), matname, rflags);
end;
return;
%------------------------------------------------------------------------
...
for i=1:length(job.subj),
    [pth,nam,ext,num] = ipctb_spm_fileparts(deblank(job.subj(i).source{:}));
...
for i=1:length(job.subj),
    ...
    for j=1:length(res),
        [pth,nam,ext,num] = ipctb_spm_fileparts(res{j});
