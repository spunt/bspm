function [X,Pnames,Index,idx,jdx,kdx]=ipctb_spm_DesMtx(varargin);
...
elseif nargin>=2 & ~(ischar(varargin{2}) | iscell(varargin{2}))
	[X1,Pnames1]=ipctb_spm_DesMtx(varargin{1});
	[X2,Pnames2]=ipctb_spm_DesMtx(varargin{2:end});
	X=[X1,X2]; Pnames=[Pnames1;Pnames2];
	return
elseif nargin>=3 & ~(ischar(varargin{3}) | iscell(varargin{3}))
	[X1,Pnames1]=ipctb_spm_DesMtx(varargin{1:2});
	[X2,Pnames2]=ipctb_spm_DesMtx(varargin{3:end});
	X=[X1,X2]; Pnames=[Pnames1;Pnames2];
	return
elseif nargin>=4
	[X1,Pnames1]=ipctb_spm_DesMtx(varargin{1:3});
	[X2,Pnames2]=ipctb_spm_DesMtx(varargin{4:end});
	X=[X1,X2]; Pnames=[Pnames1;Pnames2];
	return
else
...
if size(I,2)==1
	...
	[X,Pnames,Index,idx,jdx,kdx] = ipctb_spm_DesMtx(I,[Constraint,'(1)'],FCnames);
	return
end
...
[X,null,sIndex,idx,jdx,kdx]=ipctb_spm_DesMtx(rIndex,[Constraint,'(1)']);
...
%-----------------------------------------------------------------------
if Constraint(1)~='~',	[X,Pnames,Index] = ipctb_spm_DesMtx(F,'-',Fnames);
	else,		[X,Pnames,Index] = ipctb_spm_DesMtx(F,'~',Fnames); end
...
[X,Pnames,Index] = ipctb_spm_DesMtx(I,'-(1)',FCnames);
...
[X,Pnames,Index] = ipctb_spm_DesMtx(I,'-',FCnames);
...
%=======================================================================
% Fnames = ipctb_spm_DesMtx('FNames',Pnames)
...
Fnames = ipctb_spm_str_manip(Fnames,'v');		%- retain only legal characters
X = Fnames;


case {'TeXnames','texnames'}   %-Remove '@' & '*' for TeX interpretation
%=======================================================================
% TPnames = ipctb_spm_DesMtx('TeXnames',Pnames)
if nargin<2, varargout={''}; return, end
TPnames = varargin{2};
...
%=======================================================================
% Map = ipctb_spm_DesMtx('ParMap',aMap)
Map = {	'\mu',		'const';...
	'\theta',	'repl';...
	'\alpha',	'cond';...
	'\gamma',	'subj';...
...
%=======================================================================
% EPnames = ipctb_spm_DesMtx('TeXnames',Pnames,aMap)
if nargin<2, varargout={''}; return, end
