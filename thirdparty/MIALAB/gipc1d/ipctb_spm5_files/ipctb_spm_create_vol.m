function V = ipctb_spm_create_vol(V,varargin)
% Create a volume
% FORMAT V = ipctb_spm_create_vol(V)
% V - image volume information (see spm_vol.m)
%____________________________________________________________________________
% Copyright (C) 2005 Wellcome Department of Imaging Neuroscience

% John Ashburner
% $Id: ipctb_spm_create_vol.m 717 2007-01-17 12:46:33Z john $
...
if ~isfield(V,'dt'),
    V.dt = [ipctb_spm_type('float64') ipctb_spm_platform('bigend')];
end;

dt{1} = ipctb_spm_type(V.dt(1));
...
bits   = ipctb_spm_type(V.dt(1),'bits');
...
dim    = [V.dim(1:3) V.n];
dat    = ipctb_file_array(V.fname,dim,[dt{1} '-' dt{2}],0,V.pinfo(1),V.pinfo(2));
N      = ipctb_nifti;
N.dat  = dat;
N.mat  = V.mat;
N.mat0 = V.mat;
