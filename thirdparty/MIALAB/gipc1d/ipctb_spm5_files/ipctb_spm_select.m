function [t,sts] = ipctb_spm_select(varargin)
% File selector
% FORMAT [t,sts] = ipctb_spm_select(n,typ,mesg,sel,wd,filt,frames)
%     n    - Number of files
...
%
% FORMAT cpath = ipctb_spm_select('CPath',path,cwd)
...
%
% FORMAT [files,dirs]=ipctb_spm_select('List',direc,filt,frames)
...
% John Ashburner
% $Id: ipctb_spm_select.m 735 2007-02-12 15:07:42Z volkmar $
...
if ( ipctb_spm_matlab_version_chk('7') >= 0 ) && isdeployed,
    ...
    for j=1:numel(varargin),
        ...
        if ischar(varargin{j}),
            for i=1:size(varargin{j},1),
                [pth{i} n e v] = ipctb_spm_fileparts(deblank(varargin{j}(i,:)));
                fle{i} = [n e v];
            end;
...
if isempty(pth),
    for k = 1:numel(fle)
    [unused,nam,ext,num] = ipctb_spm_fileparts(fle{k});
    if ~isempty(num),
        ind = [str2num(num) 1 1];
        ind = ind(1);
    else
...
if filt.code==1 && (numel(filt.frames)~=1 || filt.frames(1)~=1),
    ...
    for i=1:numel(f),
        try
            ni = ipctb_nifti(fullfile(dr,f{i}));
...
case {'ipctb_nifti'},   code = 0; ext = {'.*\.nii$','.*\.img$','.*\.NII$','.*\.IMG$'};
