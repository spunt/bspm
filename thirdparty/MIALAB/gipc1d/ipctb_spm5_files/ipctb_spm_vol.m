...
if strcmpi(ext,'.nii') || (strcmpi(ext,'.img') && ...
	...
	if isempty(n), V = ipctb_spm_vol_nifti(p);
	else           V = ipctb_spm_vol_nifti(p,n); end;
    
    if isempty(n) && length(V.private.dat.dim) > 3
        V0(1) = V;
        for i = 2:V.private.dat.dim(4)
            V0(i) = ipctb_spm_vol_nifti(p, i);
        end
        V = V0;
    end
