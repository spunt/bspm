function [SPM] = ipctb_spm_spm(SPM)
% [Re]ML Estimation of a General Linear Model
% FORMAT [SPM] = ipctb_spm_spm(SPM)
%
...
%                     of design matrix
%       - Optional fields are:
%         xX.K      - cell of session-specific structures (see ipctb_spm_filter)
...
Finter   = ipctb_spm('FigName','Stats: estimation...'); ipctb_spm('Pointer','Watch')

%-Get SPM.mat[s] if necessary
%--------------------------------------------------------------------------
if nargin == 0
    P     = cellstr(ipctb_spm_select(Inf,'^SPM\.mat$','Select SPM.mat[s]'));
    for i = 1:length(P)
        swd     = fileparts(P{i});
...
if strcmp(ipctb_spm('CheckModality'), 'EEG')
    ...
    if Ishortcut
        ...
        ipctb_spm('FigName','Stats: done',Finter); ipctb_spm('Pointer','Arrow')
        fprintf('%-40s: %30s\n','Completed',ipctb_spm('time'))
...
if exist(fullfile('.','mask.img'),'file') == 2
    ...
    abort = ipctb_spm_input(str,1,'bd','stop|continue',[1,0],1);
    if abort
        ipctb_spm('FigName','Stats: done',Finter); ipctb_spm('Pointer','Arrow')
        return
    else
...
files = {'^mask\..{3}$','^ResMS\..{3}$','^RPV\..{3}$',...
         ...
         '^ess_.{4}\..{3}$', '^ipctb_spm\w{1}_.{4}\..{3}$'};

for i=1:length(files)
    j = ipctb_spm_select('List',pwd,files{i});
    for k=1:size(j,1)
        ipctb_spm_unlink(deblank(j(k,:)));
    end
end
...
%==========================================================================
xX.xKXs   = ipctb_spm_sp('Set',ipctb_spm_filter(xX.K,W*xX.X));              % KWX
xX.xKXs.X = full(xX.xKXs.X);
xX.pKX    = ipctb_spm_sp('x-',xX.xKXs);                               % projector
...
if ~isfield(xVi,'V')
    ...
    xCon   = ipctb_spm_FcUtil('Set',Fcname,'F','iX0',iX0,xX.xKXs);
    X1o    = ipctb_spm_FcUtil('X1o', xCon(1),xX.xKXs);
    Hsqr   = ipctb_spm_FcUtil('Hsqr',xCon(1),xX.xKXs);
    trRV   = ipctb_spm_SpUtil('trRV',xX.xKXs);
    trMV   = ipctb_spm_SpUtil('trMV',X1o);
    ...
    UF     = ipctb_spm_invFcdf(1 - UFp,[trMV,trRV]);
end
...
YNaNrep  = ipctb_spm_type(VY(1).dt(1),'nanrep');
...
if isfield(xX,'W')
    ...
    VM    = struct('fname',  'mask.img',...
                   'dim',    DIM',...
                   'dt',     [ipctb_spm_type('uint8') ipctb_spm_platform('bigend')],...
                   'mat',    M,...
                   'pinfo',  [1 0 0]',...
                   'descrip','ipctb_spm_spm:resultant analysis mask');
    VM    = ipctb_spm_create_vol(VM);
    ...
    Vbeta(1:nBeta) = deal(struct(...
        'fname',    [],...
        'dim',      DIM',...
        'dt',       [ipctb_spm_type('float32') ipctb_spm_platform('bigend')],...
        'mat',      M,...
        'pinfo',    [1 0 0]',...
        'descrip',  ''));
    for i = 1:nBeta
        Vbeta(i).fname   = sprintf('beta_%04d.img',i);
        Vbeta(i).descrip = sprintf('ipctb_spm_spm:beta (%04d) - %s',i,xX.name{i});
        ipctb_spm_unlink(Vbeta(i).fname)
    end
    Vbeta = ipctb_spm_create_vol(Vbeta);
    ...
    VResMS = struct('fname',    'ResMS.img',...
        ...
        'mat',      M,...
        'pinfo',    [1 0 0]',...
        'descrip',  'ipctb_spm_spm:Residual sum-of-squares');
    VResMS = ipctb_spm_create_vol(VResMS);
    ...
    VResI(1:nSres) = deal(struct(...
        ...
        'mat',      M,...
        'pinfo',    [1 0 0]',...
        'descrip',  'ipctb_spm_spm:Residual image'));

    for i = 1:nSres
        VResI(i).fname   = sprintf('ResI_%04d.img', i);
        VResI(i).descrip = sprintf('ipctb_spm_spm:ResI (%04d)', i);
        ipctb_spm_unlink(VResI(i).fname);
    end
    VResI = ipctb_spm_create_vol(VResI);
...
%==========================================================================
ipctb_spm_progress_bar('Init',100,str,'');
...
for z = 1:zdim              %-loop over planes (2D or 3D data)
    ...
    for bch = 1:nbch            %-loop over blocks
        ...
        for i = 1:length(xM.VM)
            ...
            %--------------------------------------------------------------
            Cm(Cm) = ipctb_spm_get_data(xM.VM(i),j(:,Cm)) > 0;
        end
        ...
        for i = 1:nScan
            ...
            Y(i,Cm)  = ipctb_spm_get_data(VY(i),xyz(:,Cm));
        ...
        if CrS
            ...
            res   = ipctb_spm_sp('r',xX.xKXs,KWY);     %-Residuals
    ...
    if isfield(xX,'W')
        ...
        VM    = ipctb_spm_write_plane(VM, jj, z);
        ...
        for i = 1:nBeta
            ...
            Vbeta(i) = ipctb_spm_write_plane(Vbeta(i), jj, z);
        end
    ...
    ipctb_spm_progress_bar('Set',100*(bch + nbch*(z - 1))/(nbch*zdim));
...
if ~isfield(xVi,'V')
    ...
    %----------------------------------------------------------------------
    if ~s
        ipctb_spm('FigName','Stats: no sign voxels',Finter); ipctb_spm('Pointer','Arrow')
        figure(Finter);
        plot(SPM.xGX.rg)
    ...
    if isstruct(xX.K)
        ...
        for i = 1:length(xX.K)
            ...
            %--------------------------------------------------------------
            fprintf('%-30s- %i\n','  ReML Block',i);
            [Vp,hp]  = ipctb_spm_reml(Cy(q,q),Xp,Qp);
            V(q,q)   = V(q,q) + Vp;
            h(p)     = hp;
        end
    else
        [V,h] = ipctb_spm_reml(Cy,xX.X,xVi.Vi);
    end
...
%==========================================================================
xX.V            = ipctb_spm_filter(xX.K,ipctb_spm_filter(xX.K,W*V*W')');% KWVW'K'
[trRV trRVRV]   = ipctb_spm_SpUtil('trRV',xX.xKXs,xX.V);          % trRV (for X)
...
try
    ...
    [FWHM,VRpv] = ipctb_spm_est_smoothness(VResI,VM);
    R           = ipctb_spm_resels_vol(VM,FWHM)';
end

%-Delete the residuals images
%==========================================================================
for  i = 1:nSres,
    ipctb_spm_unlink([ipctb_spm_str_manip(VResI(i).fname,'r') '.img']);
    ipctb_spm_unlink([ipctb_spm_str_manip(VResI(i).fname,'r') '.hdr']);
    ipctb_spm_unlink([ipctb_spm_str_manip(VResI(i).fname,'r') '.mat']);
end


%-Compute scaled design matrix for display purposes
%--------------------------------------------------------------------------
xX.nKX        = ipctb_spm_DesMtx('sca',xX.xKXs.X,xX.name);
