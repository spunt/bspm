function [def,jac] = ipctb_spm_get_image_def(P,ds,defa,ddefa)
...
if isreal(P)   
   ...
   if ~isfield(P,'mat') P = ipctb_spm_vol(P); end
   T = inv(P.mat) * ds.P(1).mat;
   p = [1 1 1 180/pi 180/pi 180/pi zeros(1,6)] .* ipctb_spm_imatrix(T);
...
if nargin < 3 | isempty(defa)
   Bx = ipctb_spm_dctmtx(ds.P(1).dim(1),ds.order(1));
   By = ipctb_spm_dctmtx(ds.P(1).dim(2),ds.order(2));
   Bz = ipctb_spm_dctmtx(ds.P(1).dim(3),ds.order(3));
   for i=1:prod(size(ds.fot))+size(ds.sot,1)
      def = def + q(i) * ipctb_spm_get_def(Bx,By,Bz,ds.beta(:,i));
   end
else
   def = defa * q';
end
...
if nargout > 1
   if nargin <  4 | isempty(ddefa)
      Bx = ipctb_spm_dctmtx(ds.P(1).dim(1),ds.order(1));
      dBy = ipctb_spm_dctmtx(ds.P(1).dim(2),ds.order(2),'diff');
      Bz = ipctb_spm_dctmtx(ds.P(1).dim(3),ds.order(3));
      for i=1:1:prod(size(ds.fot))+size(ds.sot,1)
         jac = jac + q(i) * ipctb_spm_get_def(Bx,dBy,Bz,ds.beta(:,i));
      end
   else
      jac = jac + ddefa * q';
   end
