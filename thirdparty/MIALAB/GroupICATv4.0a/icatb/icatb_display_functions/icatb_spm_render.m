function varargout = icatb_spm_render(dat,brt,rendfile)
...
elseif nargin < 1
    ...
    if isstruct(dat)
        ...
        num = numel(files);
        for i=1:num
            V  = icatb_spm_vol(files{i});
            d  = icatb_spm_read_vols(V);
            id = find(isfinite(d));
...
for i=1:length(rend)
    ...
    if issparse(rend{i}.ren)
        ...
        d = size(rend{i}.ren);
        B1 = icatb_spm_dctmtx(d(1),d(1));
        B2 = icatb_spm_dctmtx(d(2),d(2));
...
for j=1:length(dat)
    ...
    for i=1:length(rend)
        ...
        %------------------------------------------------------------------
        if ~isempty(d2)
            dep = icatb_spm_slice_vol(rend{i}.dep,icatb_spm_matrix([0 0 1])*inv(M2),d2,1);
        ...
        if ~isempty(msk)
            ...
            X   = icatb_spm_slice_vol(X0,icatb_spm_matrix([0 0 1])*M2,size(rend{i}.dep),hld);
            %msk = find(X<0);
            %X(msk) = 0;
        else
...
MinMaxVals = [mxmx, mnmn];
if (any(MinMaxVals < 0))
    CLIM = [-max(abs(MinMaxVals)), max(abs(MinMaxVals))];
else
    CLIM = [min(MinMaxVals), max(MinMaxVals)];
end
...
if ~isfinite(brt)
    ...
    for i=1:length(rend)
        ...
        cc= load('icatb_colors', 'coldhot_sensitive');
        cmap = gray(64);
        cmap1 = cc.coldhot_sensitive(1:128, :);
        cmap1 = cmap1(1:2:end, :);
        cmap2 = cc.coldhot_sensitive(129:end, :);
        cmap2 = cmap2(1:2:end, :);
        %cmap1 = flipud(cc.coldhot_sensitive(1:4:end, :));
        %cmap2 = cc.hot(1:4:end, :);
        ...
        minCm = 1;
        maxCm = colorLength;
        ren = scaleIm(minCm, maxCm, ren, 2);
        
        msk = find(X < 0);
        if (~isempty(msk))
            minCm = maxCm + 1;
            maxCm = maxCm + colorLength;
            ren(msk) = scaleIm(minCm, maxCm, X(msk), [CLIM(1), 0]);
            cmap = [cmap; cmap1];
        end
        ...
        if (~isempty(msk))
            minCm = maxCm + 1;
            maxCm = maxCm + colorLength;
            ren(msk) = scaleIm(minCm, maxCm, X(msk), [0, CLIM(2)]);
            cmap = [cmap; cmap2];
        end
...
switch lower(varargin{1})
    ...
    case 'setcoords'    % Set co-ordinates
        ...
        set(hMe,'XData',get(hMe,'XData') - pxyz(1) + xyz(2));
...
scaledData = reshape(scaledData, siz);
