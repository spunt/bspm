...
if (strcmpi(modalityType, 'fmri'))
    ...
    if (useTemporalICA)
        reductionStepsToRun = 1;
        if (strcmpi(algoName, 'iva-gl') || strcmpi(algoName, 'moo-icar') || strcmpi(algoName, 'constrained ica (spatial)'))
...
if ((sesInfo.numOfSub*sesInfo.numOfSess == 1) && strcmpi(algoName, 'iva-gl'))
    try
        conserve_disk_space = sesInfo.conserve_disk_space;
    catch
    end
