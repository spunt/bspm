...
if (~strcmpi(modalityType, 'smri'))
    ...
    if strcmpi(modalityType, 'fmri')
        inputText(numParameters).help = struct('title', 'Backreconstruction', 'string', char('Options available are GICA, Regular (GICA2), GICA3, Spatial-temporal regression and MOO-ICAR. GICA2 and GICA3 are not not shown in the GUI but can be called in the batch script. GICA is a more robust tool to back reconstruct components when compared to GICA2 and GICA3 for low model order.', '', ...
            ...
            '3) MOO-ICAR - Aggregate components from group ica step are used to reconstruct individual subject components using MOO-ICAR algorithm.', ...
