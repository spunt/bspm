...
if (useGUI)
    ...
    file_names = icatb_selectEntry('typeEntity', 'file', 'typeSelection', 'single', 'filter', '*.img;*.nii', 'filetype', 'image', 'title', 'Select file ...');
...
if (strcmpi(display_type, 'montage'))
    ...
    if (exist('labels', 'var'))
        title(labels, 'parent', axesH, 'color', FONT_COLOR, 'Horizontalalignment', 'center');
    end
...
elseif (strcmpi(display_type, 'render'))
    ...
    dat   = struct(	'XYZ',	XYZ,...
        ...
        'dim',	V.dim(1:3)');
    global prevrend;
    rendfile = fullfile(fileparts(which('gift.m')), 'icatb_templates', 'render_single_subj.mat');
    prevrend.rendfile = rendfile;
    prevrend.col = [1 0 0; 0 1 0; 0 0 1];
    prevrend.brt = NaN;
    [rend, cmap] = icatb_spm_render(dat, NaN, rendfile);
    rend = concatDat(rend);
