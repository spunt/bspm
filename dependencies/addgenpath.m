function p = addgenpath(d, varargin)
% ADDGENPATH genpath without slprj and SVN folders
%
%   p = addgenpath(d, excl_pattern, maxdepth)
%
    ...
    for i = 1:npath
        ...
        full_path = [full_path; cfull_path];
    end
    ...
    % ===================================================================================
    illegal_patterns = [{sprintf('\\%s\\..+', filesep) sprintf('\\%s@..+', filesep) 'gh-pages' 'private' '\.ignore' '\.svn', '_svn', 'slprj', '\.git', '_archive_'} excl_pattern];
    for i = 1:length(illegal_patterns)
        path_array(~cellfun('isempty',regexp(path_array, illegal_patterns{i}))) = [];
    end
