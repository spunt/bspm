function status = xlwrite(filename, A, sheet, templatefile, range)
...
if exist('org.apache.poi.ss.usermodel.WorkbookFactory', 'class')~=8 ...
    ...
    xlwritedir = whichdir('xlwrite');
    javaaddpath([xlwritedir filesep 'poi_library/poi-3.8-20120326.jar']);
    javaaddpath([xlwritedir filesep 'poi_library/poi-ooxml-3.8-20120326.jar']);
    javaaddpath([xlwritedir filesep 'poi_library/poi-ooxml-schemas-3.8-20120326.jar']);
    javaaddpath([xlwritedir filesep 'poi_library/xmlbeans-2.3.0.jar']);
    javaaddpath([xlwritedir filesep 'poi_library/dom4j-1.6.1.jar']);
    javaaddpath([xlwritedir filesep 'poi_library/stax-api-1.0.1.jar']);
end
...
if nargin < 4 
    templatefile = fullfile(fileparts(mfilename('fullpath')), 'xlwrite-default-workbook.xlsx');
    if ~exist(filename, 'file'), copyfile(templatefile, filename); end
else
...
if nargin < 5 && ~isempty(strfind(sheet,':'))
    range = sheet;
    sheet = [];
end
