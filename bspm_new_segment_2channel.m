function [] = bspm_new_segment_2channel(in,in2)
% BSPM_NEW_SEGMENT
%
%   ARGUMENTS:
...
if nargin<2,disp('USAGE: bspm_new_segment_2channel(in,in2)'); return, end

% make sure image names are cell arrays of strings
if ischar(in), in = cellstr(in); end
if ischar(in2), in2 = cellstr(in2); end

% fix end of image filename cell array
for i = 1:length(in), in(i) = cellstr([in{i} ',1']); end
for i = 1:length(in2), in2(i) = cellstr([in2{i} ',1']); end
...
% build job
% -------------------------------------------------                             
matlabbatch{1}.spm.tools.preproc8.channel(1).vols = in;  % cell array containing paths to all images
matlabbatch{1}.spm.tools.preproc8.channel(1).biasreg = 0.0001;
matlabbatch{1}.spm.tools.preproc8.channel(1).biasfwhm = 60;
matlabbatch{1}.spm.tools.preproc8.channel(1).write = [0 0]; % save [bias_corrected bias_field]
matlabbatch{1}.spm.tools.preproc8.channel(2).vols = in2;  % cell array containing paths to all images
matlabbatch{1}.spm.tools.preproc8.channel(2).biasreg = 0.0001;
matlabbatch{1}.spm.tools.preproc8.channel(2).biasfwhm = 60;
matlabbatch{1}.spm.tools.preproc8.channel(2).write = [0 0]; % save [bias_corrected bias_field]
...
matlabbatch{1}.spm.tools.preproc8.warp.write = [0 0];  % [inverse_deformation forward_deformation]
