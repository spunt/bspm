function [] = bspm4D_level1(image4D, general_info, runs, contrasts, jobname)
% BSPM4D_LEVEL1
%
%   USAGE:  bspm4D_level1(image4D, general_info, runs, contrasts, jobname)
%
%   ARGUMENTS:
%
%       images:                 4D image(s) (1 x N Runs)
%
%       general_info - structure with the following fields:
%           analysis:           path for the analysis directory (path/name)
%           TR:                 repetition time (in seconds)
%           mt_res:             microtime resolution (number of time bins per scan) [default = 16]
%           mt_onset:           microtime onset [default = 1]
...
%           jobname:    matlabbatch .mat file name
%
...
if nargin<5, jobname = []; end
docon = 1; if nargin<4, docon = 0; end
if nargin<3, disp('Give me arguments! bspm4D_level1(image4D, general_info, runs, contrasts, jobname)'); return; end
...
matlabbatch{1}.spm.stats.fmri_spec.dir{1} = general_info.analysis;
matlabbatch{1}.spm.stats.fmri_spec.timing.units = 'secs';
matlabbatch{1}.spm.stats.fmri_spec.timing.RT = general_info.TR;
matlabbatch{1}.spm.stats.fmri_spec.timing.fmri_t = general_info.mt_res;
matlabbatch{1}.spm.stats.fmri_spec.timing.fmri_t0 = general_info.mt_onset; 
matlabbatch{1}.spm.stats.fmri_spec.bases.hrf.derivs(1) = general_info.hrf_derivs(1); % time derivative (0=no, 1=yes)
matlabbatch{1}.spm.stats.fmri_spec.bases.hrf.derivs(2) = general_info.hrf_derivs(2); % dispersion derivative (0=no, 1=yes)
matlabbatch{1}.spm.stats.fmri_spec.volt = 1;
matlabbatch{1}.spm.stats.fmri_spec.global = 'None';
matlabbatch{1}.spm.stats.fmri_spec.mask{1} = general_info.brainmask;
if general_info.autocorrelation==1
    matlabbatch{1}.spm.stats.fmri_spec.cvi = 'AR(1)';
elseif general_info.autocorrelation==2
    matlabbatch{1}.spm.stats.fmri_spec.cvi = 'wls';
else
...
for r = 1:nruns
    ...
    hdr     = spm_vol(char(cimage));
    append  = cellfun(@num2str, num2cell(1:length(hdr))', 'Unif', false);
    cimage  = strcat(cimage, {','}, append);
    
    matlabbatch{1}.spm.stats.fmri_spec.sess(r).scans = cimage;
    for c = 1:length(conditions)
        matlabbatch{1}.spm.stats.fmri_spec.sess(r).cond(c).name = conditions(c).name;
        matlabbatch{1}.spm.stats.fmri_spec.sess(r).cond(c).onset = conditions(c).onsets;
        matlabbatch{1}.spm.stats.fmri_spec.sess(r).cond(c).duration = conditions(c).durations;
        ...
        if isfield(conditions(c), 'parameters')
            ...
            for p = 1:length(parameters)
                matlabbatch{1}.spm.stats.fmri_spec.sess(r).cond(c).pmod(p).name = parameters(p).name;
                matlabbatch{1}.spm.stats.fmri_spec.sess(r).cond(c).pmod(p).param = parameters(p).values;
                matlabbatch{1}.spm.stats.fmri_spec.sess(r).cond(c).pmod(p).poly = 1;
            end
        end  
    end
    
    matlabbatch{1}.spm.stats.fmri_spec.sess(r).multi{1} = '';
    matlabbatch{1}.spm.stats.fmri_spec.sess(r).multi_reg{1} = nuisance;
    matlabbatch{1}.spm.stats.fmri_spec.sess(r).hpf = general_info.hpf;

end
   
% Estimation Job
matlabbatch{2}.spm.stats.fmri_est.spmmat{1} = [general_info.analysis filesep 'SPM.mat'];         
