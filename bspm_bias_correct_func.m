function bspm_bias_correct_func(in)
% BSPM_BIAS_CORRECT
%
%   USAGE: bspm_bias_correct_func(in)
% 
% -------------------------------------------------------------------------
if nargin<1, error('bspm_bias_correct_func(in)'); end
if ischar(in), in = cellstr(in); end
startdir = pwd; 
cd(fileparts(in{1}));
V = spm_vol(in{1});
T = bspm_bias_estimate(V);
for i = 1:length(in)
    V = spm_vol(in{i});
...
function BC = bspm_bias_apply(V,T)
...
if ischar(V),V = spm_vol(V);end;
if ischar(T), s = load(T); T = s.T;end;
nbas           = [size(T) 1];
nbas           = nbas(1:3);
B1             = spm_dctmtx(V(1).dim(1),nbas(1));
B2             = spm_dctmtx(V(1).dim(2),nbas(2));
